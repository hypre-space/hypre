#ifndef _CommBufferDouble_h_
#define _CommBufferDouble_h_

//
// This class (and the CommBufferInt class) is a buffering class in which
// doubles can be progressively accumulated. These doubles are intended to
// be coefficients (from a row of a matrix), and so are associated with an
// equation number (which will be the matrix-row in which they belong).
//
// When all coefficients have been buffered, the user can call
...
// an offsets list (of length numEqns+1, so that the number of coefs for
// each eqn is given by offsets[i+1]-offsets[i]), which gives the offset
// into the corresponding coefficients list at which that equation's
// data begins.
//
// Example usage:
//
...
class CommBufferDouble {

  public:

    CommBufferDouble();
    virtual ~CommBufferDouble();

    void addDoubles(int eqnNum, const double* coefs,
                    int numCoefs, int destProc);

    void buildLists();

    int* pointerToEqnNumbers(int& numEqnNumbers, int destProc);

    double* pointerToDoubles(int& numDoubles, int destProc);
  ...
  private:
    ...
    RealArray* doubles_;        //table of coefficients.
