...

{
   ...
   /*-----------------------------------------------------------------------
    *  Access the CSR vectors for R and  P_ext. Also get sizes of fine and
    *  coarse grids.
    *-----------------------------------------------------------------------*/
  ...
  if (num_procs > 1)
  {
   ...
   for (ic = 0; ic < num_cols_offd_RT; ic++)
   {
      ...
      P_marker[col_map_offd_RT[ic]] = jj_counter;
   ...
   for (ic = 0; ic < num_cols_offd_RT; ic++)
   {
      ...
      /*--------------------------------------------------------------------
       *  Create diagonal entry, RAP_int_{ic,ic}.
       *--------------------------------------------------------------------*/

      ip = col_map_offd_RT[ic];
      P_marker[ip] = jj_counter;
      jj_row_begining = jj_counter;
      RAP_int_data[jj_counter] = zero;
      RAP_int_j[jj_counter] = ip;
      jj_counter++;
   ...
   *RAP_ptr = RAP;
   ...
   if (num_cols_offd_RT) hypre_DestroyCSRMatrix(R_offd);
...
hypre_GeneratePExt( hypre_ParCSRMatrix *P, hypre_ParCSRMatrix *A)
{
   ...
   num_rows_p_ext = recv_vec_starts[num_recvs]-1;
   p_int_i = hypre_CTAlloc(int, send_map_starts[num_sends]);
 ...
 *--------------------------------------------------------------------------*/
   ...
   for (i=0; i < num_sends; i++)
   {
	vec_len = send_map_starts[i+1]-send_map_starts[i];
	MPI_Address(&p_int_i[send_map_starts[i]+1],&displ);
	type = MPI_INT;
	MPI_Type_struct(1,&vec_len,&displ,&type,&send_matrix_types[i]);
   ...
   for (i=0; i < num_recvs; i++)
   {
	...
	MPI_Address(&p_ext_i[recv_vec_starts[i]+1],&displ);
	type = MPI_INT;
	MPI_Type_struct(1,&vec_len,&displ,&type,&recv_matrix_types[i]);
 ...
 *--------------------------------------------------------------------------*/
   hypre_CommPkgSendMPITypes(comm_pkg) = send_matrix_types;
   hypre_CommPkgRecvMPITypes(comm_pkg) = recv_matrix_types;
   comm_handle = hypre_InitializeCommunication(0,comm_pkg,NULL,NULL);
   ...
   for (i=0; i < num_sends; i++)
   {
	for (j = send_map_starts[i]; j < send_map_starts[i+1]; j++)
	{
	    ...
	    {
		p_int_j[counter] = diag_j[k];
		p_int_data[counter] = diag_data[k];
		counter++;
  	    }
	    for (k=offd_i[jrow]; k < offd_i[jrow+1]; k++) 
	    {
		p_int_j[counter] = offd_j[k]+num_cols_diag;
		p_int_data[counter] = offd_data[k];
		counter++;
  	    }
	}
	start_index = p_int_i[send_map_starts[i]];
	num_nonzeros = p_int_i[send_map_starts[i+1]]-start_index;
...
{
   int     *RAP_int_i = hypre_CSRMatrixI(RAP_int);
   int     *RAP_int_j = hypre_CSRMatrixJ(RAP_int);
   double  *RAP_int_data = hypre_CSRMatrixData(RAP_int);
   int     num_cols = hypre_CSRMatrixNumCols(RAP_int);
   ...
   MPI_Datatype *recv_vector_types;
   MPI_Datatype *recv_matrix_types;
   MPI_Datatype *send_matrix_types;
   hypre_CommHandle *comm_handle;
   ...
   send_matrix_types = hypre_CTAlloc(MPI_Datatype, num_sends);
   recv_matrix_types = hypre_CTAlloc(MPI_Datatype, num_recvs);
   recv_vector_types = hypre_CTAlloc(MPI_Datatype, num_recvs);
 
   RAP_ext_i = hypre_CTAlloc(int, send_map_starts[num_sends]);
 ...
 *--------------------------------------------------------------------------*/
   ...
   {
	vec_len = recv_vec_starts[i+1]-recv_vec_starts[i];
	MPI_Address(&RAP_int_i[recv_vec_starts[i]+1],&displ);
	type = MPI_INT;
	MPI_Type_struct(1,&vec_len,&displ,&type,&recv_vector_types[i]);
   	MPI_Type_commit(&recv_vector_types[i]);
   }
 
   for (i=0; i < num_sends; i++)
   {
	vec_len = send_map_starts[i+1]-send_map_starts[i];
	MPI_Address(&RAP_ext_i[send_map_starts[i]+1],&displ);
	type = MPI_INT;
	MPI_Type_struct(1,&vec_len,&displ,&type,&send_matrix_types[i]);
   	MPI_Type_commit(&send_matrix_types[i]);
   }
 ...
 *--------------------------------------------------------------------------*/
   ...
   hypre_CommPkgSendMPITypes(tmp_comm_pkg) = recv_vector_types;
 ...
 *--------------------------------------------------------------------------*/
   ...
   num_rows = send_map_starts[num_sends]-1;
   num_nonzeros = RAP_ext_i[num_rows+1];
			  ...
			  int *partitioning)
{
   ...
   int first_col_diag_RAP = hypre_ParCSRMatrixFirstColDiag(RAP);

   hypre_CommPkg *comm_pkg_RAP;
   int num_recvs_RAP;
   int *recv_procs_RAP;   
 ...
 *--------------------------------------------------------------------------*/
   ...
   {
	while (send_list[i] == -1) i++;
	for (j = i; j < num_new_send_procs; j++)
	{
		if (send_list[j] == send_list[i])
			send_list[j] = -1;
	}
   }

   for (i=0; i < num_new_send_procs; i++)
	if (send_list[i] != -1) num_sends_RAP++;
   ...
   for (i=0; i < num_new_send_procs; i++)
	if (send_list[i] != -1) send_procs_RAP[cnt++] = send_list[i];

/*--------------------------------------------------------------------------
 * generate send_map_starts and send_map_elmts
 *--------------------------------------------------------------------------*/

   send_map_starts_RAP = hypre_CTAlloc(int, num_sends_RAP+1);
   j = 0;
   for (i=0; i < num_sends_RAP; i++)
	MPI_Irecv(&send_map_starts_RAP[i+1],1,MPI_INT,send_procs_RAP[i],0,comm,
		&requests[j++]);

   for (i=0; i < num_recvs_RAP; i++)
   {
	vec_len = recv_vec_starts_RAP[i+1] - recv_vec_starts_RAP[i];
	MPI_Isend(&vec_len,1,MPI_INT, recv_procs_RAP[i],0,comm,&requests[j++]);
   }
   
   MPI_Waitall(j,requests,status);
 
   send_map_starts_RAP[0] = 0; 
   for (i=0; i < num_sends_RAP; i++)
	send_map_starts_RAP[i+1] += send_map_starts_RAP[i]; 

   send_map_elmts_RAP = hypre_CTAlloc(int,send_map_starts_RAP[num_sends_RAP]);

   j = 0;
   for (i=0; i < num_sends_RAP; i++)
   {
	vec_start = send_map_starts_RAP[i];
	vec_len = send_map_starts_RAP[i+1]-vec_start;
	MPI_Irecv(&send_map_elmts_RAP[vec_start],vec_len,MPI_INT,
		send_procs_RAP[i],0,comm,&requests[j++]);
   }

   for (i=0; i < num_recvs_RAP; i++)
   {
	vec_start = recv_vec_starts_RAP[i];
	vec_len = recv_vec_starts_RAP[i+1] - vec_start;
	MPI_Isend(&col_map_offd_RAP[vec_start],vec_len,MPI_INT, 
		recv_procs_RAP[i],0,comm,&requests[j++]);
   }
   
   MPI_Waitall(j,requests,status);

   for (i=0; i < send_map_starts_RAP[num_sends_RAP]; i++)
	send_map_elmts_RAP[i] -= first_col_diag_RAP; 
	
   comm_pkg_RAP = hypre_CTAlloc(hypre_CommPkg,1);

   hypre_CommPkgComm(comm_pkg_RAP) = comm;
   hypre_CommPkgNumSends(comm_pkg_RAP) = num_sends_RAP;
   hypre_CommPkgNumRecvs(comm_pkg_RAP) = num_recvs_RAP;
   hypre_CommPkgSendProcs(comm_pkg_RAP) = send_procs_RAP;
   hypre_CommPkgRecvProcs(comm_pkg_RAP) = recv_procs_RAP;
   hypre_CommPkgRecvVecStarts(comm_pkg_RAP) = recv_vec_starts_RAP;
   hypre_CommPkgSendMapStarts(comm_pkg_RAP) = send_map_starts_RAP;
   hypre_CommPkgSendMapElmts(comm_pkg_RAP) = send_map_elmts_RAP;

   hypre_ParCSRMatrixCommPkg(RAP) = comm_pkg_RAP;
