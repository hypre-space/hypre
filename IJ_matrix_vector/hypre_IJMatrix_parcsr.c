...
hypre_NewIJMatrixParcsr(hypre_IJMatrix *matrix)
{
   ...
   if (aux_matrix)
   {
      local_m = hypre_IJMatrixLocalNumRows(aux_matrix);   
      local_n = hypre_IJMatrixLocalNumCols(aux_matrix);
   }
   else
   {
...
hypre_InitializeIJMatrixParcsr(hypre_IJMatrix *matrix)
{
   ...
   ierr = hypre_IntializeAuxParCSRMatrix(aux_matrix);
		              ...
		              double         *coeffs)
{
   ...
   int *row_starts;
   int *col_starts;
   MPI_Comm comm = IJMatrixComm(matrix);
   int num_procs, my_id;
   int row_local;
   int col_0, col_n;
   int i, temp;
		           ...
		           double         *coeffs)
{
   ...
   int *row_starts;
   int *col_starts;
   MPI_Comm comm = hypre_IJMatrixComm(aux_matrix);
   int num_procs, my_id;
   int row_local;
   int col_0, col_n;
   int i, temp;
