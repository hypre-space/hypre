			      ...
			      hypre_ParCSRMatrix  **P_ptr)
{
  ...
  int              total_old_global_cpts, my_first_old_cpt;
  ...
  MPI_Comm_rank(comm,&my_id);
   ...
   MPI_Bcast(&total_old_global_cpts, 1, MPI_INT, num_procs-1, comm);
#else
   ...
   if (n_fine)
   {
      old_coarse_to_fine = hypre_CTAlloc(int, n_coarse_old);
