...
//  declare the FEInterface Object first 
    ...
    FEI_ISIS linearSystem(MPI_COMM_WORLD, 0);
...
//  FE interface call ----------------------------------------------
    ...
    myErrorCode = linearSystem.initSolveStep(myNumElemBlocks, mySolvType,
                                             numMatrices, matrixIDs,
                                             numRHSs, rhsIDs); 
...
//  FE interface call ----------------------------------------------
    ...
    for(int lincombcount=0; lincombcount<numMatrices; lincombcount++){

    linearSystem.setMatrixID(matrixIDs[lincombcount]);
    linearSystem.setRHSID(rhsIDs[lincombcount][0]);
...
#ifdef dbgTrace
            cout << "args before loadElemSet:" << endl;
            cout << "  index     j     = " << j << endl;
...
//  evaluate the element load vector and stiffness matrices for this workset
    ...
    //    pgm_abort(localRank);
        ...
        int myElemSetID = 0;
//  FE interface call ----------------------------------------------
        myErrorCode = linearSystem.loadElemSetMatrix(myElemSetID,
                                               myNumElems,
                                               myElemIDs,
                                               myElemConn,
                                               myElemStiffnesses,
                                               myElemFormat);
        if (myErrorCode) cout << "error in linearSystem.loadElemSetMatrix" << endl;
    
        myErrorCode = linearSystem.loadElemSetRHS(myElemSetID,
                                               myNumElems,
                                               myElemIDs,
                                               myElemConn,
                                               myElemLoads);
        if (myErrorCode) cout << "error in linearSystem.loadElemSetRHS" << endl;
...
//  FE interface call ----------------------------------------------
    ...
    double scalars[2] = {2.0, 2.0};

    double** rhsScalars = new double*[2];
    rhsScalars[0] = new double[1];
    rhsScalars[1] = new double[1];
    rhsScalars[0][0] = 2.0;
    rhsScalars[1][0] = 2.0;

    linearSystem.setMatScalars(matrixIDs, scalars, 2);
    linearSystem.setRHSScalars(&rhsIDs[0][0], &rhsScalars[0][0], 1);
    linearSystem.setRHSScalars(&rhsIDs[1][0], &rhsScalars[1][0], 1);
