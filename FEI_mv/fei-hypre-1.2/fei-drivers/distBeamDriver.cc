...
//  declare the FEInterface Object first 
    ...
    fei->parameters(1, &string);

    sprintf(string,"outputLevel 1");
    fei->parameters(1, &string);
    delete [] string;
...
//  FE interface call ----------------------------------------------
    ...
    delete [] myElemFieldIDs[0];
    delete [] myElemFieldIDs[1];
    delete [] myElemFieldIDs;
...
//  FE interface call ----------------------------------------------
    ...
    if (numProcessors > 1)
        myErrorCode = fei->initSharedNodeSet(mySharedNodes,
                                                     myLenSharedNodeSet,
                                                     mySharedProcIDs,
                                                     myLenSharedProcIDs);
    if (myErrorCode) 
            cout << "error in fei->initSharedNodeSet" << endl;
...
//  FE interface call ----------------------------------------------
    ...
    if (localRank != masterRank) {
        ...
        myErrorCode = fei->beginLoadNodeSets(myNumBCNodeSets+1);
        if (myErrorCode) 
            cout << "error in fei->beginLoadNodeSets" << endl;
...
//  FE interface call ----------------------------------------------
    //    pgm_abort(localRank);
        ...
        myErrorCode = fei->loadElemSet(myElemSetID, 
                                               ...
                                               myElemLoads, 
                                               myElemFormat);
        if (myErrorCode) cout << "error in fei->loadElementData" << endl;
...
//    get the active node list for this block of elements
    ...
    cout << "putLenList: " << putLenList << endl;
...
//    initialize the active node list solution parameters
    ...
    cout << "putNumBlkActEqns: " << putNumBlkActEqns << endl;
...
//----------------------------------------------------------------
    ...
    strcpy(paramStrings[0],"solver gmres");
