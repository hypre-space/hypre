...
#   define MPI_SUM      0
#   define MPI_MAX      1
int MPI_Comm_size(MPI_Comm comm,int *nprocs) {(void) comm,*nprocs = 1;return 0;}
int MPI_Comm_rank(MPI_Comm comm,int *mypid ) {(void) comm,*mypid  = 0;return 0;}
int MPI_Allgather(void *sendBuf, int sendCnt, MPI_Datatype sendType, 
                  void *recvBuf, int recvCnt, MPI_Datatype recvType, 
                  MPI_Comm comm)
{
   int    i, *iRecvBuf, *iSendBuf;
   double *dRecvBuf, *dSendBuf;
   char   *cRecvBuf, *cSendBuf;
   ...
   {
      case MPI_INT:
           iRecvBuf = (int *)recvBuf;
           iSendBuf = (int *)sendBuf;
           for (i = 0; i < sendCnt; i++) iRecvBuf[i] = iSendBuf[i];
           break;
      case MPI_DOUBLE:
           dRecvBuf = (double *)recvBuf;
           dSendBuf = (double *)sendBuf;
           for (i = 0; i < sendCnt; i++) dRecvBuf[i] = dSendBuf[i];
           break;
      case MPI_CHAR:
           cRecvBuf = (char *)recvBuf;
           cSendBuf = (char *)sendBuf;
           for (i = 0; i < sendCnt; i++) cRecvBuf[i] = cSendBuf[i];
           break;
   }
   return(0);
}
int MPI_Irecv(void *recvBuf, int recvCnt, MPI_Datatype recvType,
...
int MPI_Send(void *sendBuf, int sendCnt, MPI_Datatype sendType,
                  ...
                  MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
   ...
   switch (datatype)
   {
      case MPI_INT:
           iRecvBuf = (int *)recvBuf;
           iSendBuf = (int *)sendBuf;
           for ( i = 0; i < count; i++ ) iRecvBuf[i] = iSendBuf[i];
           break;
      case MPI_DOUBLE:
           dRecvBuf = (double *)recvBuf;
           dSendBuf = (double *)sendBuf;
           for ( i = 0; i < count; i++ ) dRecvBuf[i] = dSendBuf[i];
           break;
      case MPI_CHAR:
           cRecvBuf = (char *)recvBuf;
           cSendBuf = (char *)sendBuf;
           for ( i = 0; i < count; i++ ) cRecvBuf[i] = cSendBuf[i];
           break;
   }
   return 0;
}
