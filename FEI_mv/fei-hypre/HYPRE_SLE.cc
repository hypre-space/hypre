...
void HYPRE_SLE::parameters(int numParams, char **paramStrings) 
{
    ...
    if ( getParam("amgRelaxType",numParams,paramStrings,param) == 1)
    {
       ...
       {
          rtype = 4;
          printf("HYPRE_SLE::parameters - invalid relax type : set to GS.\n");
       }
    ...
    if ( getParam("mlStrongThreshold",numParams,paramStrings,param) == 1)
    {
       ...
       {
          ml_strong_threshold = 0.0;
          printf("HYPRE_SLE::parameters - invalid ml threshold => set to %e\n",
                                          ml_strong_threshold);
       }
    }
...
void HYPRE_SLE::selectPreconditioner(char *name)
{
    ...
    else if ( !strcmp(name, "ml") ) 
    {
       ...
       preconID_ = HYML;
#else
       printf("HYPRE_SLE::selectPreconditioner - MLPACK not declared.\n");
    ...
    switch ( preconID_ ) 
    {
       ...
       case HYPILUT :
            ...
            assert( !ierr );
            HYPRE_ParCSRPilutSetMaxIter( pcg_precond, 1 );
            break;
...
#ifdef MLPACK
       case HYML :
            ierr = HYPRE_ParCSRMLCreate( comm, &pcg_precond );
            break;
#endif
    }
...
void HYPRE_SLE::resetMatrixAndVector(double s)
{
    ...
    if ( s != 0.0 && my_pid == 0 )
    {
       printf("HYPRE_SLE::resetMatrixAndVector - cannot take nonzeros.\n");
       exit(1);
    }
  ...
  const double* values)
{
    ...
    for ( i = 0; i < num; i++ ) // change to 0-based
    {
       ...
       {
          printf("%d : HYPRE_SLE::sumIntoRHSVector - index %d out of range.\n", 
                                  my_pid, indices[i]);
          exit(1);
       }
    }
  ...
  const double* values)
{
    ...
    for ( i = 0; i < num; i++ ) // change to 0-based
    {
       ...
       {
          printf("%d : HYPRE_SLE::putIntoSolnVector - index %d out of range\n", 
                                  my_pid, indices[i]);
          exit(1);
       }
    }
                  ...
                  const double* values, const int* scatterIndices)
{
    ...
    if ( assemble_flag == 1 )
    {
       printf("HYPRE_SLE::sumIntoSystemMatrix error : A already assembled\n");
       exit(1);
    }
    ...
    for ( i = 0; i < numValues; i++ ) // change indices to 0-based
    {
       ...
       {
          printf("%d : HYPRE_SLE::sumIntoSystemMatrix - invalid index %d\n", 
                                  ...
                                  double* gamma, int leng) 
{
    ...
    if ( assemble_flag == 1 )
    {
       printf("HYPRE_SLE::enforceEssentialBC error : A assembled already.\n");
       exit(1);
    }
    ...
    for ( i = 0; i < leng; i++ )
    {
       ...
       {
          printf("%d : HYPRE_SLE::enforceEssentialBC - invalid index %d.\n", 
                                  my_pid, globalEqn[i]);
          exit(1);
       }
    ...
    switch ( solverID_ ) 
    {
       ...
       case HYPCG :
          ...
          switch ( preconID_ ) 
          {
             ...
             case HYPARASAILS :
                  HYPRE_ParCSRParaSailsSetParams(pcg_precond,
                                                 parasails_threshold, 
                                                 parasails_nlevels);
                  HYPRE_ParCSRPCGSetPrecond(pcg_solver,
             ...
             case HYBOOMERAMG :
                  ...
                  HYPRE_ParAMGSetStrongThreshold(pcg_precond, 
                                                 amg_strong_threshold);
...
#endif
                  HYPRE_ParCSRPCGSetPrecond(pcg_solver, HYPRE_ParAMGSolve,
                                   HYPRE_ParAMGSetup, pcg_precond);
                  break;

#ifdef MLPACK
             case HYML :

                  HYPRE_ParCSRMLSetStrongThreshold(pcg_precond, 
                                                   ml_strong_threshold);
                  HYPRE_ParCSRMLSetNumPreSmoothings(pcg_precond, 
                                                    ml_npresmoother);
                  HYPRE_ParCSRMLSetNumPostSmoothings(pcg_precond, 
                                                     ml_npostsmoother);
                  HYPRE_ParCSRMLSetPreSmoother(pcg_precond, 
                                               ml_presmoother_type);
                  HYPRE_ParCSRMLSetPostSmoother(pcg_precond, 
                                                ml_postsmoother_type);
                  HYPRE_ParCSRMLSetDampingFactor(pcg_precond, ml_relax_weight);
                  HYPRE_ParCSRPCGSetPrecond(pcg_solver, 
                                            HYPRE_ParCSRMLSolve,
                                            HYPRE_ParCSRMLSetup, 
                                            pcg_precond);
                  ...
                  if ( my_pid == 0 )
                  {
                     printf("HYPRE_SLE::ML strong threshold = %e\n",
                                        ml_strong_threshold);
                     printf("HYPRE_SLE::ML numsweeps(pre)   = %d\n",
                                        ml_npresmoother);
                     printf("HYPRE_SLE::ML numsweeps(post)  = %d\n",
                                        ml_npostsmoother);
                     printf("HYPRE_SLE::ML smoother (pre)   = %d\n",
                                        ml_presmoother_type);
                     printf("HYPRE_SLE::ML smoother (post)  = %d\n",
                                        ml_postsmoother_type);
                     printf("HYPRE_SLE::ML relax weight     = %e\n",
                                        ml_relax_weight);
                  }
#endif
                  break;
       ...
       case HYGMRES :
          ...
          switch ( preconID_ ) 
          {
             ...
             case HYPARASAILS :
#ifdef DEBUG
                     printf("HYPRE_SLE:: ParaSails - nlevels   = %d\n",
                                         parasails_nlevels);
                     printf("HYPRE_SLE:: ParaSails - threshold = %e\n",
                                         parasails_threshold);
#endif
                  HYPRE_ParCSRParaSailsSetParams(pcg_precond, 
                                                 parasails_threshold,
...
#endif
                  HYPRE_ParCSRGMRESSetPrecond(pcg_solver, HYPRE_ParAMGSolve,
                                   HYPRE_ParAMGSetup, pcg_precond);
                    break;
...
void HYPRE_SLE::buildReducedSystem()
{
    ...
    ierr = HYPRE_IJMatrixCreate(comm,&invA22,invA22GlobalNRows,
                                             invA22GlobalNCols);
    assert(!ierr);
...
void HYPRE_SLE::solveUsingSuperLU(int& status)
{
    ...
    {
       printf("HYPRE_SLE::solveUsingSuperLU error - Configure not called.\n");
       status = -1;
       return;
    }
    if ( StartRow_ != 1 )
...
void HYPRE_SLE::solveUsingSuperLUX(int& status)
{
    ...
    if ( colIndices == NULL || rowLengths == NULL )
    {
       printf("HYPRE_SLE::solveUsingSuperLUX error - Configure not called\n");
       status = -1;
       return;
    }
    if ( StartRow_ != 1 )
    {
       printf("HYPRE_SLE::solveUsingSuperLUX error - row not start at 1\n");
       status = -1;
       return;
    }
    nrows = EndRow_;
...
void HYPRE_SLE::solveUsingY12M(int& status)
{
    ...
    if ( colIndices == NULL || rowLengths == NULL )
    {
       printf("HYPRE_SLE::solveUsingY12M error - Configure not called\n");
       status = -1;
       return;
    }
    if ( StartRow_ != 1 )
...
void fei_hypre_test2(int argc, char *argv[])
{
    ...
    H.parameters(1, &paramString);
    strcpy(paramString, "parasailsNlevels 10");
    H.parameters(1, &paramString);
    ...
    H.selectPreconditioner("ml");

    H.launchSolver(&status);

    if ( status != 1 && my_rank == 0 )
    {
       printf("HYPRE_SLE : solve unsuccessful.\n");
    }
