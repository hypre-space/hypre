...
With these clarifications, the high-order setup procedure is simply
\begin{display}\begin{verbatim}
HYPRE_AMSSetDimension(solver, dim);
HYPRE_AMSSetDiscreteGradient(solver, G);
HYPRE_AMSSetNedelecInterpolation(solver, Pi);
\end{verbatim}\end{display}
...
HYPRE\_AMSSetEdgeConstantVectors instead, since this allows AMS to handle the
...
\begin{display}\begin{verbatim}
HYPRE_AMSSetNedelecInterpolations(solver, Pix, Piy, Piz);
\end{verbatim}\end{display}
