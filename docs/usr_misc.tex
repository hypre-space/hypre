...
The configure option  \file{--enable-shared} is used to build \hypre{} shared 
libraries.  In order to use the resulting shared libraries the user MUST have 
the path to the libraries defined in the environment variable \file{LD\_LIBRARY\_PATH}. 
...
\section{Error flags}
Every \hypre{} function returns an integer, which is used to indicate
errors during execution.
Note that the error flag returned by a given function reflects the
errors from {\em all} previous calls to \hypre{} functions.
In particular, a value of zero means that all \hypre{} functions up to
(and including) the current one have completed successfully.
This new error flags system is being implemented throughout the library,
but currently there are still function that do not support it.

The error flag is a combination of one or few error codes, which are
...
calling \code{HYPRE_GetError()}. If \code{HYPRE_ERROR_ARG}
has occurred the argument index can be obtained from \code{HYPRE_GetErrorArg()}.
To print a description of a given error flag, use
\begin{display} \begin{verbatim}
HYPRE_DescribeError(int ierr, FILE *stream)
\end{verbatim} \end{display}
Finally, if \hypre{} was configured with \code{--print-errors}, additional
