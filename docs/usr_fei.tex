...
There are three slide surface reduction algorithms in \hypre{}.  The first
follows the matrix formulation in this section.  The second is similar 
except that it replaces the eliminated slave equations with identity rows
so that the degree of freedom at each node is preserved.  This is essential
for certain block algorithms such as the smoothed aggregation multilevel
preconditioners.  The third is similar to the second except that it is 
more general and can be applied to problems with intersecting slide surfaces
(sequential only for intersecting slide surfaces).
                                                                                
\subsection{Other Features}
                                                                                
To improve the efficiency of the \hypre{} solvers, a few other features have
been incorporated.  We list a few of these features below :
...
\begin{description}
\item[solver xxx] where xxx specifies one of \code{cg}, 
     \code{gmres} (default), \code{fgmres}, \code{bicgs}, \code{bicgstab},
     \code{tfqmr}, \code{symqmr}, \code{superlu}, or \code{superlux}. 
     The solver type can be followed by \code{override} to specify its 
     priority when multiple solvers are declared at random order.
\item[preconditioner xxx] where xxx is one of \code{diagonal} (default), 
     \code{euclid}, \code{parasails}, \code{boomeramg}, \code{ddilut},
     \code{poly}, \code{blockP}, \code{Uzawa}, or \code{mli}.  Another
     option for xxx is \code{reuse} which allows the preconditioner to be
     reused (this is to be set after a preconditioner has been set up already).
     The preconditioner type can be followed by \code{override} to specify
...
\item[stopCrit xxx] where xxx is one of \code{absolute} or 
     \code{relative} (default) stopping criterion.
\item[superluOrdering xxx] - where xxx specifies one of \code{natural}
     (default) or \code{mmd} (minimum degree ordering).  This ordering
     is used to minimize the number of nonzeros generated in the LU
     decomposition.
\item[superluScale xxx] where xxx specifies one of \code{y} (perform row
     and column scalings before decomposition) or \code{n} (default).
\end{description}
...
\begin{description}
\item[amgCoarsenType xxx] where xxx specifies one of \code{falgout} or
     \code{ruge}, or \code{CLJP} (default) coarsening for BoomerAMG.
\item[amgMeasureType xxx] where xxx specifies one of \code{local} (default)
     or or \code{global}.  This parameter affects how coarsening is performed
     in parallel.
...
\item[amgRelaxType xxx] where xxx is one of \code{jacobi} (Damped Jacobi),
     \code{gs-slow} (sequential Gauss-Seidel), \code{gs-fast} (Gauss-Seidel
     on interior nodes), or \code{hybrid} (default).
...
\item[method xxx] where xxx is either \code{AMGSA} (default), \code{AMGSAe},
...
\item[smoother xxx] where xxx is either \code{Jacobi}, \code{BJacobi}, \code{GS},
     \code{SGS}, \code{HSGS} (SSOR,default), \code{BSGS}, \code{ParaSails},
     \code{MLS}, \code{CGJacobi}, \code{CGBJacobi}, or \code{Chebyshev}.
...
In addition, to use \code{AMGSAe}, the parameter \code{haveSFEI} has to be sent
...
\item[lumpedMassScheme xxx] where is either {\sf diag} (take the diagonal
     of the (1,1) block) or \code{ainv} (take the approximate inverse of
     the (1,1) block.
\item[invA11PSNlevels xxx] where xxx is 0 or 1 to indicate the ParaSails
     nlevels to use to generate the lumped mass matrix \code{ainv} is
     selected.)
\item[invA11PSThresh xxx] where xxx is a floating point number between 0
     ...
     mass matrix (if \code{ainv} is selected.)
\item[A11Solver xxx] where xxx is either \code{cg} or \code{gmres} as solver
     for the (1,1) block.
...
\item[A11MaxIterations xxx] where xxx is maximum number of iterations for
     the (1,1) block.
\item[A11Precon xxx] where xxx is either \code{boomeramg}, \code{euclid}, 
     \code{parasails}, \code{ddilut}, or \code{mli}.
...
\item[A22Solver xxx] where xxx is either \code{cg} or \code{gmres} as solver
     for the (2,2) block.
...
\item[A22MaxIterations xxx] where xxx is maximum number of iterations for
     the (2,2) block.
\item[A22Precon xxx] where xxx is either \code{boomeramg}, \code{euclid}, 
     \code{parasails}, \code{ddilut}, or \code{mli}.
...
\item[A11MaxIterations xxx] where xxx is maximum number of iterations for
     the (1,1) block.
\item[A11Precon xxx] where xxx is either \code{boomeramg},
     \code{euclid}, \code{parasails}, {ddilut}, or \code{mli}.
\item[A11PreconPSNlevels xxx] - ParaSails' nlevels.
\item[A11PreconPSThresh xxx] - ParaSails' threshold.
\item[A11PreconPSFilter xxx] - ParaSails' filter.
\item[A11PreconAMGThresh xxx] - BoomerAMG's threshold.
...
\item[S22Solver xxx] where xxx is either \code{cg} or \code{gmres} as solver
     for the (2,2) block.
...
\item[S22MaxIterations xxx] where xxx is maximum number of iterations for
     the (2,2) block.
\item[S22Precon xxx] where xxx is either \code{boomeramg},
     \code{euclid}, \code{parasails}, {ddilut}, or \code{mli}.
...
\item[setDebug xxx] where xxx is one of \code{slideReduction1},
     \code{slideReduction2}, \code{slideReduction3} (level 1,2,3 
     diagnostics in the slide surface reduction code), 
     \code{printMat} (print the original matrix into a file), 
     \code{printReducedMat} (print the reduced matrix
     into a file),  \code{printSol} (print the solution into a file),
     \code{ddilut} (output diagnostic information for ddilut
     preconditioner setup), and \code{amgDebug} (output diagnostic
...
\section{\hypre{} {\sf LinearSystemCore} Installation}
