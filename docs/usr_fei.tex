...
This solver object is then used to create the external {\tt FEI} object by 
\begin{tabbing}
...
\hspace{0.5in} \= {\tt elemBlkID = 0;} \\
 ...
 \> {\tt feiPtr$->$initElemBlock(elemBlkID, nElems, elemNNodes,}\\
...
\begin{tabbing}
\hspace{0.5in} \= {\tt feiPtr->initSharedNodes(hypreMesh, nShared, sharedIDs,}\\
...
\begin{tabbing}
\hspace{0.5in} \= {\tt feiPtr$->$getBlockNodeIDList(elemBlkID, nNodes, nodeIDList);}
\end{tabbing}
where {\tt nodeIDList} to be returned is a list of nodes in element 
block {\tt elemBlkID}; and
\begin{tabbing}
\hspace{0.5in} \= {\tt feiPtr$->$getBlockNodeSolution(elemBlkID, nNodes, nodeIDList,}\\
