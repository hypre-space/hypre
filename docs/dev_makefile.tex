...
script named \file{bootstrap}, is provided in the \file{config} subdirectory to
relieve developers of needing to know the Autotools constraints.  It is run 
whenever changes are made to the M4 files or the \file{configure.ac} script.

Developers are responsible for:
\begin{enumerate}
\item Creating a file named \file{Makefile.am} in each new subdirectory for which 
      they are responsible.  Usually an existing \file{Makefile.am} can be 
      copied and edited. An existing \file{Makefile.am} must be edited when 
      new source files are added to an existing directory.

\item Adding the name of new source code files to the \file{configure.ac} script 
      (located in the \file{config} subdirectory) and the top-level \file{Makefile.am}.

\item Adding the names of any new files to the \file{Makefile.am} in the \file{lib} subdirectory.

\item Ensuring all changes are made in the appropriate scripts.

\item Running the \file{bootstrap} script to generate a new \file{configure} script.
...
\linebreak{\file{Makefile.in} and \file{Makefile} are regenerated everytime \file{bootstrap}
