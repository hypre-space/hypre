...
\parbox{6in}{\hspace{1in}
             {\tt ierr = HYPRE\_IJVectorSetLocalComponentsInBlock(ij\_vec,}}
\parbox{6in}{\hspace{2.5in}
             {\tt glob\_vec\_index\_start, glob\_vec\_index\_stop,}}
\parbox{6in}{\hspace{2.5in}
             {\tt value\_indices, values);}}
...
\parbox{6in}{\hspace{2.5in}
             {\tt num\_values, glob\_vec\_indices,}}
\parbox{6in}{\hspace{2.5in}
             {\tt value\_indices, values);}}
\caption{Mapping between external array values and
    internal vector data for
