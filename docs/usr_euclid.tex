...
HYPRE_ParCSREuclidCreate(comm, &eu);

//Optionally use either or both of the following two calls to set options.
// 1. pass options from command line or string array.
HYPRE_ParCSREuclidSetParams(eu, argc, argv);

// 2. pass options from a configuration file.
HYPRE_ParCSREuclidSetParamsFromFile(eu, "filename");

//Set Euclid as the preconditioning method for some
//other solver, using the function calls HYPRE_ParCSREuclidSetup
//and HYPRE_ParCSREuclidSolve.  We assume that the pcg_solver
...
{\tt HYPRE\_ParCSREuclidCreate} is called, and parses the command line 
when {\tt HYPRE\_ParCSREuclidSetParams} is called,
...
strcpy( name[1], "-level");
strcpy( name[2], "3");
HYPRE_ParCSREuclidSetParams(solver, 2, name);
\end{verbatim}
\end{display}
...
First, you can call {\tt HYPRE\_ParCSREuclidSetParamsFromFile}
...
\item[-eu\_stats] When Euclid's destructor is called a summary of
                 ...
                 when {\tt HYPRE\_ParCSREuclidPrintParams()} is called.
