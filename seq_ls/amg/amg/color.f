...
c
        jv=ifg(i)
        if(jv.lt.jval0.or.jv.gt.jvalxx) stop 'COLOR - jv out of range'
        icg(i1)=icg(jv)
        ifg(i1)=jv
        icg(jv)=i1
        ifg(icg(i1))=i1
...
c     if(ifg(i0).gt.jvalmx) go to 140
      i1=i0+ishift
...
c
      ...
      if(icdep(iu3,iu3).eq.3) then
        ...
        if(ifg(ii0).ne.0) print *,' fully dependent var has ifg#0'
110     continue
      endif
c
...
180   if(ii.le.0) go to 100
      i=ii-ishift
ccjwr >>>>> 1/26/95 - Is this right? indicates special point.
