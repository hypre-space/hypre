...
HYPRE_Int 
hypre_AMGeSpectralBuildInterpolation(hypre_CSRMatrix     **P_pointer,

				     HYPRE_Int **i_coarseaggregate_coarsedof_pointer,
				     HYPRE_Int **j_coarseaggregate_coarsedof_pointer,
...

{
  ...
  HYPRE_Int *i_AA_coarsedof,*j_AA_coarsedof;
  ...
  /* find "interior" of AEs: ------------------------------------------:
          this is essentially AA_dof;
  ----------------------------------------------------------------------- */


  /* more generally: subdomain can be either 

     face_aggregate or element aggregate: -------------------------------- */



  ierr = matrix_matrix_product(&i_subdomain_dof,
			       &j_subdomain_dof,
  ...
  /*
  i_domain_element = i_AE_element;
  j_domain_element = j_AE_element;

  */

  for (i=0; i < num_AEs; i++)
    {
      hypre_printf("AE %d contains elements: \n", i);
      for (j=i_AE_element[i]; j < i_AE_element[i+1]; j++)
	hypre_printf("%d, ", j_AE_element[j]);

      hypre_printf("\n");

      hypre_printf("domain: %d contains elements: \n", i);
      for (j=i_domain_element[i]; j < i_domain_element[i+1]; j++)
	hypre_printf("%d, ", j_domain_element[j]);
  ...
  ierr=
    hypre_AMGeDomainElementSparseAssemble(i_domain_element, 
					  j_domain_element,
					  num_domains,
  ...
  /* it seems that AE_element == domain_element and
     AE_dof == domain_dof: ----------------------------------------------- */
  ...
  */
  ...
  ierr =  
    hypre_AMGeSchurComplement(i_domain_chord,
			      j_domain_chord,
			      a_domain_chord,
  ...
  for (i=0; i < num_domains; i++)
	...
	rs_(&local_dof_counter, &local_dof_counter, AE, W, &matz, QE, 
      ...
      /* testing eigenpair computations: --------------------------------- 

      hypre_printf("end eigenpair computations: ------------------------------\n");

      for (i_loc=0; i_loc < local_dof_counter; i_loc++)
	{
	  hypre_printf("       eignevector %d: ===========================\n", i_loc);
	  for (j_loc=0; j_loc < local_dof_counter; j_loc++)
	    {
	      Aux1[j_loc] = W[i_loc] * QE[j_loc +i_loc * local_dof_counter];
	      Aux2[j_loc] = 0.e0;
	      for (k_loc=0; k_loc < local_dof_counter; k_loc++)
		Aux2[j_loc] += AE[j_loc + k_loc * local_dof_counter]
		  * QE[k_loc + i_loc * local_dof_counter];
	    
	      hypre_printf("%e %e %e\n",  Aux1[j_loc], Aux2[j_loc],
		     Aux1[j_loc]-Aux2[j_loc]);
	    } 
	  hypre_printf("\n==============================================\n");
	}

      hypre_printf("end testing eigenpair computations: --------------------------\n");
      */
	...
	if (W[i_loc] > tolerance * W[local_dof_counter-1]
	...
	else 
	  {
	    ...
	    /* if (subdomain_coarsedof_counter == 1) break;  */
	  }

      /*
      for (j_loc = local_dof_counter-1; j_loc > i_loc; j_loc--)
	if (tolerance * W[local_dof_counter-1] < W[j_loc]
	    && subdomain_coarsedof_counter > 0)
	  break;
	else 
	  {
	    subdomain_coarsedof_counter++;
	    coarsedof_counter++;
	    i_dof_index[i_local_to_global[j_loc]] = i_local_to_global[j_loc];
	  }

      */

      /* all subdomain dofs are interpolated from the first 
  ...
  for (i=0; i < num_dofs; i++)
    if (i_dof_subdomain[i+1]==i_dof_subdomain[i])
      {
	hypre_printf("ERROR: dof %d does not belong to a subdomain;\n", i);
  ...
  */
  ...
  ierr=
    hypre_AMGeDomainElementSparseAssemble(i_AE_element, 
					  j_AE_element,
					  num_AEs,
  ...
  ierr = matrix_matrix_product(&i_AA_coarsedof,
			       &j_AA_coarsedof,
			       ...
			       num_AEs, num_dofs, num_coarsedofs);

  *i_coarseaggregate_coarsedof_pointer = i_AA_coarsedof;
  *j_coarseaggregate_coarsedof_pointer = j_AA_coarsedof;


  hypre_TFree(i_subdomain_dof);
  hypre_TFree(j_subdomain_dof);

  ierr = matrix_matrix_product(&i_AE_coarsedof,
			       &j_AE_coarsedof,

			       i_AE_dof,
			       j_AE_dof,

			       i_dof_neighbor_coarsedof,
			       j_dof_neighbor_coarsedof,
