...
hypre_AMGeSpectralBuildInterpolation(hypre_CSRMatrix     **P_pointer,
				     ...
				     int **i_coarseaggregate_coarsedof_pointer,
				     int **j_coarseaggregate_coarsedof_pointer,
...

{
  ...
  int *i_AA_coarsedof,*j_AA_coarsedof;
  ...
  */
  ...
  for (i=0; i < num_AEs; i++)
    {
      printf("AE %d contains elements: \n", i);
      for (j=i_AE_element[i]; j < i_AE_element[i+1]; j++)
	printf("%d, ", j_AE_element[j]);

      printf("\n");

      printf("domain: %d contains elements: \n", i);
      for (j=i_domain_element[i]; j < i_domain_element[i+1]; j++)
	printf("%d, ", j_domain_element[j]);
  ...
  */
  ...
  for (i=0; i < num_domains; i++)
      ...
      for (i_loc=0; i_loc < local_dof_counter; i_loc++)
	{
      ...
      printf("end testing eigenpair computations: --------------------------\n");
      */
  ...
  for (i=0; i < num_dofs; i++)
    if (i_dof_subdomain[i+1]==i_dof_subdomain[i])
      {
	printf("ERROR: dof %d does not belong to a subdomain;\n", i);
  ...
  j_dof_neighbor_coarsedof = 
    hypre_CTAlloc(int, dof_neighbor_coarsedof_counter);
