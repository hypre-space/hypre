...
sidlx_rmi_SimReturn
sidlx_rmi_SimReturn__create()
{
  return (*(_getExternals()->createObject))();
}

static sidlx_rmi_SimReturn sidlx_rmi_SimReturn__remote(const char* url,
...
{
  return sidlx_rmi_SimReturn__remote(url, _ex);
}

static struct sidlx_rmi_SimReturn__object* 
  sidlx_rmi_SimReturn__remoteConnect(const char* url, sidl_BaseInterface *_ex);
static struct sidlx_rmi_SimReturn__object* 
...
{
  return sidlx_rmi_SimReturn__remoteConnect(url, _ex);
}
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("sidlx.rmi.SimReturn",
      (void*)sidlx_rmi_SimReturn__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
const char* name)
{
  ...
  if (!strcmp(name, "sidlx.rmi.SimReturn")) {
    cast = (void*) s0;
  ...
  else if(sidlx_rmi_SimReturn_isType(self, name)) {
...
remote_sidlx_rmi_SimReturn_addRef(
  /* in */ struct sidlx_rmi_SimReturn__object* self /* TLD */)
{
...
remote_sidlx_rmi_SimReturn_queryInt(
  /* in */ struct sidlx_rmi_SimReturn__object* self /* TLD */,
  /* in */ const char* name)
{
  /* FIXME  need to think through all of these special cases */
...
static void sidlx_rmi_SimReturn__init_remote_epv(void)
{
  ...
  epv->f_queryInt           = remote_sidlx_rmi_SimReturn_queryInt;
...
static struct sidlx_rmi_SimReturn__object*
sidlx_rmi_SimReturn__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct sidlx_rmi_SimReturn__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "sidlx.rmi.SimReturn", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct sidlx_rmi_SimReturn__object*) malloc(
      sizeof(struct sidlx_rmi_SimReturn__object));
