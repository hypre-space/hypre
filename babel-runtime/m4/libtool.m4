...
_LT_AC_SHELL_INIT([tagnames=`echo "$tagnames,FC" | sed 's/^,//'`])
])# _LT_AC_LANG_FC
...
case $host_os in
  ...
  aix4* | aix5*)
    if test "$host_cpu" = ia64; then
      ...
      if ! echo $LDFLAGS | grep "Wl,-brtl" >/dev/null ; then
...
EOF
],[$1],[FC],[cat > conftest.$ac_ext <<EOF
      subroutine foo
      implicit none
      integer(selected_int_kind(9)) :: a
      a=0
      return
      end subroutine foo
EOF
  ...
  for p in `eval $output_verbose_link_cmd`; do
    ...
    *.$objext)
	 ...
	 continue
       ...
       if test "CXX" = "$1"; then
         case $CC in
	 *icpc|*icc) # Intel C++ compiler
	   case $p in
	   *crti.$objext | *crtn.$objext | *crtxi.$objext | *crtxn.$objext) # Ignore these
	     continue
	     ;;
	   esac
	 esac
