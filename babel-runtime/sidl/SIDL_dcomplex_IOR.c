...
SIDL_dcomplex__array_sdestroy(struct SIDL_dcomplex__array* array)
{
  if (array) {
    ...
    if (*orig) {
      SIDL_dcomplex__array_deleteReference(*orig);
      *orig = NULL;
    }
    SIDL_dcomplex__array_bdestroy(array);
  }
...
{
  SIDL_dcomplex__array_addReference(array);
  return array;
}
         ...
         struct SIDL_dcomplex__array *orig)
{
  ...
  if (orig) {
    ...
    *ref = orig;
    SIDL_dcomplex__array_addReference(orig);
  ...
  if (src && (src->d_dimen == dimen)) {
    ...
    case SIDL_column_major_order:
      if (SIDL_dcomplex__array_isColumnOrder(src)) {
        SIDL_dcomplex__array_addReference(src);
        result = src;
      }
      else {
