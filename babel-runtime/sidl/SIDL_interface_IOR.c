...
#define COPY_VALUE(x) ((x) ? (SIDL_BaseInterface_addReference((x)), (x)) : (x))
#define DESTROY_VALUE(x) if (x) SIDL_BaseInterface_deleteReference((x))
...
SIDL_interface__array_sdestroy(struct SIDL_interface__array* array)
{
  if (array) {
    ...
    if (*orig) {
      SIDL_interface__array_deleteReference(*orig);
      *orig = NULL;
    }
    SIDL_interface__array_bdestroy(array);
  }
...
{
  SIDL_interface__array_addReference(array);
  return array;
}
         ...
         struct SIDL_interface__array *orig)
{
  ...
  if (orig) {
    ...
    *ref = orig;
    SIDL_interface__array_addReference(orig);
  ...
  if (src && (src->d_dimen == dimen)) {
    ...
    case SIDL_column_major_order:
      if (SIDL_interface__array_isColumnOrder(src)) {
        SIDL_interface__array_addReference(src);
        result = src;
      }
      else {
