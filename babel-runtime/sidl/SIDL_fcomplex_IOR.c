...
SIDL_fcomplex__array_sdestroy(struct SIDL_fcomplex__array* array)
{
  if (array) {
    ...
    if (*orig) {
      SIDL_fcomplex__array_deleteReference(*orig);
      *orig = NULL;
    }
    SIDL_fcomplex__array_bdestroy(array);
  }
...
{
  SIDL_fcomplex__array_addReference(array);
  return array;
}
...
SIDL_fcomplex__array_borrowSmartCp(struct SIDL_fcomplex__array* array)
{
  ...
  if (array) {
    if (SIDL_fcomplex__array_isColumnOrder(array)) {
      copy = SIDL_fcomplex__array_createCol(array->d_dimen,
                                                array->d_lower,
                                                array->d_upper);
    }
    else {
      copy = SIDL_fcomplex__array_createRow(array->d_dimen,
                                                array->d_lower,
                                                array->d_upper);
    }
...
static const struct SIDL_fcomplex__vtable normal_fcomplex_vtable = {
  SIDL_fcomplex__array_destroy,
...
static const struct SIDL_fcomplex__vtable borrowed_fcomplex_vtable = {
  SIDL_fcomplex__array_bdestroy,
...
static const struct SIDL_fcomplex__vtable sliced_fcomplex_vtable = {
  SIDL_fcomplex__array_sdestroy,
  SIDL_fcomplex__array_smartCp
};
/**
...
static const struct SIDL_fcomplex__vtable bsliced_fcomplex_vtable = {
  SIDL_fcomplex__array_sdestroy,
  SIDL_fcomplex__array_borrowSmartCp
};
/**
         ...
         struct SIDL_fcomplex__array *orig)
{
  ...
  if (orig) {
    ...
    *ref = orig;
    SIDL_fcomplex__array_addReference(orig);
    result->d_vtable = &sliced_fcomplex_vtable;
  }
  else {
    result->d_vtable = &normal_fcomplex_vtable;
  }
  ...
  if (goodSliceArgs(src, dimen, srcStart, srcStride, newStart, numElem)) {
    ...
    if (result) {
      ...
      if (src->d_vtable->d_smartcopy ==
        ...
        result->d_vtable = &bsliced_fcomplex_vtable;
      }
    }
  }
  return result;
                            ...
                            const int32_t  stride[])
{
  ...
  result->d_vtable = &borrowed_fcomplex_vtable;
  return result;
}
  ...
  if (src && (src->d_dimen == dimen)) {
    ...
    case SIDL_column_major_order:
      if (SIDL_fcomplex__array_isColumnOrder(src)) {
        SIDL_fcomplex__array_addReference(src);
        result = src;
      }
      else {
