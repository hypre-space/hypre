...
function ExecuteScripts
{
  if [ "$DebugMode" -gt 0 ] ; then
    echo "In function ExecuteScripts"
  fi
  StartDir=$1
  WorkingDir=$2
  InputFile=$3
...
function ExecuteJobs
{
  ...
  if [ "$DebugMode" -gt 0 ] ; then
    set -xv
    echo "In function ExecuteJobs: WorkingDir=$WorkingDir InputFile=$InputFile"
  fi
  ...
  do
    ...
    case $InputLine in
      ...
      *mpirun*)
        if [ "$DebugMode" -gt 0 ] ; then
          echo "mpirun line"
          echo "BatchMode:$BatchMode BatchFlag:$BatchFlag"
        fi
	...
	ErrFile=`echo $OutFile | sed -e 's/\.out\./.err./'`
        ...
        if [ "$BatchMode" -eq 0 ] ; then
          if [ "$DebugMode" -gt 0 ] ; then
            echo "${RunString} > $OutFile 2> $ErrFile"
          fi
          ...
          if [ "$BatchFlag" -eq 0 ] ; then
            ...
            if [ "$DebugMode" -gt 0 ] ; then
              echo "RunName=$RunName BatchFile=$BatchFile"
            fi
            cat > $BatchFile <<- EOF 
		#!/bin/sh
		cd `pwd`
...
function ExecuteTest
{
  ...
  cd $WorkingDir
  if [ "$DebugMode" -gt 0 ] ; then
    echo "./$InputFile.sh  $InputFile.err "
  fi
    ...
    *) InputString=$1
      if [ "$InputString" ] ; then
        if [ -r $InputString ] ; then
          ...
          if [ "$DebugMode" -gt 0 ] ; then
            printf "FilePart:%s DirPart:%s CurDir:%s\n" \
              $FilePart $DirPart $CurDir
          fi
          if [ -r $DirPart/$FilePart.jobs ] ; then
            StartCrunch $CurDir $DirPart $FilePart # strict serial execution
          else
