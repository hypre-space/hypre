...
#include <assert.h> /* EO 5 May 2004 */
  ...
  */
  ...
  if ( lobpcgFlag ) {
	...
	if (myid == 0)
      ...
      HYPRE_LOBPCGSetupT(lobpcg_solver, (HYPRE_Matrix)parcsr_B, 
			 (HYPRE_Vector)x);
	...
	if ( lobpcgSeed )
      ...
      HYPRE_LOBPCGSolve(lobpcg_solver, constrains, eigenvectors, eigenvalues );
	...
	lobpcg_MultiVectorByMultiVector( NULL, eigenvectors,
					 NULL, eigenvectors, gramXX );
	...
	if ( lobpcgSeed )
      ...
      HYPRE_LOBPCGSolve(pcg_solver, constrains, eigenvectors, eigenvalues);
	...
	if ( myid == 0 ) {
      ...
      HYPRE_LOBPCGSetup(pcg_solver, (HYPRE_Matrix)parcsr_A, 
			(HYPRE_Vector)b, (HYPRE_Vector)x);
      HYPRE_LOBPCGSolve(pcg_solver, eigenvectors, eigenvalues );
#endif
