...
* Revision 1.4 by Andrew Knyazev 03/11/2004 
 *--------------------------------------------------------------------------*/
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
      ...
      char *argv[] )
{
   ...
   /* lobpcg */
   /* these are made global variables above
...
*/
   ...
   if (lobpcg_flag==TRUE)
   {
    solver_id = 1; /* set lobpcg default to AMG-PCG */
    opts.pcg_tol =  1.e-2;  /* set inner pcg tol default */
    opts.pcg_max_itr = 1;   /* set inner pcg max iterations default */
  ...
  /* end lobpcg */
           ...
           solver_id == 12 || solver_id == 43))
   {
         if (myid==0)
         {
            printf("\nSolver not supported by lobpcg (solver_id=%d)\n",solver_id);
            printf("Tested solvers for PCG/LOBPCG are: 1=AMG-PCG (default),2=DS-PCG,8=ParaSails-PCG\n");
            printf("12=Schwarz-PCG,43=Euclid-PCG\n");
         }
         /* exit(1); */
   }   
      ...
      if (lobpcg_flag==TRUE || lobpcg_flag==FALSE)
       {
         ...
         printf("  -vrand <int>     : random <int> initial eigenvectors\n");
         printf("  -veye  <int>     : initial eigenvectors are <int> first coordinate vectors (default 1 vector)\n");
         printf("  -chk   <int>     : check orthogonality of final eigenvectors (default 0)\n");
         printf("  -itr  <int>      : max lobpcg iteration count (default 500)\n");
         printf("  -tol  <scalar>   : lobpcg tolerance (default 1e-6).\n"); 
	 printf("                   : Note: Smaller tolerance may lead to instability. \n");
         printf("  -v    <int>      : 0 (no output), 1 (standard output, default), 2 (detailed output)\n");
         printf("                   : Note: detailed output (v=2) includes execution statistics\n");
         printf("  -pcgitr <int>    : maximum inner iterations for pcg solve (default=1)\n");
         printf("                   : Note: set this to 0 for no precondioner\n");
         printf("  -pcgtol <scalar> : tol for inner pcg solve (default=1e-2)\n");
         printf("  -printA          : print out matrix A\n");
          printf("Solvers supported for lobpcg:\n");
         printf("  -solver <ID>\n");
         printf("     1=AMG-PCG (default)\n");
         printf("     2=DS-PCG\n");
         printf("     8=ParaSails-PCG\n");
         printf("     12=Schwarz-PCG\n");  
         printf("     43=Euclid-PCG\n");
         printf("\n");
         printf("Examples of ij_es lobpcg options:\n");
         printf("  ij_es -lobpcg -ain laplacian_10_10_10.mtx -vrand 5  -solver 1 -pcgitr 10\n");
         printf("  ij_es -lobpcg -9pt -solver 43 -n 20 20\n");
         printf("\n");
         printf("\n");
      }
      printf("Standard ij driver options (some may be incompatible with lobpcg):\n");
      /*  end lobpcg */
...
/*  build IJMatrix using exact row_sizes for diag and offdiag */
   ...
   if (lobpcg_flag==TRUE)
   {
      ...
      {
          /* default is one coordinate vector */
          bsize=1;
          opts.Veye=1;
      }
      if (bsize>0)
