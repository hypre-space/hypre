...
int MLI_SolverGS::setup(MLI_Matrix *mat)
{
   Amat = mat;
   return 0;
}
...
int MLI_SolverGS::setParams(char *param_string, int argc, char **argv )
{
   int    i;
   double *weights;
   char   param1[200];
   ...
   else if ( !strcmp(param_string, "relaxWeight") )
   {
      if ( argc != 2 && argc != 1 ) 
      {
         cout << "SolverGS::setParams ERROR : needs 1 or 2 args.\n";
         return 1;
      }
   ...
   {   
      cout << "MLI_SolverGS::setParams - parameter not recognized.\n";
...
int MLI_SolverGS::solve(MLI_Vector *f_in, MLI_Vector *u_in)
{
...
int MLI_SolverGS::setParams( int ntimes, double *weights )
{
   if ( ntimes <= 0 )
   {
      cerr << "MLI_SolverGS::setParams WARNING : nsweeps set to 1." << endl;
      ntimes = 1;
   }
   nsweeps = ntimes;
   if ( relax_weights != NULL ) delete [] relax_weights;
   ...
   if ( weights == NULL )
   {
      cout << "MLI_SolverGS::setParams - relax_weights set to 0.5." << endl;
   ...
   else
   {
      for ( int j = 0; j < ntimes; j++ ) 
      {
         ...
         {
            printf("MLI_SolverGS::setParams - some weights set to 0.5.\n");
            relax_weights[j] = 0.5;
         }
      }
   }
