...
MLI_SolverJacobi::MLI_SolverJacobi()
{
   Amat             = NULL;
   nsweeps          = 1;
...
int MLI_SolverJacobi::setup(MLI_Matrix *mat)
{
   Amat = mat;
   return 0;
}
...
int MLI_SolverJacobi::solve(MLI_Vector *f_in, MLI_Vector *u_in)
{
...
int MLI_SolverJacobi::setParams( char *param_string, int argc, char **argv )
{
   int    i;
   double *weights;
   char   param1[200];
   ...
   else if ( !strcmp(param_string, "relaxWeight") )
   {
      if ( argc != 2 && argc != 1 ) 
      {
         cout << "SolverJacobi::setParams ERROR : needs 1 or 2 args.\n";
         return 1;
      }
   ...
   {   
      cout << "SolverJacobi::setParams - parameter not recognized.\n";
...
int MLI_SolverJacobi::setParams( int ntimes, double *weights )
{
   if ( ntimes <= 0 )
   {
      cerr << "MLI_SolverJacobi::setParams WARNING : nsweeps set to 1." << endl;
      ntimes = 1;
   }
   nsweeps = ntimes;
   if ( relax_weights != NULL ) delete [] relax_weights;
   ...
   if ( weights == NULL )
   {
      cout << "MLI_SolverJacobi::setParams - relax_weights set to 0.5." << endl;
   ...
   else
   {
      for ( int j = 0; j < ntimes; j++ ) 
      {
         ...
         {
            cout << "MLI_SolverJacobi::setParams - weights set to 0.5" << endl;
            relax_weights[j] = 0.5;
         }
      }
   }
