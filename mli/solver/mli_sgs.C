...
int MLI_SolverSGS::setup(MLI_Matrix *mat)
{
   Amat = mat;
   return 0;
}
...
int MLI_SolverSGS::solve(MLI_Vector *f_in, MLI_Vector *u_in)
{
...
int MLI_SolverSGS::setParams( char *param_string, int argc, char **argv )
{
   int    i;
   double *weights;
   char   param1[200];
   ...
   else if ( !strcmp(param_string, "relaxWeight") )
   {
      if ( argc != 2 && argc != 1 ) 
      {
         cout << "SolverSGS::setParams ERROR : needs 1 or 2 args.\n";
         return 1;
      }
   ...
   {   
      cout << "MLI_SolverSGS::setParams - parameter not recognized.\n";
...
int MLI_SolverSGS::setParams( int ntimes, double *weights )
{
   if ( ntimes <= 0 )
   {
      cerr << "MLI_SolverSGS::setParams WARNING : nsweeps set to 1." << endl;
      ntimes = 1;
   }
   nsweeps = ntimes;
   if ( relax_weights != NULL ) delete [] relax_weights;
   ...
   if ( weights == NULL )
   {
      cout << "MLI_SolverSGS::setParams - relax_weights set to 0.5." << endl;
   ...
   else
   {
      for ( int j = 0; j < ntimes; j++ ) 
      {
         ...
         {
            printf("MLI_SolverSGS::setParams - some weights set to 0.5.\n");
            relax_weights[j] = 0.5;
         }
      }
   }
