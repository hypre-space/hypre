...

{
  ...
  if (!_ior) {
    fputs("Babel: unable to load the implementation for bHYPRE.SStructParCSRMatrix; please set SIDL_DLL_PATH\n", stderr);
    exit(-1);
  }
#endif
  return _ior;
...
int32_t
bHYPRE_SStructParCSRMatrix_SetIntArrayParameter(
  bHYPRE_SStructParCSRMatrix self,
  const char* name,
...
int32_t
bHYPRE_SStructParCSRMatrix_SetDoubleArrayParameter(
  bHYPRE_SStructParCSRMatrix self,
  const char* name,
