...
#else
  _externals = (struct bHYPRE_GMRES__external*)sidl_dynamicLoadIOR(
    "bHYPRE.GMRES","bHYPRE_GMRES__externals") ;
  sidl_checkIORVersion("bHYPRE.GMRES", _externals->d_ior_major_version, 
...
static bHYPRE_GMRES bHYPRE_GMRES__remoteCreate(const char* url, 
...
{
  return bHYPRE_GMRES__remoteCreate(url, _ex);
}

static struct bHYPRE_GMRES__object* bHYPRE_GMRES__remoteConnect(const char* url,
  sidl_bool ar, sidl_BaseInterface *_ex);
static struct bHYPRE_GMRES__object* bHYPRE_GMRES__IHConnect(struct 
...
{
  return bHYPRE_GMRES__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.GMRES", (
      void*)bHYPRE_GMRES__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_GMRES__set_hooks_static(
  int32_t on,
...
bHYPRE_GMRES__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_GMRES__isRemote(self,_ex);
}
