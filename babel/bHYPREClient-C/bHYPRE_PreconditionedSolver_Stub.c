...
static struct bHYPRE_PreconditionedSolver__object* 
  bHYPRE_PreconditionedSolver__remoteConnect(const char* url,
...
{
  return bHYPRE_PreconditionedSolver__remoteConnect(url, _ex);
}
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.PreconditionedSolver",
      (void*)bHYPRE_PreconditionedSolver__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
const char* name)
{
  ...
  if (!strcmp(name, "bHYPRE._PreconditionedSolver")) {
    cast = (void*) s0;
...
remote_bHYPRE__PreconditionedSolver_queryInt(
  /* in */ struct bHYPRE__PreconditionedSolver__object* self /* TLD */,
  /* in */ const char* name)
{
  /* FIXME  need to think through all of these special cases */
  ...
  sidl_BaseInterface *_ex)
{
  ...
  return bHYPRE_PreconditionedSolver__cast(self);
}
