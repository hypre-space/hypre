...
static bHYPRE_ParaSails bHYPRE_ParaSails__remote(const char* url,
...
{
  return bHYPRE_ParaSails__remote(url, _ex);
}
...
{
  return bHYPRE_ParaSails__remoteConnect(url, _ex);
}
...
void
bHYPRE_ParaSails_Create__sexec(
        struct sidl_io_Deserializer__object* inArgs,
        struct sidl_io_Serializer__object* outArgs) {
  /* stack space for arguments */
  bHYPRE_MPICommunicator mpi_comm;
  bHYPRE_ParaSails _retval;
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.ParaSails",
      (void*)bHYPRE_ParaSails__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
struct bHYPRE_ParaSails__smethod {
  const char *d_name;
...
bHYPRE_ParaSails__sexec(
        ...
        struct sidl_io_Deserializer__object* inArgs,
        struct sidl_io_Serializer__object* outArgs ) { 
  static const struct bHYPRE_ParaSails__smethod s_methods[] = {
    { "Create", bHYPRE_ParaSails_Create__sexec }
  };
  int i, cmp, l = 0;
  int u = sizeof(s_methods)/sizeof(struct bHYPRE_ParaSails__smethod);
  if (methodName) {
...
const char* name)
{
  ...
  else if(bHYPRE_ParaSails_isType(self, name)) {
...
remote_bHYPRE_ParaSails_addRef(
  /* in */ struct bHYPRE_ParaSails__object* self /* TLD */)
{
...
static struct bHYPRE_ParaSails__object*
bHYPRE_ParaSails__remoteConnect(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_ParaSails__object* self;
...
static struct bHYPRE_ParaSails__object*
bHYPRE_ParaSails__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_ParaSails__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "bHYPRE.ParaSails", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_ParaSails__object*) malloc(
      sizeof(struct bHYPRE_ParaSails__object));
