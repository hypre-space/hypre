...

{
  ...
  sidl_checkIORVersion("bHYPRE.HPCG", _externals->d_ior_major_version, 
...
static bHYPRE_HPCG bHYPRE_HPCG__remoteCreate(const char* url, 
...
{
  return bHYPRE_HPCG__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_HPCG__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.HPCG", (
      void*)bHYPRE_HPCG__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
bHYPRE_HPCG__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_HPCG__isRemote(self,_ex);
}
