...
bHYPRE_StructJacobi
bHYPRE_StructJacobi__create()
{
  return (*(_getExternals()->createObject))();
}

static bHYPRE_StructJacobi bHYPRE_StructJacobi__remote(const char* url,
...
{
  return bHYPRE_StructJacobi__remote(url, _ex);
}

static struct bHYPRE_StructJacobi__object* 
  bHYPRE_StructJacobi__remoteConnect(const char* url, sidl_BaseInterface *_ex);
static struct bHYPRE_StructJacobi__object* 
...
{
  return bHYPRE_StructJacobi__remoteConnect(url, _ex);
}
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.StructJacobi",
      (void*)bHYPRE_StructJacobi__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
struct bHYPRE_StructJacobi__smethod {
  const char *d_name;
...
bHYPRE_StructJacobi__sexec(
        ...
        struct sidl_io_Deserializer__object* inArgs,
        struct sidl_io_Serializer__object* outArgs ) { 
  static const struct bHYPRE_StructJacobi__smethod s_methods[] = {
    { "Create", bHYPRE_StructJacobi_Create__sexec }
  };
  int i, cmp, l = 0;
  int u = sizeof(s_methods)/sizeof(struct bHYPRE_StructJacobi__smethod);
  if (methodName) {
...
const char* name)
{
  ...
  else if(bHYPRE_StructJacobi_isType(self, name)) {
...
remote_bHYPRE_StructJacobi_addRef(
  /* in */ struct bHYPRE_StructJacobi__object* self /* TLD */)
{
...
remote_bHYPRE_StructJacobi_queryInt(
  /* in */ struct bHYPRE_StructJacobi__object* self /* TLD */,
  /* in */ const char* name)
{
  /* FIXME  need to think through all of these special cases */
...
static void bHYPRE_StructJacobi__init_remote_epv(void)
{
  ...
  epv->f_queryInt                      = remote_bHYPRE_StructJacobi_queryInt;
...
static struct bHYPRE_StructJacobi__object*
bHYPRE_StructJacobi__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_StructJacobi__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "bHYPRE.StructJacobi", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_StructJacobi__object*) malloc(
      sizeof(struct bHYPRE_StructJacobi__object));
