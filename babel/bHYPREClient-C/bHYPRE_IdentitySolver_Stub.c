...
bHYPRE_IdentitySolver__create()
{
  return (*(_getExternals()->createObject))();
}

static bHYPRE_IdentitySolver bHYPRE_IdentitySolver__remote(const char* url,
...
{
  return bHYPRE_IdentitySolver__remote(url, _ex);
}

static struct bHYPRE_IdentitySolver__object* 
  bHYPRE_IdentitySolver__remoteConnect(const char* url,
  sidl_BaseInterface *_ex);
static struct bHYPRE_IdentitySolver__object* 
...
{
  return bHYPRE_IdentitySolver__remoteConnect(url, _ex);
}
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.IdentitySolver",
      (void*)bHYPRE_IdentitySolver__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
const char* name)
{
  ...
  if (!strcmp(name, "bHYPRE.IdentitySolver")) {
    cast = (void*) s0;
  ...
  else if(bHYPRE_IdentitySolver_isType(self, name)) {
...
remote_bHYPRE_IdentitySolver_queryInt(
  /* in */ struct bHYPRE_IdentitySolver__object* self /* TLD */,
  /* in */ const char* name)
{
  /* FIXME  need to think through all of these special cases */
...
static void bHYPRE_IdentitySolver__init_remote_epv(void)
{
  ...
  epv->f_queryInt                      = remote_bHYPRE_IdentitySolver_queryInt;
...
static struct bHYPRE_IdentitySolver__object*
bHYPRE_IdentitySolver__remoteConnect(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_IdentitySolver__object* self;
...
static struct bHYPRE_IdentitySolver__object*
bHYPRE_IdentitySolver__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_IdentitySolver__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "bHYPRE.IdentitySolver", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_IdentitySolver__object*) malloc(
      sizeof(struct bHYPRE_IdentitySolver__object));
