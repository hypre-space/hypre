...

{
  ...
  sidl_checkIORVersion("bHYPRE.PCG", _externals->d_ior_major_version,
...
static bHYPRE_PCG bHYPRE_PCG__remoteCreate(const char* url,
...
{
  return bHYPRE_PCG__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_PCG__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.PCG",
      (void*)bHYPRE_PCG__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
bHYPRE_PCG__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_PCG__isRemote(self,_ex);
}
