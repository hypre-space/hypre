...

{
  ...
  sidl_checkIORVersion("bHYPRE.CGNR", _externals->d_ior_major_version, 
...
static bHYPRE_CGNR bHYPRE_CGNR__remoteCreate(const char* url, 
...
{
  return bHYPRE_CGNR__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_CGNR__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.CGNR", (
      void*)bHYPRE_CGNR__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
bHYPRE_CGNR__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_CGNR__isRemote(self,_ex);
}
