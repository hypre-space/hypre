...
static const struct bHYPRE_SStructVector__external* _getIOR(void)
{
  ...
  if (!_ior) {
    ...
    if (dll && sidl_DLL_loadLibrary(dll, "main:", TRUE, FALSE)) {
      dll_f =
        (const struct bHYPRE_SStructVector__external*(*)(void)) 
          ...
          dll, "bHYPRE_SStructVector__externals");
      _ior = (dll_f ? (*dll_f)() : NULL);
    }
    if (dll) sidl_DLL_deleteRef(dll);
    if (!_ior) {
      dll = sidl_Loader_findLibrary("bHYPRE.SStructVector",
    ...
    if (!_ior) {
      fputs("Unable to find the implementation for bHYPRE.SStructVector; please set SIDL_DLL_PATH\n", stderr);
      exit(-1);
    }
#endif
  }
