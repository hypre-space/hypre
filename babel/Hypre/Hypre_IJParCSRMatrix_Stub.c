/*
 * File:          Hypre_IJParCSRMatrix_Stub.c
...
static const struct Hypre_IJParCSRMatrix__external* _loadIOR(void)
/*
...

{
  ...
  if (!_ior) {
    fputs("Babel: unable to load the implementation for Hypre.IJParCSRMatrix; please set SIDL_DLL_PATH\n", stderr);
    exit(-1);
  }
#endif
  return _ior;
...
Hypre_IJParCSRMatrix__cast(
  void* obj)
{
  Hypre_IJParCSRMatrix cast = NULL;

  if (obj != NULL) {
    SIDL_BaseInterface base = (SIDL_BaseInterface) obj;
    cast = (Hypre_IJParCSRMatrix) (*base->d_epv->f__cast)(
      base->d_object,
      "Hypre.IJParCSRMatrix");
  }
...
{
  return (struct 
    Hypre_IJParCSRMatrix__array*)SIDL_interface__array_createCol(dimen, lower,
    upper);
}
...
{
  return (struct 
    Hypre_IJParCSRMatrix__array*)SIDL_interface__array_createRow(dimen, lower,
    upper);
}
...
{
  return (struct 
    Hypre_IJParCSRMatrix__array*)SIDL_interface__array_create1d(len);
}
...
{
  return (struct 
    Hypre_IJParCSRMatrix__array*)SIDL_interface__array_create2dCol(m, n);
}
...
{
  return (struct 
    Hypre_IJParCSRMatrix__array*)SIDL_interface__array_create2dRow(m, n);
}
...
{
  return (struct Hypre_IJParCSRMatrix__array*)SIDL_interface__array_borrow(
 ...
 */
struct Hypre_IJParCSRMatrix__array*
Hypre_IJParCSRMatrix__array_smartCopy(struct Hypre_IJParCSRMatrix__array *array)
{
  return (struct Hypre_IJParCSRMatrix__array*)
 ...
 */
void
Hypre_IJParCSRMatrix__array_deleteRef(struct Hypre_IJParCSRMatrix__array* array)
{
...
Hypre_IJParCSRMatrix__array_copy(const struct Hypre_IJParCSRMatrix__array* src,
                                       struct Hypre_IJParCSRMatrix__array* dest)
{
