...
static const struct Hypre_StructToIJMatrix__external* _getIOR(void)
{
  ...
  if (!_ior) {
    ...
    if (!_ior) {
      fputs("Unable to find the implementation for Hypre.StructToIJMatrix; please set SIDL_DLL_PATH\n", stderr);
      exit(-1);
    }
#endif
  }
