...
struct Hypre_Operator__array {
  struct Hypre_Operator__object** d_firstElement;
  int32_t                         *d_lower;
  int32_t                         *d_upper;
  int32_t                         *d_stride;
...
static struct Hypre_Operator__array*
newArray(int32_t dimen, const int32_t lower[], const int32_t upper[]) {
  static const size_t arraySize = sizeof(struct Hypre_Operator__array)
    + (sizeof(int32_t) - (sizeof(struct Hypre_Operator__array)
    % sizeof(int32_t))) % sizeof(int32_t);
  struct Hypre_Operator__array *result =
    (struct Hypre_Operator__array *)
...
{
  int32_t size=1, i;
  struct Hypre_Operator__array *result = newArray(dimen, lower, upper);
  for(i = 0; i < dimen; ++i) {
    result->d_stride[i] = size;
    size *= (1 + upper[i] - lower[i]);
  }
  size *= sizeof(struct Hypre_Operator__object*);
  result->d_firstElement = (struct Hypre_Operator__object**)
    malloc(size);
  ...
  struct Hypre_Operator__array* array)
{
  if (array) {
    ...
    if (!(array->d_borrowed)) {
      if (dimen > 0) {
        int32_t size = 1;
        struct Hypre_Operator__object** start = array->d_firstElement;
        struct Hypre_Operator__object** end;
        if (dimen > 1) {
    ...
    memset(array, 0, sizeof(struct Hypre_Operator__array)
...
{
  struct Hypre_Operator__object** result = NULL;
...
static const struct Hypre_Operator__external
s_externalEntryPoints = {
  Hypre_Operator__iorarray_create,
  Hypre_Operator__iorarray_borrow,
  Hypre_Operator__iorarray_destroy,
  Hypre_Operator__iorarray_dimen,
  Hypre_Operator__iorarray_lower,
  Hypre_Operator__iorarray_upper,
  Hypre_Operator__iorarray_get,
  Hypre_Operator__iorarray_get4,
  Hypre_Operator__iorarray_set,
  Hypre_Operator__iorarray_set4
...
static int32_t
remote_Hypre_Operator_SetParameter(
  void* self,
  const char* name,
  double value)
{
...
static void Hypre_Operator__init_remote_epv(void)
{
  ...
  epv->f_SetParameter    = remote_Hypre_Operator_SetParameter;
