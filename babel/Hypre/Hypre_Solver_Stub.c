...
static const struct Hypre_Solver__external* _getIOR(void)
{
  static const struct Hypre_Solver__external *_ior = NULL;
  if (!_ior) {
#ifdef SIDL_STATIC_LIBRARY
    _ior = Hypre_Solver__externals();
#else
    const struct Hypre_Solver__external*(*dll_f)(void) =
      (const struct Hypre_Solver__external*(*)(void)) SIDL_Loader_lookupSymbol(
        "Hypre_Solver__externals");
    _ior = (dll_f ? (*dll_f)() : NULL);
    if (!_ior) {
      fputs("Unable to find the implementation for Hypre.Solver; please set SIDL_DLL_PATH\n", stderr);
      exit(-1);
    }
#endif
  }
