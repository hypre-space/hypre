/*
 * File:          Hypre_SStructParCSRVector_Stub.c
...
static const struct Hypre_SStructParCSRVector__external *_ior = NULL;
/*
 * Lookup the symbol to get the IOR functions.
 */

static const struct Hypre_SStructParCSRVector__external* _loadIOR(void)
/*
...

{
  ...
  _ior = Hypre_SStructParCSRVector__externals();
#else
  const struct Hypre_SStructParCSRVector__external*(*dll_f)(void) =
    (const struct Hypre_SStructParCSRVector__external*(*)(void)) 
      SIDL_Loader_lookupSymbol(
      "Hypre_SStructParCSRVector__externals");
  _ior = (dll_f ? (*dll_f)() : NULL);
  if (!_ior) {
    fputs("Babel: unable to load the implementation for Hypre.SStructParCSRVector; please set SIDL_DLL_PATH\n", stderr);
    exit(-1);
  }
#endif
  return _ior;
...
Hypre_SStructParCSRVector__create()
{
...
Hypre_SStructParCSRVector__cast(
  void* obj)
{
  Hypre_SStructParCSRVector cast = NULL;

  if (obj != NULL) {
    SIDL_BaseInterface base = (SIDL_BaseInterface) obj;
    cast = (Hypre_SStructParCSRVector) (*base->d_epv->f__cast)(
      base->d_object,
      "Hypre.SStructParCSRVector");
  }
...
{
  return (struct 
    Hypre_SStructParCSRVector__array*)SIDL_interface__array_createCol(dimen,
    lower, upper);
}
...
{
  return (struct 
    Hypre_SStructParCSRVector__array*)SIDL_interface__array_createRow(dimen,
    lower, upper);
}
...
{
  return (struct 
    Hypre_SStructParCSRVector__array*)SIDL_interface__array_create1d(len);
}
...
{
  return (struct 
    Hypre_SStructParCSRVector__array*)SIDL_interface__array_create2dCol(m, n);
}
...
{
  return (struct 
    Hypre_SStructParCSRVector__array*)SIDL_interface__array_create2dRow(m, n);
}
...
{
  return (struct Hypre_SStructParCSRVector__array*)SIDL_interface__array_borrow(
...
{
  return (struct Hypre_SStructParCSRVector__array*)
...
Hypre_SStructParCSRVector__array_get1(const struct 
  Hypre_SStructParCSRVector__array* array,
                                      const int32_t i1)
{
  return (Hypre_SStructParCSRVector)
