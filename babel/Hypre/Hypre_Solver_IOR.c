...
struct Hypre_Solver__array {
  struct Hypre_Solver__object** d_firstElement;
  int32_t                       *d_lower;
  int32_t                       *d_upper;
  int32_t                       *d_stride;
...
static struct Hypre_Solver__array*
newArray(int32_t dimen, const int32_t lower[], const int32_t upper[]) {
  static const size_t arraySize = sizeof(struct Hypre_Solver__array)
    + (sizeof(int32_t) - (sizeof(struct Hypre_Solver__array)
    % sizeof(int32_t))) % sizeof(int32_t);
  struct Hypre_Solver__array *result =
    (struct Hypre_Solver__array *)
...
{
  int32_t size=1, i;
  struct Hypre_Solver__array *result = newArray(dimen, lower, upper);
  for(i = 0; i < dimen; ++i) {
    result->d_stride[i] = size;
    size *= (1 + upper[i] - lower[i]);
  }
  size *= sizeof(struct Hypre_Solver__object*);
  result->d_firstElement = (struct Hypre_Solver__object**)
    malloc(size);
  ...
  struct Hypre_Solver__array* array)
{
  if (array) {
    ...
    if (!(array->d_borrowed)) {
      if (dimen > 0) {
        int32_t size = 1;
        struct Hypre_Solver__object** start = array->d_firstElement;
        struct Hypre_Solver__object** end;
        if (dimen > 1) {
    ...
    memset(array, 0, sizeof(struct Hypre_Solver__array)
...
{
  struct Hypre_Solver__object** result = NULL;
...
static const struct Hypre_Solver__external
s_externalEntryPoints = {
  Hypre_Solver__iorarray_create,
  Hypre_Solver__iorarray_borrow,
  Hypre_Solver__iorarray_destroy,
  Hypre_Solver__iorarray_dimen,
  Hypre_Solver__iorarray_lower,
  Hypre_Solver__iorarray_upper,
  Hypre_Solver__iorarray_get,
  Hypre_Solver__iorarray_get4,
  Hypre_Solver__iorarray_set,
  Hypre_Solver__iorarray_set4
