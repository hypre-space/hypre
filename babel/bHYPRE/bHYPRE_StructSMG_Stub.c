...
static const struct bHYPRE_StructSMG__external *_externals = NULL;
/*
...

{
  ...
  _externals = bHYPRE_StructSMG__externals();
#else
  _externals = (struct 
    bHYPRE_StructSMG__external*)sidl_dynamicLoadIOR("bHYPRE.StructSMG",
    "bHYPRE_StructSMG__externals") ;
  sidl_checkIORVersion("bHYPRE.StructSMG", _externals->d_ior_major_version,
...
static bHYPRE_StructSMG bHYPRE_StructSMG__remoteCreate(const char* url,
...
{
  return bHYPRE_StructSMG__remoteCreate(url, _ex);
}
...
bHYPRE_StructSMG
bHYPRE_StructSMG__connect(const char* url, sidl_BaseInterface *_ex)
{
  return bHYPRE_StructSMG__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.StructSMG",
      (void*)bHYPRE_StructSMG__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_StructSMG__set_hooks_static(
  int32_t on,
...
bHYPRE_StructSMG__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_StructSMG__isRemote(self,_ex);
}
