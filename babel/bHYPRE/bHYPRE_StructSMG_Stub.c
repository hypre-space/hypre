...
static bHYPRE_StructSMG bHYPRE_StructSMG__remote(const char* url,
...
{
  return bHYPRE_StructSMG__remote(url, _ex);
}
...
{
  return bHYPRE_StructSMG__remoteConnect(url, _ex);
}
...
void
bHYPRE_StructSMG_Create__sexec(
  ...
  void* obj)
{
  ...
  if(!connect_loaded) {
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.StructSMG",
      (void*)bHYPRE_StructSMG__IHConnect);
    connect_loaded = 1;
  }
  if (obj != NULL) {
...
struct bHYPRE_StructSMG__smethod {
  const char *d_name;
...
bHYPRE_StructSMG__sexec(
        ...
        struct sidl_io_Deserializer__object* inArgs,
        struct sidl_io_Serializer__object* outArgs ) { 
  static const struct bHYPRE_StructSMG__smethod s_methods[] = {
    { "Create", bHYPRE_StructSMG_Create__sexec }
  };
  int i, cmp, l = 0;
  int u = sizeof(s_methods)/sizeof(struct bHYPRE_StructSMG__smethod);
  if (methodName) {
...
const char* name)
{
  ...
  else if(bHYPRE_StructSMG_isType(self, name)) {
...
static struct bHYPRE_StructSMG__object*
bHYPRE_StructSMG__remoteConnect(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_StructSMG__object* self;
...
static struct bHYPRE_StructSMG__object*
bHYPRE_StructSMG__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_StructSMG__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "bHYPRE.StructSMG", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_StructSMG__object*) malloc(
      sizeof(struct bHYPRE_StructSMG__object));
