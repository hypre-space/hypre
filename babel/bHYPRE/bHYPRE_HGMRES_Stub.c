...
#else
  _externals = (struct bHYPRE_HGMRES__external*)sidl_dynamicLoadIOR(
    "bHYPRE.HGMRES","bHYPRE_HGMRES__externals") ;
  sidl_checkIORVersion("bHYPRE.HGMRES", _externals->d_ior_major_version, 
...
static bHYPRE_HGMRES bHYPRE_HGMRES__remoteCreate(const char* url, 
...
{
  return bHYPRE_HGMRES__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_HGMRES__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.HGMRES", (
      void*)bHYPRE_HGMRES__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_HGMRES__set_hooks_static(
  int32_t on,
...
bHYPRE_HGMRES__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_HGMRES__isRemote(self,_ex);
}
...
bHYPRE_HGMRES__remoteConnect(const char *url, sidl_bool ar, sidl_BaseInterface 
  *_ex)
{
  struct bHYPRE_HGMRES__object* self;
