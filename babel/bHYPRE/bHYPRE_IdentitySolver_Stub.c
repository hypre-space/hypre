...
static const struct bHYPRE_IdentitySolver__external *_ior = NULL;
/*
...

{
  ...
  _ior = bHYPRE_IdentitySolver__externals();
#else
  sidl_DLL dll = sidl_DLL__create();
  const struct bHYPRE_IdentitySolver__external*(*dll_f)(void);
  /* check global namespace for symbol first */
  if (dll && sidl_DLL_loadLibrary(dll, "main:", TRUE, FALSE)) {
    dll_f =
      (const struct bHYPRE_IdentitySolver__external*(*)(void)) 
        sidl_DLL_lookupSymbol(
        dll, "bHYPRE_IdentitySolver__externals");
    _ior = (dll_f ? (*dll_f)() : NULL);
  }
  if (dll) sidl_DLL_deleteRef(dll);
  if (!_ior) {
    dll = sidl_Loader_findLibrary("bHYPRE.IdentitySolver",
  ...
  if (!_ior) {
    fputs("Babel: unable to load the implementation for bHYPRE.IdentitySolver; please set SIDL_DLL_PATH\n", stderr);
    exit(-1);
  }
#endif
  return _ior;
