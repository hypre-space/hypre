...
bHYPRE_Hybrid
bHYPRE_Hybrid__create()
{
  return (*(_getExternals()->createObject))();
}

static bHYPRE_Hybrid bHYPRE_Hybrid__remote(const char* url,
...
{
  return bHYPRE_Hybrid__remote(url, _ex);
}
...
{
  return bHYPRE_Hybrid__remoteConnect(url, _ex);
}
...
const char* name)
{
  ...
  else if(bHYPRE_Hybrid_isType(self, name)) {
...
static void bHYPRE_Hybrid__init_remote_epv(void)
{
  ...
  epv->f_queryInt                      = remote_bHYPRE_Hybrid_queryInt;
...
static struct bHYPRE_Hybrid__object*
bHYPRE_Hybrid__remoteConnect(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_Hybrid__object* self;
...
static struct bHYPRE_Hybrid__object*
bHYPRE_Hybrid__remote(const char *url, sidl_BaseInterface *_ex)
{
  struct bHYPRE_Hybrid__object* self;
  ...
  sidl_rmi_InstanceHandle instance = 
    sidl_rmi_ProtocolFactory_createInstance(url, "bHYPRE.Hybrid", _ex );
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_Hybrid__object*) malloc(
      sizeof(struct bHYPRE_Hybrid__object));
