...
{
  return bHYPRE_IJVectorView__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.IJVectorView",
      (void*)bHYPRE_IJVectorView__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
SIDL_C_INLINE_DEFN
void
bHYPRE_IJVectorView__set_hooks(
  /* in */ bHYPRE_IJVectorView self,
  /* in */ sidl_bool on,
  /* out */ sidl_BaseInterface *_ex)
#if SIDL_C_INLINE_REPEAT_DEFN
...
bHYPRE_IJVectorView__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_IJVectorView__isRemote(self,_ex);
}
