...

{
  ...
  sidl_checkIORVersion("bHYPRE.BiCGSTAB", _externals->d_ior_major_version,
...
static bHYPRE_BiCGSTAB bHYPRE_BiCGSTAB__remoteCreate(const char* url,
...
{
  return bHYPRE_BiCGSTAB__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_BiCGSTAB__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.BiCGSTAB",
      (void*)bHYPRE_BiCGSTAB__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_BiCGSTAB__set_hooks_static(
  int32_t on,
...
bHYPRE_BiCGSTAB__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_BiCGSTAB__isRemote(self,_ex);
}
