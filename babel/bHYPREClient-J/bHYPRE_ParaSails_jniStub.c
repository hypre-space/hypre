/*
 * File:          bHYPRE_ParaSails_jniStub.c
 * Symbol:        bHYPRE.ParaSails-v1.0.0
 * Symbol Type:   class
 * Babel Version: 1.0.4
 * Description:   Client-side JNI glue code for bHYPRE.ParaSails
 * 
...
#include "bHYPRE_ParaSails_jniStub.h"
#endif
...
static struct bHYPRE_ParaSails__object* _get_ior(
  JNIEnv* env,
  jobject obj)
{
  void* ptr = NULL;
  ...
  return (struct bHYPRE_ParaSails__object*) ptr;
}
...
static const struct bHYPRE_ParaSails__external* s_external = NULL;

/*
 * External reference to static EPV.
 */

static const struct bHYPRE_ParaSails__sepv* s_sepv = NULL;
  ...
  jstring url)
{
  ...
  _res = POINTER_TO_JLONG(bHYPRE_ParaSails__remoteCreate(_tmp_url, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject obj)
{
  ...
  struct bHYPRE_ParaSails__object *_ptr = (*s_external->createObject)(g_data, 
    &_ex);
  if(_ex) {
    sidl_Java_CheckException(
      env,
  ...
  jstring url)
{
  ...
  jlong _res = 0;
  char* _tmp_url = sidl_Java_J2I_string(env, url);
  _res = POINTER_TO_JLONG(bHYPRE_ParaSails__remoteConnect(_tmp_url, 1, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject _arg_A)
{
  ...
  struct bHYPRE_ParaSails__object* _ior_res = (struct 
    bHYPRE_ParaSails__object*) NULL;
  ...
  _res = sidl_Java_I2J_cls(env, _ior_res, "bHYPRE.ParaSails", FALSE);JAVA_CHECK(
    env);
  ...
  jobject _arg_A)
{
  ...
  struct bHYPRE_ParaSails__object* _ior = NULL;
...
void bHYPRE_ParaSails__register(JNIEnv* env)
{
  JNINativeMethod methods[29];
  jclass cls;

#ifdef SIDL_STATIC_LIBRARY
  s_external = bHYPRE_ParaSails__externals();
#else
  s_external = (struct bHYPRE_ParaSails__external*)sidl_dynamicLoadIOR(
    "bHYPRE.ParaSails","bHYPRE_ParaSails__externals") ;
  sidl_checkIORVersion("bHYPRE.ParaSails", s_external->d_ior_major_version, 
  ...
  methods[2].signature = "(LbHYPRE/ParaSails;)J";
  ...
  methods[7].signature = "(LbHYPRE/MPICommunicator;LbHYPRE/IJParCSRMatrix;)LbHYPRE/ParaSails;";
  ...
  cls = (*env)->FindClass(env, "bHYPRE/ParaSails");
  if (cls) {
