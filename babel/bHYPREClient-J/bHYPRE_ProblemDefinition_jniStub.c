/*
 * File:          bHYPRE_ProblemDefinition_jniStub.c
 * Symbol:        bHYPRE.ProblemDefinition-v1.0.0
 * Symbol Type:   interface
 * Babel Version: 1.0.4
 * Description:   Client-side JNI glue code for bHYPRE.ProblemDefinition
 * 
...
static struct bHYPRE_ProblemDefinition__object* _get_ior(
  JNIEnv* env,
  jobject obj)
{
  void* ptr = NULL;
  ...
  return (struct bHYPRE_ProblemDefinition__object*) ptr;
}
  ...
  jstring url)
{
  ...
  _res = POINTER_TO_JLONG(bHYPRE_ProblemDefinition__remoteConnect(_tmp_url, 1, 
    &_ex));
  if(_ex) {
  ...
  jobject _arg_mpi_comm)
{
  ...
  struct bHYPRE_ProblemDefinition__object* _ior = NULL;
...
void bHYPRE_ProblemDefinition__register(JNIEnv* env)
{
  ...
  cls = (*env)->FindClass(env, "bHYPRE/ProblemDefinition$Wrapper");
  if (cls) {
