/*
 * File:          bHYPRE_GMRES_jniStub.c
 * Symbol:        bHYPRE.GMRES-v1.0.0
 * Symbol Type:   class
 * Babel Version: 1.0.4
 * Description:   Client-side JNI glue code for bHYPRE.GMRES
 * 
...
#ifndef included_bHYPRE_GMRES_jniStub_h
#include "bHYPRE_GMRES_jniStub.h"
#endif
  ...
  *_ex = NULL; /* default to no exception */
  ...
  if (!cmp0) {
    ...
    if (!cmp1) {
      ...
      if (!cmp2) {
        (*self->d_epv->f_addRef)(self, _ex); SIDL_CHECK(*_ex);
        cast = ((struct bHYPRE_GMRES__object*)self);
        return cast;
      }
    }
    else if (cmp1 > 0) {
...
static void bHYPRE_GMRES__init_remote_epv(void)
{
  ...
  epv->f_GetPreconditioner             = remote_bHYPRE_GMRES_GetPreconditioner;
...
bHYPRE_GMRES__remoteConnect(const char *url, sidl_bool ar, sidl_BaseInterface 
  *_ex)
{
  struct bHYPRE_GMRES__object* self;
...
static struct bHYPRE_GMRES__object*
bHYPRE_GMRES__IHConnect(sidl_rmi_InstanceHandle instance, sidl_BaseInterface 
  *_ex)
{
  struct bHYPRE_GMRES__object* self;
...
bHYPRE_GMRES__remoteCreate(const char *url, sidl_BaseInterface *_ex)
{
  ...
  sidl_rmi_InstanceHandle instance = sidl_rmi_ProtocolFactory_createInstance(
    url, "bHYPRE.GMRES", _ex ); SIDL_CHECK(*_ex);
  if ( instance == NULL) { return NULL; }
  self =
    (struct bHYPRE_GMRES__object*) malloc(
      sizeof(struct bHYPRE_GMRES__object));
...
static struct bHYPRE_GMRES__object* _get_ior(
  JNIEnv* env,
  jobject obj)
{
  void* ptr = NULL;
  ...
  return (struct bHYPRE_GMRES__object*) ptr;
}
...
static const struct bHYPRE_GMRES__external* s_external = NULL;

/*
 * External reference to static EPV.
 */

static const struct bHYPRE_GMRES__sepv* s_sepv = NULL;
  ...
  jstring url)
{
  ...
  _res = POINTER_TO_JLONG(bHYPRE_GMRES__remoteCreate(_tmp_url, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject obj)
{
  ...
  struct bHYPRE_GMRES__object *_ptr = (*s_external->createObject)(g_data, &_ex);
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jstring url)
{
  ...
  jlong _res = 0;
  char* _tmp_url = sidl_Java_J2I_string(env, url);
  _res = POINTER_TO_JLONG(bHYPRE_GMRES__remoteConnect(_tmp_url, 1, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject _arg_A)
{
  ...
  struct bHYPRE_Operator__object* _tmp_A = (struct bHYPRE_Operator__object*) 
    NULL;
  struct bHYPRE_GMRES__object* _ior_res = (struct bHYPRE_GMRES__object*) NULL;
  ...
  _res = sidl_Java_I2J_cls(env, _ior_res, "bHYPRE.GMRES", FALSE);JAVA_CHECK(
    env);
  ...
  jobject _arg_s)
{
  ...
  struct bHYPRE_GMRES__object* _ior = NULL;
...
void bHYPRE_GMRES__register(JNIEnv* env)
{
  JNINativeMethod methods[32];
  jclass cls;

#ifdef SIDL_STATIC_LIBRARY
  s_external = bHYPRE_GMRES__externals();
#else
  s_external = (struct bHYPRE_GMRES__external*)sidl_dynamicLoadIOR(
    "bHYPRE.GMRES","bHYPRE_GMRES__externals") ;
  sidl_checkIORVersion("bHYPRE.GMRES", s_external->d_ior_major_version, 
  ...
  methods[2].signature = "(LbHYPRE/GMRES;)J";
  ...
  methods[7].signature = "(LbHYPRE/MPICommunicator;LbHYPRE/Operator;)LbHYPRE/GMRES;";
  ...
  cls = (*env)->FindClass(env, "bHYPRE/GMRES");
  if (cls) {
