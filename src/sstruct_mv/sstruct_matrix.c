...
#if defined(HYPRE_USING_SYCL)
         hypre_SyclSequence( nonzero_rows, nonzero_rows + num_rows, 0 );
#else
      ...
      for (var = 0; var < nvars; var++)
      {
         ...
         hypre_ForBoxI(i, grid_boxes)
         {
            ...
            if (exec == HYPRE_EXEC_DEVICE)
            {
               ...
               if (ndim > 0)
               {
                  hypre_SyclGather( box_nnzrows, box_nnzrows_end, all_indices_0, indices_0 );
               }

               if (ndim > 1)
               {
                  hypre_SyclGather( box_nnzrows, box_nnzrows_end, all_indices_1, indices_1 );
               }

               if (ndim > 2)
               {
                  hypre_SyclGather( box_nnzrows, box_nnzrows_end, all_indices_2, indices_2 );
               }
#else
               /* Get the nonzero rows for this box */
                              ...
                              HYPRE_Int            fill_diagonal )
{
   ...
   if (fill_diagonal)
   {
      ...
      if (exec == HYPRE_EXEC_DEVICE)
      {
         ...
         hypre_SyclSequence( rowidx, rowidx + nrows, 0 );
         hypre_SyclSequence( rows, rows + nrows, sizes[0] );
         hypre_SyclSequence( cols, cols + nrows, sizes[2] );
#else
