   ...
   for (fpart = 0; fpart < fnparts; fpart++)
   {
      if (coarsen_p[fpart])
      {
         ...
         for (fvar = 0; fvar < fnvars; fvar++)
         {
            ...
            {
               sfgrid = hypre_SStructPGridSGrid(pfgrid, fvar);
               bbox   = hypre_StructGridBoundingBox(sfgrid);
               start  = hypre_BoxIMin(bbox);

               for (d = 0; d < ndim; d++)
               {
                  hypre_IndexD(new_origin, d) = hypre_IndexD(origin, d) +
               ...
               cgrid_boxes = hypre_StructGridBoxes(scgrid);
               hypre_ForBoxI(i, cgrid_boxes)
               {
                  box = hypre_BoxArrayBox(cgrid_boxes, i);
                  hypre_AddIndexes(hypre_BoxIMin(box), new_origin, ndim, hypre_BoxIMin(box));
                  hypre_AddIndexes(hypre_BoxIMax(box), new_origin, ndim, hypre_BoxIMax(box));
               }
               ...
               hypre_ForBoxI(i, cpbnd_boxa)
               {
                  ...
                  hypre_SnapIndexPos(hypre_BoxIMin(box), new_origin, strides[fid], ndim);
                  hypre_MapToCoarseIndex(hypre_BoxIMin(box), new_origin, strides[fid], ndim);
                  hypre_AddIndexes(hypre_BoxIMin(box), new_origin, ndim, hypre_BoxIMin(box));

                  hypre_SnapIndexPos(hypre_BoxIMax(box), new_origin, strides[fid], ndim);
                  hypre_MapToCoarseIndex(hypre_BoxIMax(box), new_origin, strides[fid], ndim);
                  hypre_AddIndexes(hypre_BoxIMax(box), new_origin, ndim, hypre_BoxIMax(box));
               }
