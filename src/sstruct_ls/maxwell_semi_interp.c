                          ...
                          void                 *PTopology_vdata)
{
   ...
   int                    nFaces, nEdges, nElements, nedges;
   int                    nxFaces, nyFaces, nzFaces, nxEdges, nyEdges, nzEdges;
   int                    n_xFace_iedges, n_yFace_iedges, n_zFace_iedges;
   int                    n_Cell_iedges;

   int                    nElements_iedges, nFaces_iedges, nEdges_iedges;
   int                    nElements_Faces, nElements_Edges, nEdges_edges;

   int                   *iFace, *iEdge, *iElement, *iedgeEdge;
   int                   *jFace_edge, *jEdge_iedge, *jElement_edge;
   int                   *jElement_Face, *jElement_Edge, *jedge_Edge;

   double                *vals_ElementEdge, *vals_ElementFace, *vals_edgeEdge, *vals_Faceedge;
   double                *vals_Elementedge, *vals_Edgeiedge;
   int                   *ncols_Elementedge, *ncols_Edgeiedge, *ncols_edgeEdge, *ncols_Faceedge;
   int                   *ncols_ElementFace, *ncols_ElementEdge;
   int                   *bdryedge_location;
   double                 fCedge_ratio;
   ...
   int                    n_boxoffsets, component_stride;

   int                    nparts= hypre_SStructGridNParts(fgrid_element);
   int                    ndim  = hypre_SStructGridNDim(fgrid_element);
   ...
   int                    nvars, Face_nvars, Edge_nvars, part, var, box, fboxi;
   int                    tot_vars= 8;

   int                    t, i, j, k, l, m, n, p, r;

   int                    ilower, iupper;
   int                    jlower, jupper;
   int                  **flower_ranks, **fupper_ranks;
   int                  **clower_ranks, **cupper_ranks;
                                                                                                                  
   int                 ***n_CtoVbox, ****CtoVboxnums;
   int                   *num_vboxes, **vboxnums;
   ...
   vartype_map= hypre_CTAlloc(int, 8);
   ...
   *--------------------------------------------------------------------------*/
   cfbox_mapping= hypre_TAlloc(int *, nparts);
   fcbox_mapping= hypre_TAlloc(int *, nparts);
  ...
  /* variable rank bounds for this processor */
   n_CtoVbox   = hypre_TAlloc(int **, nparts);
   CtoVboxnums = hypre_TAlloc(int ***, nparts);
   for (part= 0; part< nparts; part++)
   {
  ...
  /* variable rank bounds for this processor */
   flower_ranks= hypre_TAlloc(int *, nparts);
   fupper_ranks= hypre_TAlloc(int *, nparts);

   clower_ranks= hypre_TAlloc(int *, nparts);
   cupper_ranks= hypre_TAlloc(int *, nparts);
   for (part= 0; part< nparts; part++)
   {
      flower_ranks[part]= hypre_CTAlloc(int, tot_vars);
      fupper_ranks[part]= hypre_CTAlloc(int, tot_vars);
     ...
     /* cell grid ranks */
      ...
      hypre_SStructGridFindMapEntry(fgrid_element, part, findex, 0,
                                   &entry);
      hypre_SStructMapEntryGetGlobalRank(entry, findex, &flower_ranks[part][0],
                                         matrix_type);

      fbox= hypre_BoxArrayBox(box_array, hypre_BoxArraySize(box_array)-1);
      hypre_CopyIndex(hypre_BoxIMax(fbox), findex);
      hypre_SStructGridFindMapEntry(fgrid_edge, part, findex, 0,
                                   &entry);
      hypre_SStructMapEntryGetGlobalRank(entry, findex, &fupper_ranks[part][0],
                                         matrix_type);

      clower_ranks[part]= hypre_CTAlloc(int, tot_vars);
      cupper_ranks[part]= hypre_CTAlloc(int, tot_vars);
      ...
      hypre_SStructGridFindMapEntry(cgrid_element, part, cindex, 0,
                                   &entry);
      hypre_SStructMapEntryGetGlobalRank(entry, cindex, &clower_ranks[part][0],
                                         matrix_type);

      cbox= hypre_BoxArrayBox(box_array, hypre_BoxArraySize(box_array)-1);
      hypre_CopyIndex(hypre_BoxIMax(cbox), cindex);
      hypre_SStructGridFindMapEntry(cgrid_edge, part, cindex, 0,
                                   &entry);
      hypre_SStructMapEntryGetGlobalRank(entry, cindex, &cupper_ranks[part][0],
                                         matrix_type);
     ...
     /* face grid ranks */
      ...
      for (i= 0; i< nvars; i++)
      {
         ...
         fbox     = hypre_BoxArrayBox(box_array, 0);
         hypre_CopyIndex(hypre_BoxIMin(fbox), findex);
         hypre_SStructGridFindMapEntry(fgrid_face, part, findex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, findex, &flower_ranks[part][t],
                                            matrix_type);
         ...
         hypre_SStructGridFindMapEntry(fgrid_face, part, findex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, findex, &fupper_ranks[part][t],
                                            matrix_type);
         ...
         cbox     = hypre_BoxArrayBox(box_array, 0);
         hypre_CopyIndex(hypre_BoxIMin(cbox), cindex);
         hypre_SStructGridFindMapEntry(cgrid_face, part, cindex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, cindex, &clower_ranks[part][t],
                                            matrix_type);
         ...
         hypre_SStructGridFindMapEntry(cgrid_face, part, cindex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, cindex, &cupper_ranks[part][t],
                                            matrix_type);
      }
     /* edge grid ranks */
      ...
      for (i= 0; i< nvars; i++)
      {
         ...
         fbox     = hypre_BoxArrayBox(box_array, 0);
         hypre_CopyIndex(hypre_BoxIMin(fbox), findex);
         hypre_SStructGridFindMapEntry(fgrid_edge, part, findex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, findex, &flower_ranks[part][t],
                                            matrix_type);
         ...
         cbox     = hypre_BoxArrayBox(box_array, 0);
         hypre_CopyIndex(hypre_BoxIMin(cbox), cindex);
         hypre_SStructGridFindMapEntry(cgrid_edge, part, cindex, i,
                                       &entry);
         hypre_SStructMapEntryGetGlobalRank(entry, cindex, &clower_ranks[part][t],
                                            matrix_type);
  ...
  /* lower rank */
   part= 0;
   box = 0;
   hypre_SStructGridBoxProcFindMapEntry(cgrid_element, part, 0, box, myproc, &entry);

   p_cgrid  = hypre_SStructGridPGrid(cgrid_element, part);
   var_cgrid= hypre_SStructPGridCellSGrid(p_cgrid) ;
   cboxes   = hypre_StructGridBoxes(var_cgrid);
   cbox     = hypre_BoxArrayBox(cboxes, 0);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMin(cbox), &ilower);

   p_fgrid = hypre_SStructGridPGrid(fgrid_edge, part);
   hypre_SStructGridBoxProcFindMapEntry(fgrid_edge, part, 0, box, myproc, &entry);
   
   var_fgrid= hypre_SStructPGridSGrid(p_fgrid, 0);
   fboxes   = hypre_StructGridBoxes(var_fgrid);
   fbox     = hypre_BoxArrayBox(fboxes, 0);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMin(fbox), &jlower);
  ...
  /* upper rank */
   ...
   hypre_SStructGridBoxProcFindMapEntry(cgrid_element, part, 0, hypre_BoxArraySize(cboxes)-1, 
                                        myproc, &entry);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMax(cbox), &iupper);
   ...
   hypre_SStructGridBoxProcFindMapEntry(fgrid_edge, part, nvars-1, hypre_BoxArraySize(fboxes)-1, 
                                        myproc, &entry);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMax(fbox), &jupper);
  ...
  /* lower rank */
   part= 0;
   box = 0;
   hypre_SStructGridBoxProcFindMapEntry(cgrid_edge, part, 0, box, myproc, &entry);
   p_cgrid = hypre_SStructGridPGrid(cgrid_edge, part);
   var_cgrid= hypre_SStructPGridSGrid(p_cgrid, 0);
   cboxes   = hypre_StructGridBoxes(var_cgrid);
   cbox    = hypre_BoxArrayBox(cboxes, 0);
  ...
  /* lower rank */
   ...
   fboxes   = hypre_StructGridBoxes(var_fgrid);
   fbox    = hypre_BoxArrayBox(fboxes, 0);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMin(fbox), &ilower);
   ...
   cboxes   = hypre_StructGridBoxes(var_cgrid);
   cbox    = hypre_BoxArrayBox(cboxes, 0);
   hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMin(cbox), &jlower);
  ...
  /* Face_iedge. Only needed in 3-d. */
   if (ndim == 3)
   {
      /* lower rank */
       part= 0;
       box = 0;
       hypre_SStructGridBoxProcFindMapEntry(cgrid_face, part, 0, box, myproc, &entry);
      ...
      /* upper rank */
       ...
       hypre_SStructGridBoxProcFindMapEntry(cgrid_face, part, nvars-1, 
                                       hypre_BoxArraySize(cboxes)-1, myproc, &entry);
       hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMax(cbox), &iupper);
  ...
  /* lower rank */
   if (ndim == 3)
   {
      part= 0;
      box = 0;
      hypre_SStructGridBoxProcFindMapEntry(cgrid_element, part, 0, box, 
                                           myproc, &entry);
      ...
      hypre_SStructGridBoxProcFindMapEntry(cgrid_face, part, 0, box, 
                                           myproc, &entry);
      p_cgrid = hypre_SStructGridPGrid(cgrid_face, part);
      var_cgrid= hypre_SStructPGridSGrid(p_cgrid, 0);
      cboxes  = hypre_StructGridBoxes(var_cgrid);
     ...
     /* upper rank */
      ...
      hypre_SStructGridBoxProcFindMapEntry(cgrid_element, part, nvars-1, 
                                     hypre_BoxArraySize(cboxes)-1, myproc, &entry);
      hypre_SStructMapEntryGetGlobalCSRank(entry, hypre_BoxIMax(cbox), &iupper);
   ...
   * Need to distinguish between 2 and 3-d. */
   ...
   if (ndim == 3)
   {
      iFace   = hypre_CTAlloc(int, nFaces);
   }
   iEdge   = hypre_CTAlloc(int, nEdges);
   iElement= hypre_CTAlloc(int, nElements);

  /* array structures needed for forming ij_matrices */

  /* Element_edge. Same for 2-d and 3-d. */
   ncols_Elementedge= hypre_CTAlloc(int, nElements);
   for (i= 0; i< nElements; i++)
   {
      ncols_Elementedge[i]= n_Cell_iedges;
   }
   jElement_edge    = hypre_CTAlloc(int, nElements_iedges);
...
#include "hypre_box_smp_forloop.h"
         hypre_BoxLoop0For(loopi, loopj, loopk)
         {
            ...
            /* Element(i,j,k) rank */
             hypre_SStructGridFindMapEntry(cgrid_element, part, cindex, 0, &entry);
             hypre_SStructMapEntryGetGlobalRank(entry, cindex, &rank, matrix_type);
                      ...
                      for (k= low_index[1]; k<= hi_index[1]; k++)
                      {
                         for (j= low_index[0]; j<= hi_index[0]; j++)
                         {
                            ...
                            hypre_SStructGridFindMapEntry(fgrid_edge, part, var_index, 
                                                          t, &entry);
                            hypre_SStructMapEntryGetGlobalRank(entry, var_index, 
                                                               &rank, matrix_type);
   ...
   HYPRE_IJMatrixSetValues(Element_iedge, nElements, ncols_Elementedge,
                          (const int*) iElement, (const int*) jElement_edge,
   ...
   *------------------------------------------------------------------------------*/
   if (ndim == 3)
   {
      ncols_Faceedge = hypre_CTAlloc(int, nFaces);
      nFaces= 0;
      j= 0; 
      for (part= 0; part< nparts; part++)
      {
      ...
      jFace_edge= hypre_CTAlloc(int, j);
...
#include "hypre_box_smp_forloop.h"
                      hypre_BoxLoop0For(loopi, loopj, loopk)
                      {
                         ...
                         hypre_SStructGridFindMapEntry(cgrid_face, part, cindex, t, &entry);
                         hypre_SStructMapEntryGetGlobalRank(entry, cindex, &rank, matrix_type);
...
#include "hypre_box_smp_forloop.h"
      ...
      HYPRE_IJMatrixSetValues(Face_iedge, nFaces, ncols_Faceedge,
                             (const int*) iFace, (const int*) jFace_edge,
  ...
  /* nEdges should be correct for 2-d & 3-d */
   ncols_Edgeiedge= hypre_CTAlloc(int, nEdges);
   ...
   jEdge_iedge= hypre_CTAlloc(int, k);
...
#include "hypre_box_smp_forloop.h"
            hypre_BoxLoop0For(loopi, loopj, loopk)
            {
              ...
              /* row rank */
               hypre_SStructGridFindMapEntry(cgrid_edge, part, cindex, t,
                                             &entry);
               ...
               /* cell_index is refined to the upper fine index. Shift
                  ...
                  switch(var)
                  {
                     case 2:    /* 2-d, x_face */
                     {
                         ...
                         {
                            hypre_SStructGridFindMapEntry(fgrid_edge, part,
                                                          var_index, t, &entry);
                            hypre_SStructMapEntryGetGlobalRank(entry, var_index,
                                                              &rank, matrix_type);
   ...
   HYPRE_IJMatrixSetValues(Edge_iedge, nEdges, ncols_Edgeiedge,
                          (const int*) iEdge, (const int*) jEdge_iedge,
  ...
  /* Element_Face & Element_Edge. Element_Face only for 3-d. */
   if (ndim == 3)
   {
      ncols_ElementFace= hypre_CTAlloc(int, nElements);
      j= 2*ndim;
      for (i= 0; i< nElements; i++)
      {
      ...
      j*= nElements;
      jElement_Face   = hypre_CTAlloc(int, j);
   ...
   k*= nElements;
   jElement_Edge   = hypre_CTAlloc(int, k);
...
#include "hypre_box_smp_forloop.h"
            hypre_BoxLoop0For(loopi, loopj, loopk)
            {
               ...
               for (t= 0; t< Face_nvars; t++)
               {
                  ...
                  hypre_SStructGridFindMapEntry(cgrid_face, part, cindex, t,
                                                &entry);
                  hypre_SStructMapEntryGetGlobalRank(entry, cindex, &rank,
                                                     matrix_type);
                  ...
                  hypre_SStructGridFindMapEntry(cgrid_face, part, var_index, t,
                                                &entry);
...
#include "hypre_box_smp_forloop.h"
              ...
              /* Edge (i,j,k) */
               ...
               switch (var)
               {
                  case 2: /* x_Face= {(i,j), (i-1,j)} */
                  {
                      ...
                      hypre_SStructGridFindMapEntry(cgrid_edge, part, var_index, t,
                                                    &entry);
                      hypre_SStructMapEntryGetGlobalRank(entry, var_index, &rank,
                                                         matrix_type);
                      jElement_Edge[nElements_Edges]= rank;
   ...
   if (ndim == 3)
   {
       HYPRE_IJMatrixSetValues(Element_Face, nElements, ncols_ElementFace,
                              (const int*) iElement, (const int*) jElement_Face,
  ...
  /* count the row/col connections */
   iedgeEdge     = hypre_CTAlloc(int, nedges);
   ncols_edgeEdge= hypre_CTAlloc(int, nedges);
   ...
   *-----------------------------------------------------------------------*/
   ...
   bdryedge_location= hypre_CTAlloc(int, nedges);
   for (part= 0; part< nparts; part++)
   {
...
#define HYPRE_BOX_SMP_PRIVATE loopk,loopi,loopj,m
#include "hypre_box_smp_forloop.h"
             hypre_BoxLoop1For(loopi, loopj, loopk, m)
             {
                ...
                hypre_SStructMapEntryGetGlobalRank(entry, findex, &rank, matrix_type);
...
#include "hypre_box_smp_forloop.h"
   ...
   jedge_Edge    = hypre_CTAlloc(int, k);
   size1         = j;
...
#include "hypre_box_smp_forloop.h"
             hypre_BoxLoop1For(loopi, loopj, loopk, m)
             {
                ...
                hypre_AddIndex(findex, start, findex);
                hypre_SStructGridFindMapEntry(fgrid_edge, part, findex, t, &entry);
                hypre_SStructMapEntryGetGlobalRank(entry, findex, &j, matrix_type);
  ...
  /* generate the face interpolation weights/info. Only for 3-d */
   if (ndim == 3)
   {
     /* Allocate memory to arrays for the tridiagonal system & solutions.
      ...
      off_proc_flag= hypre_CTAlloc(int, i+1);
...
#define HYPRE_BOX_SMP_PRIVATE loopk,loopi,loopj,r
#include "hypre_box_smp_forloop.h"
...
#include "hypre_box_smp_forloop.h"
   ...
   HYPRE_IJMatrixSetValues(edge_Edge, size1, ncols_edgeEdge,
                          (const int*) iedgeEdge, (const int*) jedge_Edge,
                               ...
                               double                **collapsed_vals_ptr)
{
   ...
   int                      matrix_type= HYPRE_PARCSR;
   int                      start_rank = hypre_ParCSRMatrixFirstRowIndex(Aee);
   int                      end_rank   = hypre_ParCSRMatrixLastRowIndex(Aee);

   hypre_BoxMapEntry       *entry;

   int                     *ranks;
   int                     *marker;     /* marker to record the rank groups */
   int                      max_ranksize= 9;
   ...
   int                      size, *col_inds, *col_inds2;
   double                  *values;
   int                      rank, row_rank, *swap_inds;

   int                      i, j, m, centre, found;
   int                      getrow_ierr;
   int                      cnt;
  ...
  /* check if the row corresponding to pt_location is on this proc. If
    ...
    hypre_SStructGridFindMapEntry(grid, part, pt_location, var, &entry);
    hypre_SStructMapEntryGetGlobalRank(entry, pt_location, &rank, matrix_type);
   ...
   /* Extract the ranks of the collapsed stencil pattern. Since only like-var
    ...
    ranks = hypre_TAlloc(int, max_ranksize);
    marker= hypre_TAlloc(int, max_ranksize);
    ...
    for (j= -1; j<= 1; j++)
    {
       ...
       for (i= -1; i<= 1; i++)
       {
          ...
          hypre_SStructGridFindMapEntry(grid, part, index2, var, &entry);
          if (entry)
          {
             hypre_SStructMapEntryGetGlobalRank(entry, index2, &rank, matrix_type);
             ranks[cnt] = rank;
             marker[cnt]= j+1;
    ...
    if (getrow_ierr < 0)
       printf("offproc collapsing problem");

    swap_inds= hypre_TAlloc(int, size);
    col_inds2= hypre_TAlloc(int, size);
    for (i= 0; i< size; i++)
    {
       swap_inds[i]= i;
