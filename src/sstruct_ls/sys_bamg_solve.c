...
HYPRE_Int
hypre_SysBAMGSolve( void                 *sys_bamg_vdata,
                    ...
                    hypre_SStructVector  *x_in         )
{
   ...
   HYPRE_Real            tol             = (sys_bamg_data -> tol);
   ...
   hypre_SStructPMatrix  **RT_l          = (sys_bamg_data -> RT_l);
   hypre_SStructPVector  **b_l           = (sys_bamg_data -> b_l);
   hypre_SStructPVector  **x_l           = (sys_bamg_data -> x_l);
   hypre_SStructPVector  **r_l           = (sys_bamg_data -> r_l);
   hypre_SStructPVector  **e_l           = (sys_bamg_data -> e_l);
   void                **relax_data_l    = (sys_bamg_data -> relax_data_l);
   void                **matvec_data_l   = (sys_bamg_data -> matvec_data_l);
   void                **restrict_data_l = (sys_bamg_data -> restrict_data_l);
   void                **interp_data_l   = (sys_bamg_data -> interp_data_l);
   HYPRE_Int             logging         = (sys_bamg_data -> logging);
   HYPRE_Real           *norms           = (sys_bamg_data -> norms);
   HYPRE_Real           *rel_norms       = (sys_bamg_data -> rel_norms);
   HYPRE_Int            *active_l        = (sys_bamg_data -> active_l);
   ...
   hypre_BeginTiming(sys_bamg_data -> time_index);
   ...
   (sys_bamg_data -> num_iterations) = 0;
   ...
   if (max_iter == 0)
   {
      ...
      hypre_EndTiming(sys_bamg_data -> time_index);
   ...
   for (i = 0; i < max_iter; i++)
   {
      ...
      (sys_bamg_data -> num_iterations) = (i + 1);
   }
