                                ...
                                hypre_IntArray       **CF_marker_ptr)
{
   ...
   while (1)
   {
      ...
      FILE *my_fp;
      HYPRE_Int my_print_debug_files = 0;
   ...
   /* WM: look at number of F-F connections that don't share a C-pt */
   HYPRE_Int k, l;
   HYPRE_Int num_bad_ff_connections = 0;
   for (i = 0; i < num_variables; i++)
   {
      /* for each F-pt */
      if (CF_marker[i] < 0)
      {
         /* get lists of adjacent C and F points */
         HYPRE_Int row_size = S_diag_i[i+1] - S_diag_i[i] + S_offd_i[i+1] - S_offd_i[i];
         HYPRE_Int *C_list = hypre_CTAlloc(HYPRE_Int, row_size, HYPRE_MEMORY_HOST);
         HYPRE_Int *F_list = hypre_CTAlloc(HYPRE_Int, row_size, HYPRE_MEMORY_HOST);
         HYPRE_Int C_cnt = 0;
         HYPRE_Int F_cnt = 0;
         for (j = S_diag_i[i]; j < S_diag_i[i+1]; j++)
         {
            ...
            {
               C_list[C_cnt++] = S_diag_j[j];
            }
            else
            {
               F_list[F_cnt++] = S_diag_j[j];
            }
         }
         for (j = S_offd_i[i]; j < S_offd_i[i+1]; j++)
         {
            ...
            {
               C_list[C_cnt++] = S_offd_j[j];
            }
            /* WM: how to count the offd connections??? */
            /* The problem is that I don't have access to the row for the adjacent F-pt */ 
            /* else */
            /* { */
            /*    F_list[F_cnt++] = S_offd_j[j]; */
            /* } */
         }
         /* loop over adjacent F-pts */
         for (j = 0; j < F_cnt; j++)
         {
            HYPRE_Int shared_C = 0;
            /* look for diag C connections of the adjacent F-pt */
            for (k = S_diag_i[ F_list[j] ]; k < S_diag_i[ F_list[j] + 1 ]; k++)
            {
               if (CF_marker[ S_diag_j[k] ] > 0)
               {
                  /* look for this C-pt in the C_list (i.e. C connections to the original F point) */
                  for (l = 0; l < C_cnt; l++)
                  {
                     if (C_list[l] == S_diag_j[k])
                     {
                        shared_C = 1;
                        break;
                     }
            ...
            /* look for offd C connections of the adjacent F-pt */
            for (k = S_offd_i[ F_list[j] ]; k < S_offd_i[ F_list[j] + 1 ]; k++)
            {
            ...
            if (!shared_C)
            {
               num_bad_ff_connections++;
            }
         }
      }
   }
   HYPRE_Int total_num_bad_ff_connections = 0;
   hypre_MPI_Allreduce(&num_bad_ff_connections, &total_num_bad_ff_connections, 1, HYPRE_MPI_INT, hypre_MPI_SUM, comm);
   if (my_id == 0)
   {
      hypre_printf("WM: number of strong F-F connections without shared C point = %d\n", total_num_bad_ff_connections / 2);
   }
...
/* WM: Test implementation for parallel RS coarsening idea
 * stated by copying PMISHost and modifying */
HYPRE_Int
hypre_BoomerAMGCoarsenPRS( hypre_ParCSRMatrix    *S,
                           hypre_ParCSRMatrix    *A,
                           HYPRE_Int              CF_init,
                           HYPRE_Int              debug_flag,
                           hypre_IntArray       **CF_marker_ptr)
{
   ...
   /* WM: get the matrix describing all distance 2 paths through the graph of S */
   /* remove diagonal from S */
   hypre_ParCSRMatrix *S_nodiag = hypre_ParCSRMatrixClone(S, 0);
   hypre_CSRMatrixRemoveDiagonalHost(hypre_ParCSRMatrixDiag(S_nodiag));
   /* make sure data is all ones */
   hypre_CSRMatrix *S_nodiag_diag = hypre_ParCSRMatrixDiag(S_nodiag);
   HYPRE_Int S_nodiag_diag_nnz = hypre_CSRMatrixNumNonzeros(S_nodiag_diag);
   hypre_CSRMatrixData(S_nodiag_diag) = hypre_CTAlloc(HYPRE_Complex, S_nodiag_diag_nnz, HYPRE_MEMORY_DEVICE);
   for (i = 0; i < S_nodiag_diag_nnz; i++)
   {
      hypre_CSRMatrixData(S_nodiag_diag)[i] = 1.0;
   }
   hypre_CSRMatrix *S_nodiag_offd = hypre_ParCSRMatrixOffd(S_nodiag);
   HYPRE_Int S_nodiag_offd_nnz = hypre_CSRMatrixNumNonzeros(S_nodiag_offd);
   hypre_CSRMatrixData(S_nodiag_offd) = hypre_CTAlloc(HYPRE_Complex, S_nodiag_offd_nnz, HYPRE_MEMORY_DEVICE);
   for (i = 0; i < S_nodiag_offd_nnz; i++)
   {
      hypre_CSRMatrixData(S_nodiag_offd)[i] = 1.0;
   }
   /* multiply to get matrix of all distance 2 walks */
   hypre_ParCSRMatrix *dist2walks = hypre_ParMatmul(S_nodiag, S_nodiag);
   /* remove diagonal */
   hypre_CSRMatrixRemoveDiagonalHost(hypre_ParCSRMatrixDiag(dist2walks));
   /* get diag and offd */
   hypre_CSRMatrix *dist2walks_diag = hypre_ParCSRMatrixDiag(dist2walks);
   hypre_CSRMatrix *dist2walks_offd = hypre_ParCSRMatrixOffd(dist2walks);

   /* WM: debug */
   /* hypre_ParCSRMatrixPrint(dist2walks, "dist2walks"); */

   /* WM: init some vectors */
   hypre_ParVector *rs_weights = hypre_ParVectorCreate(hypre_ParCSRMatrixComm(S),
                                                       hypre_ParCSRMatrixGlobalNumRows(S),
                                                       hypre_ParCSRMatrixRowStarts(S));
   hypre_ParVectorInitialize(rs_weights);
   hypre_ParVector *cpt_vector = hypre_ParVectorCreate(hypre_ParCSRMatrixComm(S),
                                                       hypre_ParCSRMatrixGlobalNumRows(S),
                                                       hypre_ParCSRMatrixRowStarts(S));
   hypre_ParVectorInitialize(cpt_vector);
   ...
   /* WM: TODO figure out the single processor case */
   if (num_procs == 1)
   {
      hypre_printf("WM: PRS not yet implemented for single processor case\n");
      hypre_MPI_Finalize();
      exit(1);
   }
   ...
   while (1)
   {
      ...
      if (!CF_init || iter)
      {
         ...
         if (iter > 0)
         {
            /* WM: reject C-pts that are not within distance 2 of at least 1 C-pt */
            for (ig = 0; ig < graph_size; ig++)
            {
               i = graph_array[ig];
               if (CF_marker[i] > 0 && measure_array[i] < 2.0)
               {
                  CF_marker[i] = 0;
               }
            }
      ...
      if (iter > 100)
      {
         hypre_printf("PRS iteration = %d... stopping and calling hmis instead\n", iter);
         ...
         /* } */
         /* hypre_printf("\n"); */
         hypre_IntArraySetConstantValues(*CF_marker_ptr, 0);
         hypre_BoomerAMGCoarsenHMIS(S, A, 0, 0, debug_flag, CF_marker_ptr);
         hypre_ParCSRMatrixDestroy(dist2walks);
         hypre_TFree(measure_array, HYPRE_MEMORY_HOST);
         hypre_TFree(graph_array, HYPRE_MEMORY_HOST);
         hypre_TFree(graph_array2, HYPRE_MEMORY_HOST);
         hypre_TFree(graph_array_offd2, HYPRE_MEMORY_HOST);
...
/* WM: Scratch work section with saved snippets from all ideas */
HYPRE_Int
hypre_BoomerAMGCoarsenPRSScratch( hypre_ParCSRMatrix    *S,
                                  hypre_ParCSRMatrix    *A,
                                  HYPRE_Int              CF_init,
                                  HYPRE_Int              debug_flag,
                                  hypre_IntArray       **CF_marker_ptr)
