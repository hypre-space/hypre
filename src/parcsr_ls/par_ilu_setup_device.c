 ...
 *
 * TODO (VPM): Refactor this function after creating another routine for
                              ...
                              hypre_CSRMatrix       **Fptr)
{
   ...
   if (ilu_type != 0)
   {
      hypre_error_w_msg(HYPRE_ERROR_GENERIC, "Iterative algorithm only works for ILU0!");
      return hypre_error_flag;
   }
   ...
   {
      /* Copy diagonal matrix into a new place with permutation
       * That is, A_diag = A_diag(perm, perm); */
