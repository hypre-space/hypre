                                  ...
                                  hypre_ParCSRMatrix **P_ptr)
{
   ...
   /*------------- allocate arrays */
   P_diag_i    = hypre_CTAlloc(HYPRE_Int,  n_fine+1);
   P_diag_j    = hypre_CTAlloc(HYPRE_Int,  nnz_diag);
   P_diag_data = hypre_CTAlloc(HYPRE_Real, nnz_diag);

   /* not in ``if num_procs > 1'', 
    * allocation needed even for empty CSR */
   P_offd_i    = hypre_CTAlloc(HYPRE_Int,  n_fine+1);
   P_offd_j    = hypre_CTAlloc(HYPRE_Int,  nnz_offd);
   P_offd_data = hypre_CTAlloc(HYPRE_Real, nnz_offd);
   ...
   for (i = 0; i < num_sends; i++)
   {
      ...
      for (j = start; j < hypre_ParCSRCommPkgSendMapStart(comm_pkg, i+1); j++)
      {
         int_buf_data[index++] = fine_to_coarse[hypre_ParCSRCommPkgSendMapElmt(comm_pkg,j)];
      }
   }
   ...
   {
      i1 = P_offd_j[i];
      k1 = hypre_BinarySearch(col_map_offd_P, i1, num_cols_offd_P);
      /* search must succeed */
