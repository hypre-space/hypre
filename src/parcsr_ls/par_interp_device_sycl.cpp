                                     ...
                                     hypre_ParCSRMatrix  **P_ptr)
{
   ...
   HYPRE_Int          *tmp_map_offd_h = nullptr;

   HYPRE_Int       *CF_marker_dev = nullptr;
   HYPRE_Int       *CF_marker_offd = nullptr;
   HYPRE_Int       *dof_func_offd = nullptr;
   HYPRE_Int       *dof_func_dev = nullptr;
   ...
   sycl::range<1> gDim = hypre_GetDefaultSYCLGridDimension(n_fine, "warp", bDim);

   HYPRE_SYCL_1D_LAUNCH( hypre_BoomerAMGBuildDirInterp_getnnz, gDim, bDim,
   ...
   if (interp_type == 3)
   {
     HYPRE_SYCL_1D_LAUNCH( hypre_BoomerAMGBuildDirInterp_getcoef, gDim, bDim,
   ...
   {
     HYPRE_SYCL_1D_LAUNCH( hypre_BoomerAMGBuildDirInterp_getcoef_v2, gDim, bDim,
			   n_fine, A_diag_i, A_diag_j, A_diag_data,
			   A_offd_i, A_offd_j, A_offd_data,
			   hypre_ParCSRMatrixSocDiagJ(S),
			   hypre_ParCSRMatrixSocOffdJ(S),
   ...
   */
   ...
   if (P_offd_size)
   {
      ...
      HYPRE_ONEDPL_CALL(gather, P_offd_j, P_offd_j + P_offd_size, P_colids, P_offd_j);
                                      ...
                                      HYPRE_Int *col_offd_S_to_A )
{
    ...
    *--------------------------------------------------------------------*/
   if (num_functions > 1 && dof_func != nullptr)
   {
      if (lane == 0)
      {
         dof_func_i = read_only_load(&dof_func[i]);
      }
      dof_func_i = SG.shuffle( dof_func_i, 0);
   }
                                       ...
                                       HYPRE_Int  *fine_to_coarse )
{
   ...
   */
   ...
   q_diag_A = SG.shuffle( p_diag_A, 1);
   p_diag_A = SG.shuffle( p_diag_A, 0);
   q_diag_P = SG.shuffle( p_diag_P, 1);
   ...
   for (HYPRE_Int j = p_diag_A + lane; sycl::ONEAPI::any_of(grp, j < q_diag_A); j += sub_group_size)
   {
      ...
      pos = warp_prefix_sum(lane, is_SC, sum, item);
   ...
   q_offd_A = SG.shuffle( p_offd_A, 1);
   p_offd_A = SG.shuffle( p_offd_A, 0);
   q_offd_P = SG.shuffle( p_offd_P, 1);
   ...
   diagonal  = warp_allreduce_sum(diagonal, item);
   sum_N_pos = warp_allreduce_sum(sum_N_pos, item);
   sum_N_neg = warp_allreduce_sum(sum_N_neg, item);
   sum_P_pos = warp_allreduce_sum(sum_P_pos, item);
   sum_P_neg = warp_allreduce_sum(sum_P_neg, item);
                                          ...
                                          HYPRE_Int  *fine_to_coarse )
{
   ...
   */
   ...
   diagonal  = warp_allreduce_sum(diagonal, item);
   sum_F     = warp_allreduce_sum(sum_F, item);
