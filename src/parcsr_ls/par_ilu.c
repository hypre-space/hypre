...
hypre_ILUSortOffdColmap(hypre_ParCSRMatrix *A)
{
   ...
   hypre_TMemcpy(A_offd_j_host, A_offd_j, HYPRE_Int,  A_offd_nnz, HYPRE_MEMORY_HOST,
                 HYPRE_MEMORY_DEVICE);
#else
...
#if defined(HYPRE_USING_GPU) && !defined(HYPRE_USING_UNIFIED_MEMORY)
   hypre_TMemcpy(A_offd_j, A_offd_j_host, HYPRE_Int, A_offd_nnz, HYPRE_MEMORY_DEVICE,
                 HYPRE_MEMORY_HOST);
                  ...
                  HYPRE_Int        sym)
{
   ...
   /* Check if G has only diagonal (no need to do any kind of RCM) */
   if (G_nnz == 0)
   {
   ...
   if (memory_location == HYPRE_MEMORY_DEVICE)
   {
      ...
      d_qperm = hypre_TAlloc(HYPRE_Int, num_nodes, HYPRE_MEMORY_DEVICE);

      hypre_TMemcpy(d_perm, perm, HYPRE_Int, num_nodes,
