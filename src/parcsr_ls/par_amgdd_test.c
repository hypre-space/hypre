...
TestBoomerAMGSolve( void               *amg_vdata,
                   ...
                   hypre_ParVector    *f,
                   hypre_ParVector    *u,
                   hypre_ParVector    **suppress_relax         );
                   ...
                   hypre_ParVector    *u                 )
{
  ...
  hypre_ParCSRMatrixMatvec(-1.0, A, u, 1.0, f );
  ...
  for (proc = 0; proc < num_procs; proc++)
  {
    ...
    hypre_ParVector  **suppress_relax = hypre_CTAlloc(hypre_ParVector*, num_levels, HYPRE_MEMORY_HOST);
    for (level = 0; level < num_levels; level++)
    {
      // Create and initialize the suppress_relax vector on this level
      suppress_relax[level] = hypre_ParVectorCreate(hypre_MPI_COMM_WORLD, hypre_ParVectorGlobalSize(U_array[level]), hypre_ParVectorPartitioning(U_array[level]));
      hypre_ParVectorInitialize(suppress_relax[level]);
    ...
    TestBoomerAMGSolve(amg_data, A, f, U_array[0], suppress_relax);


    // Update the values in the global solution for this proc
    if (myid == proc)
    {
      // add local part of u_array[0] to local part of u 
    }
  }
...
*/
   ...
   while ( (relative_resid >= tol || cycle_count < min_iter) && cycle_count < max_iter )
   {
      ...
      && (simple < 0 || simple >= num_levels) )
         TestBoomerAMGCycle(amg_data, F_array, U_array, suppress_relax); 
      else
