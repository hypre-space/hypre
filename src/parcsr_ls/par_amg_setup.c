...
char aname[20];
char sname[20];
hypre_sprintf(aname, "%s.%d", "S_aux_", level);
hypre_sprintf(sname, "%s.%d", "A_", level);
                     /* Build auxilliary matrix for strength */
                     ...
                     hypre_BoomerAMGCreateS(S_aux, strong_threshold, max_row_sum,
                                         num_functions, dof_func_data, &S);
hypre_ParCSRMatrixPrintIJ(A_array[level], 0, 0, aname);
hypre_ParCSRMatrixPrintIJ(S_aux, 0, 0, sname);
