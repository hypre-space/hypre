...
/*--------------------------------------------------------------------------
 * hypre_SchwarzRASVariantFromLocalSolver
 *--------------------------------------------------------------------------*/

static HYPRE_Int
hypre_SchwarzRASVariantFromLocalSolver(HYPRE_Int local_solver_type)
{
   switch (local_solver_type)
   {
      case HYPRE_SCHWARZ_LOCAL_SOLVER_ILUK:
         return HYPRE_SCHWARZ_VARIANT_RAS_ILUK;

      case HYPRE_SCHWARZ_LOCAL_SOLVER_ILUT:
         return HYPRE_SCHWARZ_VARIANT_RAS_ILUT;

      case HYPRE_SCHWARZ_LOCAL_SOLVER_AMG:
         return HYPRE_SCHWARZ_VARIANT_RAS_AMG;

      case HYPRE_SCHWARZ_LOCAL_SOLVER_SUPERLU:
         return HYPRE_SCHWARZ_VARIANT_RAS_SUPERLU;
...
hypre_SchwarzOverlapDataDestroy(hypre_SchwarzData *schwarz_data)
{
   ...
   if (hypre_SchwarzDataALocalParCSR(schwarz_data))
   {
      ...
      hypre_ParCSRMatrixDiag(A_local_parcsr) = NULL;
      hypre_ParCSRMatrixDestroy(A_local_parcsr);
      hypre_SchwarzDataALocalParCSR(schwarz_data) = NULL;
   }

   hypre_CSRMatrixDestroy(hypre_SchwarzDataALocal(schwarz_data));
   hypre_SchwarzDataALocal(schwarz_data) = NULL;

   hypre_TFree(hypre_SchwarzDataColMap(schwarz_data), HYPRE_MEMORY_HOST);
   hypre_SchwarzDataColMap(schwarz_data) = NULL;
                          ...
                          hypre_ParVector          *u)
{
   ...
   hypre_SchwarzDataALocal(schwarz_data) = A_local;
   hypre_SchwarzDataColMap(schwarz_data) = col_map;
   ...
   switch (local_solver_type)
   {
      ...
      case HYPRE_SCHWARZ_LOCAL_SOLVER_AMG:
      {
         ...
         /* Set AMG parameters for single V-cycle */
      ...
      case HYPRE_SCHWARZ_LOCAL_SOLVER_SUPERLU:
      {
         ...
         /* Setup SuperLU_dist on local matrix */
   ...
   /* Local ParVector wrappers for local solver */
   {
      ...
      /* Create local vector wrappers using hypre_ParVectorCreate */
   ...
   if (print_level > 0)
   {
      ...
      local_num_rows = hypre_OverlapDataNumLocalRows(overlap_data);
      ...
      /* Compute local matrix statistics using utility function */
      ...
      /* Stddev computation helper variables */
      HYPRE_Real stdev_local, stdev_overlap, stdev_nnz;

      /* Gather global statistics via MPI reductions */
      {
         HYPRE_Real sqsum;
         hypre_MPI_Allreduce(&local_size_real, &avg_local_size, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         avg_local_size /= (HYPRE_Real) num_procs;
         hypre_MPI_Allreduce(&local_size_real, &max_local_size, 1, HYPRE_MPI_REAL, hypre_MPI_MAX, comm);
         hypre_MPI_Allreduce(&local_size_real, &min_local_size, 1, HYPRE_MPI_REAL, hypre_MPI_MIN, comm);
         HYPRE_Real local_sqsum = local_size_real * local_size_real;
         hypre_MPI_Allreduce(&local_sqsum, &sqsum, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         HYPRE_Real var = sqsum / (HYPRE_Real) num_procs - avg_local_size * avg_local_size;
      ...
      /* Overlap statistics */
      {
         ...
         HYPRE_Real sqsum;
         hypre_MPI_Allreduce(&overlap_real, &avg_overlap, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         avg_overlap /= (HYPRE_Real) num_procs;
         hypre_MPI_Allreduce(&overlap_real, &max_overlap, 1, HYPRE_MPI_REAL, hypre_MPI_MAX, comm);
         hypre_MPI_Allreduce(&overlap_real, &min_overlap, 1, HYPRE_MPI_REAL, hypre_MPI_MIN, comm);
         HYPRE_Real local_sqsum = overlap_real * overlap_real;
         hypre_MPI_Allreduce(&local_sqsum, &sqsum, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         HYPRE_Real var = sqsum / (HYPRE_Real) num_procs - avg_overlap * avg_overlap;
      ...
      /* NNZ statistics */
      {
         ...
         HYPRE_Real sqsum;
         hypre_MPI_Allreduce(&nnz_real, &avg_nnz, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         avg_nnz /= (HYPRE_Real) num_procs;
         hypre_MPI_Allreduce(&nnz_real, &max_nnz, 1, HYPRE_MPI_REAL, hypre_MPI_MAX, comm);
         hypre_MPI_Allreduce(&nnz_real, &min_nnz, 1, HYPRE_MPI_REAL, hypre_MPI_MIN, comm);
         HYPRE_Real local_sqsum = nnz_real * nnz_real;
         hypre_MPI_Allreduce(&local_sqsum, &sqsum, 1, HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
         HYPRE_Real var = sqsum / (HYPRE_Real) num_procs - avg_nnz * avg_nnz;
      ...
      /* Print statistics on rank 0 - compact table format */
      if (my_id == 0)
      {
                          ...
                          hypre_ParVector          *u)
{
   ...
   A_local = hypre_SchwarzDataALocal(schwarz_data);
   col_map = hypre_SchwarzDataColMap(schwarz_data);
   ...
   /* Variables for residual tracking */
   ...
   HYPRE_UNUSED_VAR(A_local);
   HYPRE_UNUSED_VAR(col_map);
   ...
   for (iter = 0; iter < max_iter; iter++)
   {
      ...
      /* Gather external rows via communication */
      if (overlap_comm_pkg && num_procs > 1)
      {
         HYPRE_Int  num_sends = hypre_ParCSRCommPkgNumSends(overlap_comm_pkg);
         HYPRE_Int  num_recvs = hypre_ParCSRCommPkgNumRecvs(overlap_comm_pkg);
         HYPRE_Int *send_map_starts = hypre_ParCSRCommPkgSendMapStarts(overlap_comm_pkg);
         HYPRE_Int *send_map_elmts = hypre_ParCSRCommPkgSendMapElmts(overlap_comm_pkg);
         HYPRE_Int *recv_vec_starts = hypre_ParCSRCommPkgRecvVecStarts(overlap_comm_pkg);

         HYPRE_Int  num_send_vals = send_map_starts[num_sends];
         HYPRE_Int  num_recv_vals = recv_vec_starts[num_recvs];

         HYPRE_Real *send_buf = hypre_TAlloc(HYPRE_Real, num_send_vals, HYPRE_MEMORY_HOST);
         HYPRE_Real *recv_buf = hypre_TAlloc(HYPRE_Real, num_recv_vals, HYPRE_MEMORY_HOST);

         /* Pack send buffer */
         for (i = 0; i < num_send_vals; i++)
         {
            HYPRE_Int idx = send_map_elmts[i];
            send_buf[i] = vtemp_data[idx];
         }

         /* Communicate */
         comm_handle = hypre_ParCSRCommHandleCreate(1, overlap_comm_pkg, send_buf, recv_buf);
         hypre_ParCSRCommHandleDestroy(comm_handle);

         /* Copy received values to external rows in f_local */
         HYPRE_Int ext_idx = 0;
         for (i = 0; i < num_extended_rows; i++)
         {
            if (!row_is_owned[i])
            {
               f_local_data[i] = recv_buf[ext_idx++];
            }
         }
      ...
      if (hypre_SchwarzOverlapVariantIsRAS(variant))
      {
         ...
         for (i = 0; i < num_extended_rows; i++)
         {
            if (row_is_owned[i])
            {
               ...
               if (col_idx >= 0 && col_idx < num_cols_local)
               {
      ...
      else /* HYPRE_SCHWARZ_OVERLAP_VARIANT_AS */
      {
         ...
         HYPRE_Int *as_multiplicity = hypre_TAlloc(HYPRE_Int, num_local_rows,
                                                   HYPRE_MEMORY_HOST);
         ...
         if (overlap_comm_pkg && num_procs > 1)
         {
            ...
            num_send_vals = recv_vec_starts[num_recvs];
            num_recv_vals = send_map_starts[num_sends];

            for (i = 0; i < num_recv_vals; i++)
            {
         ...
         {
            HYPRE_Real *send_buf = hypre_TAlloc(HYPRE_Real, num_send_vals, HYPRE_MEMORY_HOST);
            HYPRE_Real *recv_buf = hypre_CTAlloc(HYPRE_Real, num_recv_vals, HYPRE_MEMORY_HOST);
            HYPRE_Int   ext_idx = 0;
            ...
            for (i = 0; i < num_extended_rows; i++)
            {
               ...
               {
                  HYPRE_Int col_idx = row_to_col_map[i];
                  send_buf[ext_idx++] = (col_idx >= 0 && col_idx < num_cols_local) ?
                                        relax_weight * u_local_data[col_idx] : 0.0;
               }
            }

            comm_handle = hypre_ParCSRCommHandleCreate(2, overlap_comm_pkg, send_buf, recv_buf);
            hypre_ParCSRCommHandleDestroy(comm_handle);

            for (i = 0; i < num_recv_vals; i++)
            {
               HYPRE_Int local_row = send_map_elmts[i];
               u_data[local_row] += recv_buf[i] / (HYPRE_Real) as_multiplicity[local_row];
            }
...
hypre_SchwarzDestroy( void *data )
{
   ...
   /* Destroy overlapping Schwarz data if using new implementation */
   if (HYPRE_SCHWARZ_IS_OVERLAPPING(variant))
   {
      hypre_SchwarzOverlapDataDestroy(schwarz_data);
   }
...
hypre_SchwarzSetLocalSolverType( void *data, HYPRE_Int local_solver_type )
{
   hypre_SchwarzData  *schwarz_data = (hypre_SchwarzData*) data;
   HYPRE_Int           variant;
   HYPRE_Int           ras_variant;

   hypre_SchwarzDataLocalSolverType(schwarz_data) = local_solver_type;

   variant = hypre_SchwarzDataVariant(schwarz_data);
   ras_variant = hypre_SchwarzRASVariantFromLocalSolver(local_solver_type);
   if (variant == HYPRE_SCHWARZ_VARIANT_MP && ras_variant >= 0)
   {
      hypre_SchwarzDataVariant(schwarz_data) = ras_variant;
   }
