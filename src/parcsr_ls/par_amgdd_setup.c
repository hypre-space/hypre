   ...
   HYPRE_Int partition, HYPRE_Int current_level, HYPRE_Int num_levels, HYPRE_Int *padding, HYPRE_Int num_ghost_layers )
{
   ...
   for (level = current_level + 1; level < transition_level; level++)
   {
      ...
      if (nodes_to_add)
      {
         ...
         send_flag[current_level][partition][level] = hypre_CTAlloc( HYPRE_Int, num_send_nodes[current_level][partition][level], HYPRE_MEMORY_SHARED );
         cnt =  0;
   ...
   *recv_buffer_size = 0;
   for (level = current_level; level < num_levels; level++)
   {
      ...
      if (recv_map[level])
      {
         ...
         recv_map[level] = hypre_TReAlloc(recv_map[level], HYPRE_Int, num_recv_nodes[level], HYPRE_MEMORY_SHARED);
      }
