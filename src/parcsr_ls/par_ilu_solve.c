                  ...
                  hypre_ParVector    *u )
{
   ...
   HYPRE_Solver         schur_precond = (ilu_data ->schur_precond);
   hypre_ParVector      *rhs = (ilu_data -> rhs);
   hypre_ParVector      *x = (ilu_data -> x);
 ...
 * S might have different comm than A, be careful with the solve
