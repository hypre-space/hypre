...
HYPRE_Int
hypre_BoomerAMGBuildModMultipass2( hypre_ParCSRMatrix  *A,
                                   HYPRE_Int           *CF_marker,
                                   ...
                                   hypre_ParCSRMatrix **P_ptr )
{
   ...
   hypre_GenerateMultipassPi(A, S, num_cpts_global, &pass_order[pass_starts[1]], pass_marker, 
                             pass_marker_offd, pass_starts[2]-pass_starts[1], 1, &Pi[0]);
   for (i=1; i<num_passes-1; i++)
   {
      ...
      hypre_GenerateMultiPi(A, S, Pi[i-1], c_pts_starts, &pass_order[pass_starts[i+1]], pass_marker, 
                             pass_marker_offd, pass_starts[i+2]-pass_starts[i+1], i+1, &Pi[i]);
   }
...
/* free stuff */
   ...
   for (i=0; i < num_points; i++)
   {
      ...
      {
         row_sums[i] += Pi_diag_data[j];
      }
      for (j=Pi_offd_i[i]; j < Pi_offd_i[i+1]; j++)
      {
         row_sums[i] += Pi_offd_data[j];
      }
      value = row_sums[i]*diagonal;
      row_sums[i] += w_row_sum[i];
      if (value != 0) 
      {
         row_sums[i] /= value;
      }
      for (j = Pi_diag_i[i]; j < Pi_diag_i[i+1]; j++)
      { 
         Pi_diag_data[j] = -Pi_diag_data[j]*row_sums[i];
      }
      for (j = Pi_offd_i[i]; j < Pi_offd_i[i+1]; j++)
      { 
         Pi_offd_data[j] = -Pi_offd_data[j]*row_sums[i];
      }
   }
