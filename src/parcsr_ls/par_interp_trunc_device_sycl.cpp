                                  ...
                                  HYPRE_Real *P_a)
{
   ...
   row_max = warp_allreduce_max(row_max, item) * trunc_factor;
   ...
   for (HYPRE_Int i = p + lane; sycl::ONEAPI::any_of(grp, i < q); i += sub_group_size)
   {
      ...
      cond_prev = i == p + lane || warp_allreduce_min(cond, item);
   ...
   row_scal = row_sum / warp_allreduce_sum(row_scal, item);

   /* 3. scale the row */
   for (HYPRE_Int i = p + lane; i <= last_pos; i += sub_group_size)
   {
      P_a[i] *= row_scal;
   }
}
...
hypre_BoomerAMGInterpTruncationDevice( hypre_ParCSRMatrix *P, HYPRE_Real trunc_factor, HYPRE_Int max_elmts )
{
   ...
   */
   ...
   sycl::range<1> gDim = hypre_GetDefaultSYCLGridDimension(nrows, "warp", bDim);

   HYPRE_SYCL_1D_LAUNCH( hypreSYCLKernel_InterpTruncation, gDim, bDim,
   ...
   if (nnz_diag)
   {
      auto new_end = HYPRE_ONEDPL_CALL(
            ...
            P_j,
            oneapi::dpl::make_zip_iterator(std::make_tuple(tmp_rowid, P_diag_j,  P_diag_a)),
            is_nonnegative<HYPRE_Int>() );

      new_nnz_diag = std::get<0>(new_end.get_iterator_tuple()) - tmp_rowid;
   ...
   if (nnz_offd)
   {
      ...
      auto new_end = HYPRE_ONEDPL_CALL(
            ...
            oneapi::dpl::make_zip_iterator(std::make_tuple(P_i+nnz_P, P_j+nnz_P, P_a+nnz_P)),
            P_j,
            oneapi::dpl::make_zip_iterator(std::make_tuple(tmp_rowid, P_offd_j,  P_offd_a)),
            pred );

      new_nnz_offd = std::get<0>(new_end.get_iterator_tuple()) - tmp_rowid;
