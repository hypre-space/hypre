...
template <HYPRE_Int NUM_GROUPS_PER_BLOCK, HYPRE_Int GROUP_SIZE, HYPRE_Int SHMEM_HASH_SIZE, HYPRE_Int FAILED_SYMBL, char HASHTYPE, bool HAS_GHASH>
__global__ void
...
#pragma unroll
      ...
      group_read<GROUP_SIZE>(ia + i, GROUP_SIZE >= HYPRE_WARP_SIZE || i < M,
                             istart_a, iend_a,
                             GROUP_SIZE >= HYPRE_WARP_SIZE ? warp_lane_id : lane_id);
                                    ...
                                    HYPRE_Int      *nnzC)
{
   ...
   printf("%s[%d]: SHMEM_HASH_SIZE %d, GROUP_SIZE %d, EXACT_ROWNNZ %d, ghash %p size %d\n", __func__, __LINE__,
         SHMEM_HASH_SIZE, GROUP_SIZE, EXACT_ROWNNZ, d_ghash_i, ghash_size);
   ...
   if (EXACT_ROWNNZ)
   {
      if (ghash_size)
      {
         HYPRE_CUDA_LAUNCH ( (hypre_spgemm_numeric<num_groups_per_block, GROUP_SIZE, SHMEM_HASH_SIZE, 0, 'D', true>),
                             gDim, bDim, /* shmem_size, */
                             m, /* k, n, */ d_ia, d_ja, d_a, d_ib, d_jb, d_b, d_ic, d_jc, d_c, NULL,
                             d_ghash_i, d_ghash_j, d_ghash_a );
      }
      else
      {
         HYPRE_CUDA_LAUNCH ( (hypre_spgemm_numeric<num_groups_per_block, GROUP_SIZE, SHMEM_HASH_SIZE, 0, 'D', false>),
                             gDim, bDim, /* shmem_size, */
                             m, /* k, n, */ d_ia, d_ja, d_a, d_ib, d_jb, d_b, d_ic, d_jc, d_c, NULL,
                             d_ghash_i, d_ghash_j, d_ghash_a );
      }
   ...
   else
   {
      if (ghash_size)
      {
         HYPRE_CUDA_LAUNCH ( (hypre_spgemm_numeric<num_groups_per_block, GROUP_SIZE, SHMEM_HASH_SIZE, 1, 'D', true>),
                             gDim, bDim, /* shmem_size, */
      ...
      {
         HYPRE_CUDA_LAUNCH ( (hypre_spgemm_numeric<num_groups_per_block, GROUP_SIZE, SHMEM_HASH_SIZE, 1, 'D', false>),
                             gDim, bDim, /* shmem_size, */
                             m, /* k, n, */ d_ia, d_ja, d_a, d_ib, d_jb, d_b, d_ic, d_jc, d_c, d_rc,
                             d_ghash_i, d_ghash_j, d_ghash_a );
      }
                                                ...
                                                HYPRE_Int      *nnzC )
{
   ...
   hypre_spgemm_numerical_with_rownnz<HYPRE_SPGEMM_NUMER_HASH_SIZE, HYPRE_WARP_SIZE>
