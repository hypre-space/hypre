                      ...
                      T            *d_y)
{
   ...
   HYPRE_Int gridDim_x   = item.get_group_range(0);
   HYPRE_Int blockIdx_x  = item.get_group(0);
   HYPRE_Int threadIdx_x = item.get_local_id(0);
#else
    ...
    *------------------------------------------------------------*/
   const HYPRE_Int grid_ngroups = gridDim_x * (SPMV_BLOCKDIM / K);
   HYPRE_Int grid_group_id = (blockIdx_x * SPMV_BLOCKDIM + threadIdx_x) / K;
   const HYPRE_Int group_lane = threadIdx_x & (K - 1);
   const HYPRE_Int warp_lane = threadIdx_x & (warp_size - 1);
   const HYPRE_Int warp_group_id = warp_lane / K;
   const HYPRE_Int warp_ngroups = warp_size / K;

#ifdef HYPRE_USING_SYCL
   for (; sycl::any_of_group(SG, grid_group_id < n); grid_group_id += grid_ngroups)
#else
...
#pragma unroll 1
#ifdef HYPRE_USING_SYCL
      for (p += group_lane; sycl::any_of_group(SG, p < q); p += K * 2)
#else
...
#pragma unroll 1
#ifdef HYPRE_USING_SYCL
      for (p += group_lane; sycl::any_of_group(SG, p < q); p += K)
#else
