...
HYPRE_Int hypreDevice_CSRSpGemmBinnedGetBlockNumDim()
{
   ...
   /* symbolic */
   hypre_spgemm_symbolic_max_num_blocks < HYPRE_SPGEMM_SYMBL_HASH_SIZE / 16,
                                        HYPRE_SPGEMM_BASE_GROUP_SIZE / 16 >
                                        (multiProcessorCount, &max_nblocks[0][1], &max_nblocks[2][1]);
   hypre_spgemm_symbolic_max_num_blocks < HYPRE_SPGEMM_SYMBL_HASH_SIZE /  8,
                                        HYPRE_SPGEMM_BASE_GROUP_SIZE /  8 >
                                        (multiProcessorCount, &max_nblocks[0][2], &max_nblocks[2][2]);
   hypre_spgemm_symbolic_max_num_blocks < HYPRE_SPGEMM_SYMBL_HASH_SIZE /  4,
                                        HYPRE_SPGEMM_BASE_GROUP_SIZE /  4 >
                                        (multiProcessorCount, &max_nblocks[0][3], &max_nblocks[2][3]);
   hypre_spgemm_symbolic_max_num_blocks < HYPRE_SPGEMM_SYMBL_HASH_SIZE /  2,
   ...
   /* numeric */
   hypre_spgemm_numerical_max_num_blocks < HYPRE_SPGEMM_NUMER_HASH_SIZE / 16,
                                         HYPRE_SPGEMM_BASE_GROUP_SIZE / 16 >
                                         (multiProcessorCount, &max_nblocks[1][1], &max_nblocks[3][1]);
   hypre_spgemm_numerical_max_num_blocks < HYPRE_SPGEMM_NUMER_HASH_SIZE /  8,
                                         HYPRE_SPGEMM_BASE_GROUP_SIZE /  8 >
                                         (multiProcessorCount, &max_nblocks[1][2], &max_nblocks[3][2]);
   hypre_spgemm_numerical_max_num_blocks < HYPRE_SPGEMM_NUMER_HASH_SIZE /  4,
                                         HYPRE_SPGEMM_BASE_GROUP_SIZE /  4 >
                                         (multiProcessorCount, &max_nblocks[1][3], &max_nblocks[3][3]);
   hypre_spgemm_numerical_max_num_blocks < HYPRE_SPGEMM_NUMER_HASH_SIZE /  2,
                                         HYPRE_SPGEMM_BASE_GROUP_SIZE /  2 >
