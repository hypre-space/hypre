...
static __device__ __forceinline__
HYPRE_Int
csr_spmmm_compute_row_symbl(HYPRE_Int   rowi,                HYPRE_Int  lane_id,
      ...
      for (HYPRE_Int k = rowB_start; k < rowB_end; k += blockDim.x)
      {
         if (k + threadIdx.x < rowB_end)
         {
            ...
            for (HYPRE_Int l = rowC_start; l < rowC_end; l++) 
            {
               ...
               //printf("(%i, %i, %i, %i)\n", rowi, rowB, rowC, l_idx);
               /* first try to insert into shared memory hash table */
               HYPRE_Int pos = hash_insert_symbl<HashType>(s_HashSize, s_HashKeys, l_idx, num_new_insert);
               if (-1 == pos)
               {
                  //printf("Insert into global\n");
                  pos = hash_insert_symbl<HashType>(g_HashSize, g_HashKeys, l_idx, num_new_insert);
               }
...
__global__
void csr_spmmm_symbolic(HYPRE_Int  M, /* HYPRE_Int K, HYPRE_Int N, */
                       HYPRE_Int *ia, HYPRE_Int *ja,
                       HYPRE_Int *ib, HYPRE_Int *jb,
                       HYPRE_Int *ic, HYPRE_Int *jc,
                       HYPRE_Int *ig, HYPRE_Int *jg,
                       HYPRE_Int *rd, HYPRE_Int *rf)
{
...
#pragma unrolll
      ...
      j = csr_spmmm_compute_row_symbl<HashType>(i, lane_id, ia, ja, ib, jb, ic, jc,
...
void gpu_csr_spmmm_rownnz_attempt(HYPRE_Int m, HYPRE_Int k, HYPRE_Int r, HYPRE_Int n,
                                 HYPRE_Int *d_ia, HYPRE_Int *d_ja, 
                                 HYPRE_Int *d_ib, HYPRE_Int *d_jb,
                                 HYPRE_Int *d_ic, HYPRE_Int *d_jc,
                                 HYPRE_Int *d_rd, HYPRE_Int *d_rf)
{
#ifdef HYPRE_PROFILE
   ...
   csr_spmm_create_hash_table(m, d_rd, d_act, shmem_hash_size, num_act_warps,
   ...
   if (hash_type == 'L')
   {
      HYPRE_CUDA_LAUNCH( (csr_spmmm_symbolic<num_warps_per_block, shmem_hash_size, ATTEMPT, 'L'>), gDim, bDim,
                         m, /*k, n,*/ d_ia, d_ja, d_ib, d_jb, d_ic, d_jc, d_ghash_i, d_ghash_j, d_rd, d_rf );
   }
   else if (hash_type == 'Q')
   {
      HYPRE_CUDA_LAUNCH( (csr_spmmm_symbolic<num_warps_per_block, shmem_hash_size, ATTEMPT, 'Q'>), gDim, bDim,
                         m, /*k, n,*/ d_ia, d_ja, d_ib, d_jb, d_ic, d_jc, d_ghash_i, d_ghash_j, d_rd, d_rf );
   }
   else if (hash_type == 'D')
   {
      HYPRE_CUDA_LAUNCH( (csr_spmmm_symbolic<num_warps_per_block, shmem_hash_size, ATTEMPT, 'D'>), gDim, bDim,
                         m, /*k, n,*/ d_ia, d_ja, d_ib, d_jb, d_ic, d_jc, d_ghash_i, d_ghash_j, d_rd, d_rf );
   }
   else
   {
...
HYPRE_Int
hypreDevice_CSRSpGemmmRownnzUpperbound(HYPRE_Int m, HYPRE_Int k, HYPRE_Int r, HYPRE_Int n,
                                      HYPRE_Int *d_ia, HYPRE_Int *d_ja,
                                      HYPRE_Int *d_ib, HYPRE_Int *d_jb,
                                      HYPRE_Int *d_ic, HYPRE_Int *d_jc,
                                      HYPRE_Int *d_rd, HYPRE_Int *d_rf)
{
   gpu_csr_spmmm_rownnz_attempt<1> (m, k, r, n, d_ia, d_ja, d_ib, d_jb, d_ic, d_jc, d_rd, d_rf);

   return hypre_error_flag;
}

HYPRE_Int
hypreDevice_CSRSpGemmmRownnz(HYPRE_Int m, HYPRE_Int k, HYPRE_Int r, HYPRE_Int n,
                            HYPRE_Int *d_ia, HYPRE_Int *d_ja,
                            HYPRE_Int *d_ib, HYPRE_Int *d_jb,
                            HYPRE_Int *d_ic, HYPRE_Int *d_jc,
                            HYPRE_Int *d_rd)
{
   ...
   if (rownnz_exact != 0)
   {
      gpu_csr_spmmm_rownnz_attempt<2> (m, k, r, n, d_ia, d_ja, d_ib, d_jb, d_ic, d_jc, d_rd, d_rf);
