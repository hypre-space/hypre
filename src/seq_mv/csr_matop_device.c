...
hypre_GPUMatDataSetCSRData(hypre_CSRMatrix *matrix)
{
   ...
   HYPRE_ONEMKL_CALL( oneapi::mkl::sparse::set_csr_data(*hypre_HandleComputeStream(hypre_handle()),
                                                        ...
                                                        (std::int64_t) hypre_CSRMatrixNumNonzeros(matrix),
...
hypre_CSRMatrix *
hypre_CSRMatrixDeleteZerosDevice( hypre_CSRMatrix *A,
                                  HYPRE_Real       tol )
{
   ...
   hypre_GpuProfilingPushRange("CSRMatrixDeleteZeros");

   /* Count nonzeros with absolute value less than or equal to tol */
   ...
   if (num_zeros)
   {
      ...
      HYPRE_Int        *B_i;
      HYPRE_Int        *B_j;
      HYPRE_Int         num_nonzeros_B = num_nonzeros - num_zeros;

      B = hypre_CSRMatrixCreate(nrows_A, ncols_A, num_nonzeros_B);
      ...
      auto new_end = HYPRE_THRUST_CALL( copy_if,
                                        thrust::make_zip_iterator(thrust::make_tuple(A_data, A_j)),
      ...
      /* Recompute row pointers */
      HYPRE_Int *row_indices = hypre_CTAlloc(HYPRE_Int, num_nonzeros, memory_location);
      ...
      HYPRE_Int *B_row_indices = hypre_CTAlloc(HYPRE_Int, num_nonzeros_B, memory_location);
#if defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_HIP)
                                ...
                                HYPRE_Int    **map_B_to_C_ptr )
{
   ...
   else if (col_map_offd_B && !B_ext_offd_bigj)
   {
      ...
      *map_B_to_C_ptr      = hypre_TAlloc(HYPRE_Int, num_cols_offd_B, HYPRE_MEMORY_DEVICE);
      ...
      hypre_SequenceSycl(*map_B_to_C_ptr, *map_B_to_C_ptr + num_cols_offd_B, 0);
#else
                                     ...
                                     HYPRE_BigInt    **col_map_new_ptr)
{
   ...
   if (num_cols_new < num_cols)
   {
      ...
      hypre_GatherSycl(A_j, A_j + nnz, offd_mark, A_j);

      if (col_map_new_ptr)
      {
         hypre_GatherSycl(tmp_j, tmp_j + num_cols_new, col_map, col_map_new);
      }
#else
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRMoveDiagFirst
 *--------------------------------------------------------------------------*/

__global__ void
hypre_GPUKernelCSRMoveDiagFirst( hypre_DeviceItem    &item,
                                 HYPRE_Int      nrows,
                                 HYPRE_Int     *ia,
                                 HYPRE_Int     *ja,
                                 HYPRE_Complex *aa )
...
hypre_CSRMatrixMoveDiagFirstDevice( hypre_CSRMatrix  *A )
{
   ...
   HYPRE_GPU_LAUNCH(hypre_GPUKernelCSRMoveDiagFirst, gDim, bDim,
...
template<HYPRE_Int type, bool compressed>
__global__ void
hypre_GPUKernelCSRRowSumCF( hypre_DeviceItem            &item,
                            HYPRE_Int                    nrows,
                            HYPRE_Int* __restrict__      ir,
                            HYPRE_Int* __restrict__      ia,
                            HYPRE_Int* __restrict__      ja,
                            HYPRE_Complex* __restrict__  aa,
                            HYPRE_Int* __restrict__      CF_i,
                            HYPRE_Int* __restrict__      CF_j,
                            HYPRE_Complex* __restrict__  row_sum,
                            HYPRE_Complex                scal,
                            HYPRE_Int                    set)
{
   ...
   {
      if (read_only_load(&CF_i[row_i]) != read_only_load(&CF_j[ja[j]]))
      {
         continue;
      }
...
template<HYPRE_Int type, HYPRE_Int compressed>
__global__ void
hypre_GPUKernelCSRRowSum( hypre_DeviceItem            &item,
                          HYPRE_Int                    nrows,
                          HYPRE_Int* __restrict__      ir,
                          HYPRE_Int* __restrict__      ia,
                          HYPRE_Complex* __restrict__  aa,
...
{
   HYPRE_Int      nrows   = hypre_CSRMatrixNumRows(A);
   HYPRE_Int      nrownnz = hypre_CSRMatrixNumRownnz(A);
   HYPRE_Int     *A_r     = hypre_CSRMatrixRownnz(A);
   HYPRE_Int     *A_i     = hypre_CSRMatrixI(A);
   HYPRE_Int     *A_j     = hypre_CSRMatrixJ(A);
   HYPRE_Complex *A_data  = hypre_CSRMatrixData(A);
   ...
   if (nrownnz < nrows && CF_i && CF_j)
   {
      if (type == 0)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<0, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
      else if (type == 1)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<1, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
      else if (type == 2)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<2, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
   }
   else if (CF_i && CF_j)
   {
      if (type == 0)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<0, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
      else if (type == 1)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<1, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
      else if (type == 2)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSumCF<2, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_j, A_data,
                           CF_i, CF_j, row_sum, scal, set );
      }
   }
   else if (nrownnz < nrows && !CF_i && !CF_j)
   {
      if (type == 0)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<0, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_data, row_sum, scal, set );
      }
      else if (type == 1)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<1, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_data, row_sum, scal, set );
      }
      else if (type == 2)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<2, 1>), gDim, bDim,
                           nrownnz, A_r, A_i, A_data, row_sum, scal, set );
      }
   }
   else
   {
      if (type == 0)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<0, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_data, row_sum, scal, set );
      }
      else if (type == 1)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<1, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_data, row_sum, scal, set );
      }
      else if (type == 2)
      {
         HYPRE_GPU_LAUNCH( (hypre_GPUKernelCSRRowSum<2, 0>), gDim, bDim,
                           nrows, A_r, A_i, A_data, row_sum, scal, set );
      }
   }
...
__global__ void
hypre_GPUKernelCSRMatrixComputeColSum(hypre_DeviceItem    &item,
                                      HYPRE_Int            nrows,
                                      const HYPRE_Int     *ia,
                                      const HYPRE_Int     *ja,
                                   ...
                                   HYPRE_Complex     scal)
{
   ...
   hypre_GpuProfilingPushRange("CSRMatrixComputeColSum");

   /* Launch kernel based on type */
   if (type == 0)
   {
      HYPRE_GPU_LAUNCH(hypre_GPUKernelCSRMatrixComputeColSum<0>, gDim, bDim,
                       nrows, A_i, A_j, A_a, scal, col_sum);
   }
   else if (type == 1)
   {
      HYPRE_GPU_LAUNCH(hypre_GPUKernelCSRMatrixComputeColSum<1>, gDim, bDim,
                       nrows, A_i, A_j, A_a, scal, col_sum);
   }
   else if (type == 2)
   {
      HYPRE_GPU_LAUNCH(hypre_GPUKernelCSRMatrixComputeColSum<2>, gDim, bDim,
                       nrows, A_i, A_j, A_a, scal, col_sum);
   }
...
__global__ void
hypre_GPUKernelCSRMatrixTaggedFnormAccum(hypre_DeviceItem    &item,
#if defined (HYPRE_USING_SYCL)
                                         char                *shmem_ptr,
#endif
                                 ...
                                 HYPRE_Real       *tnorms)
{
   ...
   HYPRE_GPU_LAUNCH2(hypre_GPUKernelCSRMatrixTaggedFnormAccum, gDim, bDim, shmem_bytes,
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRMatrixIntersectPattern
 *
 * mark is of size nA
...
__global__ void
hypre_GPUKernelCSRMatrixIntersectPattern(hypre_DeviceItem &item,
                                         HYPRE_Int  n,
                                         HYPRE_Int  nA,
                                         HYPRE_Int *rowid,
                                         HYPRE_Int *colid,
                                ...
                                HYPRE_Int        diag_opt)
{
...
#if defined(HYPRE_USING_SYCL)
   hypre_SequenceSycl(idx, idx + nnzA + nnzB, 0);
   ...
   HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRMatrixIntersectPattern, gDim, bDim,
...
/*--------------------------------------------------------------------------
 * GPU kernel for extracting diagonal entries from a matrix.
 * For available types, see hypre_CSRMatrixExtractDiagonal
 *--------------------------------------------------------------------------*/

template<HYPRE_Int type>
__global__ void
hypre_GPUKernelCSRExtractDiag( hypre_DeviceItem    &item,
                               HYPRE_Int            nrows,
                               HYPRE_Int           *ia,
                               HYPRE_Int           *ja,
                               HYPRE_Complex       *aa,
                                      ...
                                      HYPRE_Int        type)
{
   ...
   {
      case 0:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<0>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 1:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<1>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 2:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<2>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 3:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<3>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 4:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<4>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 10:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<10>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;

      case 11:
         HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRExtractDiag<11>, gDim, bDim, nrows, A_i, A_j, A_a, d );
         break;
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRCheckDiagFirst
 *
...
__global__ void
hypre_GPUKernelCSRCheckDiagFirst( hypre_DeviceItem &item,
                                  HYPRE_Int  nrows,
                                  HYPRE_Int *ia,
                                  HYPRE_Int *ja,
                                  HYPRE_Int *result )
...
hypre_CSRMatrixCheckDiagFirstDevice( hypre_CSRMatrix *A )
{
   ...
   HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRCheckDiagFirst, gDim, bDim,
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRMatrixCheckForMissingDiagonal
 *--------------------------------------------------------------------------*/

__global__ void
hypre_GPUKernelCSRMatrixCheckForMissingDiagonal( hypre_DeviceItem    &item,
                                                 HYPRE_Int      nrows,
                                                 HYPRE_Int     *ia,
                                                 HYPRE_Int     *ja,
                                                 HYPRE_Int     *result )
...
hypre_CSRMatrixCheckForMissingDiagonal( hypre_CSRMatrix *A )
{
   ...
   HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRMatrixCheckForMissingDiagonal, gDim, bDim,
                     num_rows, A_i, A_j, result );
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRMatrixReplaceDiag
 *--------------------------------------------------------------------------*/

__global__ void
hypre_GPUKernelCSRMatrixReplaceDiag( hypre_DeviceItem    &item,
                                  ...
                                  HYPRE_Real       tol )
{
   ...
   HYPRE_Int        *A_j = hypre_CSRMatrixJ(A);
   HYPRE_Complex *A_data = hypre_CSRMatrixData(A);
   HYPRE_GPU_LAUNCH( hypre_GPUKernelCSRMatrixReplaceDiag, gDim, bDim,
                     new_diag, v, num_rows,
                     A_i, A_j, A_data,
                     tol, result );
...
/*--------------------------------------------------------------------------
 * hypre_GPUKernelCSRDiagScale
 *--------------------------------------------------------------------------*/

__global__ void
hypre_GPUKernelCSRDiagScale( hypre_DeviceItem    &item,
                             HYPRE_Int      nrows,
                             HYPRE_Int     *ia,
                             HYPRE_Int     *ja,
                             HYPRE_Complex *aa,
                                ...
                                hypre_Vector    *rd)
{
   ...
   HYPRE_GPU_LAUNCH(hypre_GPUKernelCSRDiagScale, gDim, bDim,
                                       ...
                                       HYPRE_Real      *elmt_tols)
{
   ...
   hypre_CsrRowIndicesToPtrsDevice_v2(nrows, new_nnz, new_ii, hypre_CSRMatrixI(A));
   hypre_CSRMatrixJ(A)        = new_j;
   hypre_CSRMatrixData(A)     = new_data;
...
#if defined(HYPRE_USING_SYCL)
   hypre_SequenceSycl( hypre_CSRMatrixI(A),
                       hypre_CSRMatrixI(A) + n + 1,
                       0  );

   hypre_SequenceSycl( hypre_CSRMatrixJ(A),
...
/*--------------------------------------------------------------------------
 * Functor for checking whether a matrix row is empty or not
 *--------------------------------------------------------------------------*/

#if defined(HYPRE_USING_CUDA) || defined(HYPRE_USING_HIP)
struct is_non_empty_row
{
   const HYPRE_Int *A_i;

   is_non_empty_row(const HYPRE_Int *A_i) : A_i(A_i) {}

   __host__ __device__
   HYPRE_Int operator()(const HYPRE_Int i) const
   {
      return (A_i[i + 1] - A_i[i]) > 0;
   }
};
#endif
...
hypre_CSRMatrixSetRownnzDevice( hypre_CSRMatrix *A )
{
   HYPRE_Int             num_rows  = hypre_CSRMatrixNumRows(A);
   HYPRE_Int            *A_i       = hypre_CSRMatrixI(A);
   HYPRE_Int            *Arownnz   = hypre_CSRMatrixRownnz(A);
   HYPRE_Int             irownnz;
   ...
   hypre_TFree(Arownnz, HYPRE_MEMORY_DEVICE);

   /* Set new rownnz pointer */
   if (irownnz == 0 || irownnz == num_rows)
   {
      hypre_CSRMatrixRownnz(A) = NULL;
   }
   else
   {
      Arownnz = hypre_TAlloc(HYPRE_Int, irownnz, HYPRE_MEMORY_DEVICE);
                                           ...
                                           HYPRE_Complex    *u_data )
{
   ...
   if (uplo == 'L')
   {
      ...
      if (!hypre_CsrsvDataAnalyzedL(csrsv_data))
      {
         ...
         if (hypre_CsrsvDataBufferSize(csrsv_data) < (HYPRE_Int) buffer_size)
         {
            buffer = hypre_TReAlloc_v2(hypre_CsrsvDataBuffer(csrsv_data),
