...
hypre_SeqVectorCreate( HYPRE_Int size )
{
   ...
   hypre_VectorData(vector) = nullptr;
...
#if defined(HYPRE_USING_ONEMKL)
   HYPRE_ONEMKL_CALL( oneapi::mkl::blas::scal(*hypre_HandleSyclComputeQueue(hypre_handle()), size, alpha, y_data, 1) );
#else
...
#if defined(HYPRE_USING_SYCL)
#if defined(HYPRE_USING_ONEMKL)
   HYPRE_ONEMKL_CALL( oneapi::mkl::blas::axpy(*hypre_HandleSyclComputeQueue(hypre_handle()), size, alpha, x_data, 1, y_data, 1) );
#else
...
#if defined(HYPRE_USING_ONEMKL)
   HYPRE_ONEMKL_CALL( oneapi::mkl::blas::dot( hypre_HandleCublasHandle(hypre_handle()), size, x_data, 1, y_data, 1, &result) );
#else
                    ...
                    hypre_Vector *y     )
{
   ...
   HYPRE_ONEDPL_CALL( std::transform,
                      oneapi::dpl::make_transform_iterator(x_data,        alpha * _1),
                      oneapi::dpl::make_transform_iterator(x_data + size, alpha * _1),
                      oneapi::dpl::make_transform_iterator(y_data,        beta  * _1),
                      y_data,
                      oneapi::dpl::maximum<HYPRE_Complex>() );
