...
csr_spmmm_numeric(HYPRE_Int  M, /* HYPRE_Int K, HYPRE_Int N, */
                 ...
                 HYPRE_Int *ib, HYPRE_Int *jb, HYPRE_Complex *ab,
                 HYPRE_Int *ic, HYPRE_Int *jc, HYPRE_Complex *ac,
                 HYPRE_Int *id, HYPRE_Int *jd, HYPRE_Complex *ad,
                 HYPRE_Int *rc,
...
#pragma unrolll
      ...
      j = copy_from_hash_into_C_row<NUM_WARPS_PER_BLOCK, SHMEM_HASH_SIZE>
             (lane_id, warp_s_HashKeys, warp_s_HashVals, ghash_size, jg + istart_g,
              ag + istart_g, jd + istart_c, ad + istart_c);
...
HYPRE_Int
hypreDevice_CSRSpGemmmWithRownnzUpperbound(HYPRE_Int   m,        HYPRE_Int   k,       HYPRE_Int r, HYPRE_Int       n,
                                          ...
                                          HYPRE_Int  *d_rd,     HYPRE_Int   exact_rownnz,
                                          ...
                                          HYPRE_Int *nnzD)
{
   ...
   csr_spmm_create_hash_table(m, d_rd, NULL, shmem_hash_size, num_act_warps,
                              &d_ghash_i, &d_ghash_j, &d_ghash_a, &ghash_size);

   /* ---------------------------------------------------------------------------
    * numerical multiplication:
    * ---------------------------------------------------------------------------*/
   HYPRE_Int *d_id, *d_jd, nnzD_nume, *d_id_new = NULL, *d_jd_new = NULL, nnzD_nume_new = -1;
   HYPRE_Complex *d_d, *d_d_new = NULL;
   ...
   csr_spmm_create_ija(m, d_rd, &d_id, &d_jd, &d_d, &nnzD_nume);

   if (!exact_rownnz)
   {
      d_id_new = hypre_TAlloc(HYPRE_Int, m+1, HYPRE_MEMORY_DEVICE);
   }
   ...
   {
      /* alloc final C */
      csr_spmm_create_ija(m, d_id_new, &d_jd_new, &d_d_new, &nnzD_nume_new);

      hypre_assert(nnzD_nume_new <= nnzD_nume);

      if (nnzD_nume_new < nnzD_nume)
      {
         /* copy to the final C */
         dim3 gDim( (m + bDim.z - 1) / bDim.z );
         HYPRE_CUDA_LAUNCH( (copy_from_Cext_into_C<num_warps_per_block>), gDim, bDim,
                            m, d_id, d_jd, d_d, d_id_new, d_jd_new, d_d_new );

         hypre_TFree(d_id, HYPRE_MEMORY_DEVICE);
         hypre_TFree(d_jd, HYPRE_MEMORY_DEVICE);
         hypre_TFree(d_d,  HYPRE_MEMORY_DEVICE);

         d_id = d_id_new;
         d_jd = d_jd_new;
         d_d = d_d_new;
         nnzD_nume = nnzD_nume_new;
      }
      else
      {
         hypre_TFree(d_id_new, HYPRE_MEMORY_DEVICE);
         hypre_TFree(d_jd_new, HYPRE_MEMORY_DEVICE);
         hypre_TFree(d_d_new,  HYPRE_MEMORY_DEVICE);
      }
   }
