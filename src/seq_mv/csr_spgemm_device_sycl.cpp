...
hypre_CSRMatrixDeviceSpGemmSetRownnzEstimateMethod( HYPRE_Int value )
{
   ...
   {
      hypre_HandleSyclData(hypre_handle())->spgemm_rownnz_estimate_method = value;
   }
   else
   {
      return -1;
...
{
   hypre_HandleSyclData(hypre_handle())->spgemm_rownnz_estimate_nsamples = value;
...
hypre_CSRMatrixDeviceSpGemmSetRownnzEstimateMultFactor( HYPRE_Real value )
{
   if (value > 0.0)
   {
      hypre_HandleSyclData(hypre_handle())->spgemm_rownnz_estimate_mult_factor = value;
   }
   else
   {
      return -1;
...
hypre_CSRMatrixDeviceSpGemmSetHashType( char value )
{
   ...
   {
      hypre_HandleSyclData(hypre_handle())->spgemm_hash_type = value;
   }
   else
   {
      return -1;
...
HYPRE_Int
hypre_CSRMatrixDeviceSpGemmSetUseOnemklsparse( HYPRE_Int use_onemklsparse )
{
   hypre_HandleSyclData(hypre_handle())->spgemm_use_onemklsparse = use_onemklsparse;
   return 0;
}
