/*
 * File:          bHYPRE_HGMRES_jniStub.c
 * Symbol:        bHYPRE.HGMRES-v1.0.0
 * Symbol Type:   class
 * Babel Version: 1.0.4
 * Description:   Client-side JNI glue code for bHYPRE.HGMRES
 * 
...
#include "bHYPRE_HGMRES_jniStub.h"
#endif
...
static struct bHYPRE_HGMRES__object* _get_ior(
  JNIEnv* env,
  jobject obj)
{
  void* ptr = NULL;
  ...
  return (struct bHYPRE_HGMRES__object*) ptr;
}
...
static const struct bHYPRE_HGMRES__external* s_external = NULL;

/*
 * External reference to static EPV.
 */

static const struct bHYPRE_HGMRES__sepv* s_sepv = NULL;
  ...
  jstring url)
{
  ...
  _res = POINTER_TO_JLONG(bHYPRE_HGMRES__remoteCreate(_tmp_url, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject obj)
{
  ...
  struct bHYPRE_HGMRES__object *_ptr = (*s_external->createObject)(g_data, 
    &_ex);
  if(_ex) {
    sidl_Java_CheckException(
      env,
  ...
  jstring url)
{
  ...
  jlong _res = 0;
  char* _tmp_url = sidl_Java_J2I_string(env, url);
  _res = POINTER_TO_JLONG(bHYPRE_HGMRES__remoteConnect(_tmp_url, 1, &_ex));
  if(_ex) {
    sidl_Java_CheckException(
      env,
      _ex,
  ...
  jobject _arg_mpi_comm)
{
  ...
  struct bHYPRE_HGMRES__object* _ior_res = (struct bHYPRE_HGMRES__object*) NULL;
  ...
  _res = sidl_Java_I2J_cls(env, _ior_res, "bHYPRE.HGMRES", FALSE);JAVA_CHECK(
    env);
  ...
  jobject _arg_s)
{
  ...
  struct bHYPRE_HGMRES__object* _ior = NULL;
...
void bHYPRE_HGMRES__register(JNIEnv* env)
{
  JNINativeMethod methods[32];
  jclass cls;

#ifdef SIDL_STATIC_LIBRARY
  s_external = bHYPRE_HGMRES__externals();
#else
  s_external = (struct bHYPRE_HGMRES__external*)sidl_dynamicLoadIOR(
    "bHYPRE.HGMRES","bHYPRE_HGMRES__externals") ;
  sidl_checkIORVersion("bHYPRE.HGMRES", s_external->d_ior_major_version, 
  ...
  methods[2].signature = "(LbHYPRE/HGMRES;)J";
  ...
  methods[7].signature = "(LbHYPRE/MPICommunicator;)LbHYPRE/HGMRES;";
  ...
  cls = (*env)->FindClass(env, "bHYPRE/HGMRES");
  if (cls) {
