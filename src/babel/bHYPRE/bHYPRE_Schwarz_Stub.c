...
#else
  _externals = (struct bHYPRE_Schwarz__external*)sidl_dynamicLoadIOR(
    "bHYPRE.Schwarz","bHYPRE_Schwarz__externals") ;
  sidl_checkIORVersion("bHYPRE.Schwarz", _externals->d_ior_major_version, 
...
static bHYPRE_Schwarz bHYPRE_Schwarz__remoteCreate(const char* url, 
...
{
  return bHYPRE_Schwarz__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_Schwarz__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.Schwarz", (
      void*)bHYPRE_Schwarz__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_Schwarz__set_hooks_static(
  int32_t on,
...
bHYPRE_Schwarz__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_Schwarz__isRemote(self,_ex);
}
