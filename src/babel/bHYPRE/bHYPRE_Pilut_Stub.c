...

{
  ...
  sidl_checkIORVersion("bHYPRE.Pilut", _externals->d_ior_major_version,
...
static bHYPRE_Pilut bHYPRE_Pilut__remoteCreate(const char* url,
...
{
  return bHYPRE_Pilut__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_Pilut__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.Pilut",
      (void*)bHYPRE_Pilut__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_Pilut__set_hooks_static(
  int32_t on,
...
bHYPRE_Pilut__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_Pilut__isRemote(self,_ex);
}
