...
{
  return bHYPRE_Solver__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.Solver",
      (void*)bHYPRE_Solver__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
bHYPRE_Solver__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_Solver__isRemote(self,_ex);
}
