 ...
 * Generated:     20030401 14:48:17 PST
...
static const struct bHYPRE_SStructStencil__external* _getIOR(void)
{
  ...
  if (!_ior) {
    ...
    if (!_ior) {
      fputs("Unable to find the implementation for bHYPRE.SStructStencil; please set SIDL_DLL_PATH\n", stderr);
      exit(-1);
    }
#endif
  }
...
void
SIDLFortran77Symbol(bhypre_sstructstencil_queryint_f,BHYPRE_SSTRUCTSTENCIL_QUERYINT_F,bHYPRE_SStructStencil_queryInt_f)
(
  int64_t *self,
  SIDL_F77_String name
  SIDL_F77_STR_NEAR_LEN_DECL(name),
