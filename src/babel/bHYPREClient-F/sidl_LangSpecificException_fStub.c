 ...
 * Revision:      @(#) $Id: sidl_LangSpecificException_fStub.c,v 1.4 2007/09/27 19:56:34 painter Exp $
...
static struct sidl_LangSpecificException__object* 
  sidl_LangSpecificException__remoteCreate(const char* url, sidl_BaseInterface 
  *_ex);
static struct sidl_LangSpecificException__object* 
  ...
  *_ex)
{
  ...
  sidl_rmi_InstanceHandle instance = sidl_rmi_ProtocolFactory_createInstance(
    url, "sidl.LangSpecificException", _ex ); SIDL_CHECK(*_ex);
  if ( instance == NULL) { return NULL; }
  self =
    (struct sidl_LangSpecificException__object*) malloc(
      sizeof(struct sidl_LangSpecificException__object));
