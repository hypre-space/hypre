...
#else
  _externals = (struct bHYPRE_Euclid__external*)sidl_dynamicLoadIOR(
    "bHYPRE.Euclid","bHYPRE_Euclid__externals") ;
  sidl_checkIORVersion("bHYPRE.Euclid", _externals->d_ior_major_version, 
...
static bHYPRE_Euclid bHYPRE_Euclid__remoteCreate(const char* url, 
...
{
  return bHYPRE_Euclid__remoteCreate(url, _ex);
}
...
{
  return bHYPRE_Euclid__remoteConnect(url, TRUE, _ex);
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("bHYPRE.Euclid", (
      void*)bHYPRE_Euclid__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
void
bHYPRE_Euclid__set_hooks_static(
  int32_t on,
...
bHYPRE_Euclid__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !bHYPRE_Euclid__isRemote(self,_ex);
}
