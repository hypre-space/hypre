...
HYPRE_Int hypre_APSubdivideRegion( hypre_Box *region, HYPRE_Int dim,
                                   HYPRE_Int level, hypre_BoxArray *box_array,
                                        ...
                                        double **p_vol_array)
{
   ...
   /* may need to add some sort of sorting to make this more efficient - though
      we shouldn't have many regions*/
                                 ...
                                 hypre_BoxArray *my_box_array, MPI_Comm comm )
{
   ...
   hypre_MPI_Allreduce(indices, recvbuf, num_regions*6, HYPRE_MPI_INT,
                       hypre_MPI_MIN, comm);
...
HYPRE_Int hypre_APPruneRegions( hypre_BoxArray *region_array,
                                HYPRE_Int **p_count_array, 
                                double **p_vol_array )
{
   /*-------------------------------------------
...
HYPRE_Int hypre_APRefineRegionsByVol( hypre_BoxArray *region_array,
                                      double *vol_array, HYPRE_Int max_regions,
                                      double gamma, HYPRE_Int dim,
                                      HYPRE_Int *return_code, MPI_Comm comm)
{
...
HYPRE_Int hypre_StructAssumedPartitionCreate(
   HYPRE_Int dim, hypre_Box *bounding_box, 
   double global_boxes_size, HYPRE_Int global_num_boxes,
   hypre_BoxArray *local_boxes, HYPRE_Int *local_boxnums,
   HYPRE_Int max_regions, HYPRE_Int max_refinements, 
   double gamma, MPI_Comm comm, hypre_StructAssumedPart **p_assumed_partition)
{
...
hypre_APFillResponseStructAssumedPart(
   void *p_recv_contact_buf, HYPRE_Int contact_size, HYPRE_Int contact_proc,
   void *ro, MPI_Comm comm, void **p_send_response_buf, 
...
HYPRE_Int hypre_StructAssumedPartitionGetRegionsFromProc(
   hypre_StructAssumedPart *assumed_part , HYPRE_Int proc_id,
...
HYPRE_Int hypre_StructAssumedPartitionGetProcsFromBox(
   hypre_StructAssumedPart *assumed_part , 
   hypre_Box *box, 
...
*/ 
   ...
   HYPRE_Int hypre_StructCoarsenAP(hypre_StructAssumedPart *ap,
                                   hypre_Index index, hypre_Index stride,
                                   hypre_StructAssumedPart **new_ap_ptr)
   {
      ...
      /* now coarsen and copy the boxes - here we are going to prune - don't want
         size 0 boxes */
