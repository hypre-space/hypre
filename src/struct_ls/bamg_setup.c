   ...
   for (l = 0; ; l++)
   {
      ...
      if (cdir != -1)
      {
         ...
         if ((periodic) && (periodic % 2))
         {
            bamg_ifdbg
            {
               hypre_printf("  stop coarsening - periodic = %d\n", periodic);
               fflush(stdout);
            }
            cdir = -1;
         }
      ...
      // NB: fail if BoxIMin != (0,0,0,...)! Otherwise, have to change a lot to
      ...
      */
      ...
      bamg_ifdbg
      {
         hypre_printf("2) IMin(cbox): %d %d %d    IMax(cbox): %d %d %d\n",
                      hypre_IndexD(hypre_BoxIMin(cbox),0), hypre_IndexD(hypre_BoxIMin(cbox),1),
                      hypre_IndexD(hypre_BoxIMin(cbox),2),
                      hypre_IndexD(hypre_BoxIMax(cbox),0), hypre_IndexD(hypre_BoxIMax(cbox),1),
                      hypre_IndexD(hypre_BoxIMax(cbox),2));
         fflush(stdout);
      }
