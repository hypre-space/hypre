...
HYPRE_Int
hypre_BAMGSolve( void               *bamg_vdata,
                 ...
                 hypre_StructVector *x         )
{
   ...
   HYPRE_Int             rel_change      = (bamg_data -> rel_change);
   HYPRE_Int             zero_guess      = (bamg_data -> zero_guess);
   HYPRE_Int             num_pre_relax   = (bamg_data -> num_pre_relax);
   HYPRE_Int             num_post_relax  = (bamg_data -> num_post_relax);
   HYPRE_Int             num_levels      = (bamg_data -> num_levels);
   hypre_StructMatrix  **A_l             = (bamg_data -> A_l);
   hypre_StructMatrix  **P_l             = (bamg_data -> P_l);
   hypre_StructMatrix  **RT_l            = (bamg_data -> RT_l);
   hypre_StructVector  **b_l             = (bamg_data -> b_l);
   hypre_StructVector  **x_l             = (bamg_data -> x_l);
   hypre_StructVector  **r_l             = (bamg_data -> r_l);
   hypre_StructVector  **e_l             = (bamg_data -> e_l);
   void                **relax_data_l    = (bamg_data -> relax_data_l);
   void                **matvec_data_l   = (bamg_data -> matvec_data_l);
   void                **restrict_data_l = (bamg_data -> restrict_data_l);
   void                **interp_data_l   = (bamg_data -> interp_data_l);
   HYPRE_Int             logging         = (bamg_data -> logging);
   HYPRE_Real           *norms           = (bamg_data -> norms);
   HYPRE_Real           *rel_norms       = (bamg_data -> rel_norms);
   HYPRE_Int            *active_l        = (bamg_data -> active_l);
   ...
   hypre_BeginTiming(bamg_data -> time_index);
   ...
   if (max_iter == 0)
   {
      ...
      hypre_EndTiming(bamg_data -> time_index);
   ...
   for (i = 0; i < max_iter; i++)
   {
      ...
      hypre_BAMGRelaxSetPreRelax(relax_data_l[0]);
      hypre_BAMGRelaxSetMaxIter(relax_data_l[0], num_pre_relax);
      hypre_BAMGRelaxSetZeroGuess(relax_data_l[0], zero_guess);
      hypre_BAMGRelax(relax_data_l[0], A_l[0], b_l[0], x_l[0]);
      zero_guess = 0;
      ...
      if (num_levels > 1)
      {
         ...
         for (l = 1; l <= (num_levels - 2); l++)
         {
            ...
            {
               /* pre-relaxation */
               hypre_BAMGRelaxSetPreRelax(relax_data_l[l]);
               hypre_BAMGRelaxSetMaxIter(relax_data_l[l], num_pre_relax);
               hypre_BAMGRelaxSetZeroGuess(relax_data_l[l], 1);
               hypre_BAMGRelax(relax_data_l[l], A_l[l], b_l[l], x_l[l]);
         ...
         for (l = (num_levels - 2); l >= 1; l--)
         {
            ...
            {
               /* post-relaxation */
               hypre_BAMGRelaxSetPostRelax(relax_data_l[l]);
               hypre_BAMGRelaxSetMaxIter(relax_data_l[l], num_post_relax);
               hypre_BAMGRelaxSetZeroGuess(relax_data_l[l], 0);
               hypre_BAMGRelax(relax_data_l[l], A_l[l], b_l[l], x_l[l]);
            }
         }
      ...
      hypre_BAMGRelaxSetPostRelax(relax_data_l[0]);
      hypre_BAMGRelaxSetMaxIter(relax_data_l[0], num_post_relax);
      hypre_BAMGRelaxSetZeroGuess(relax_data_l[0], 0);
      hypre_BAMGRelax(relax_data_l[0], A_l[0], b_l[0], x_l[0]);

      (bamg_data -> num_iterations) = (i + 1);
   }
