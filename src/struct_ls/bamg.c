...
void* hypre_BAMGCreate( MPI_Comm comm )
{
  ...
  (bamg_data -> relax_type)        = 1;
  (bamg_data -> jacobi_weight)     = 0.6;
  (bamg_data -> usr_jacobi_weight) = 1;
  (bamg_data -> num_pre_relax)     = 1;
  (bamg_data -> num_post_relax)    = 1;

  (bamg_data -> num_tv1)           = 10;
  (bamg_data -> num_tv2)           = 10;
  (bamg_data -> num_tv_relax)      = 20;
...
HYPRE_Int hypre_BAMGDestroy( void *bamg_vdata )
{
  ...
  if (bamg_data)
  {
    ...
    if ((bamg_data -> num_levels) > -1)
    {
      ...
      for (l = 0; l < (bamg_data -> num_levels); l++)
      {
        hypre_StructVectorDestroy(bamg_data -> tx_l[l]);
        hypre_StructGridDestroy(bamg_data -> grid_l[l]);
        hypre_StructMatrixDestroy(bamg_data -> A_l[l]);
        hypre_StructVectorDestroy(bamg_data -> b_l[l]);
        hypre_StructVectorDestroy(bamg_data -> x_l[l]);
        if ( l > 0 )
        {
          hypre_StructGridDestroy(bamg_data -> P_grid_l[l]);
...
HYPRE_Int hypre_BAMGSetNumTvRelax( void *bamg_vdata, HYPRE_Int num_tv_relax )
{
  ...
  (bamg_data -> num_tv_relax) = num_tv_relax;
...
HYPRE_Int hypre_BAMGGetNumTvRelax( void *bamg_vdata, HYPRE_Int *num_tv_relax )
{
  ...
  *num_tv_relax = (bamg_data -> num_tv_relax);
...
HYPRE_Int hypre_BAMGGetFinalRelativeResidualNorm(
  void       *bamg_vdata,
