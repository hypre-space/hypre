...
void
hypreSYCLKernel_IJMatrixValues_dev1(sycl::nd_item<1>& item, HYPRE_Int n, HYPRE_Int *rowind,
				    HYPRE_Int *row_ptr, HYPRE_Int *row_len, HYPRE_Int *mark)
{
   HYPRE_Int global_thread_id = item.get_global_linear_id();
...
hypre_IJMatrixSetAddValuesParCSRDevice( hypre_IJMatrix       *matrix,
                                        HYPRE_Int             nrows,
                                        HYPRE_Int            *ncols,        /* if nullptr, == all ones */
                                        const HYPRE_BigInt   *rows,
                                        const HYPRE_Int      *row_indexes,  /* if nullptr, == ex_scan of ncols, i.e, no gap */
                                        ...
                                        const char           *action )
{
   ...
   HYPRE_Int  nelms;
   HYPRE_Int *row_ptr = nullptr;

   /* expand rows into full expansion of rows based on ncols
    * if ncols == nullptr, ncols is all ones, so rows are indeed full expansion */
   if (ncols)
   {
   ...
   if (!aux_matrix)
   {
      hypre_AuxParCSRMatrixCreate(&aux_matrix, num_local_rows, num_local_cols, nullptr);
   ...
   HYPRE_ONEDPL_CALL(std::fill_n, stack_sora + stack_elmts_current, nelms, SorA);
   ...
   if (row_indexes)
   {
      ...
      sycl::range<1> gDim = hypre_GetDefaultSYCLGridDimension(len1, "thread", bDim);
...
struct hypre_IJMatrixAssembleFunctor : public thrust::binary_function< std::tuple<T1, T2>, std::tuple<T1, T2>, std::tuple<T1, T2> >
{
                                     ...
                                     HYPRE_Int *N1, HYPRE_BigInt **I1, HYPRE_BigInt **J1, char **X1, HYPRE_Complex **A1 )
{
   ...
   HYPRE_CUDA_LAUNCH( hypreSYCLKernel_IJMatrixAssembleSortAndReduce1, gDim, bDim, N0, I0, J0, X0, A0 );
   */
...
template<typename T1, typename T2>
struct hypre_IJMatrixAssembleFunctor2 : public thrust::binary_function< std::tuple<T1, T2>, std::tuple<T1, T2>, std::tuple<T1, T2> >
{
   typedef std::tuple<T1, T2> Tuple;
...
hypre_IJMatrixAssembleSortAndRemove(HYPRE_Int N0, HYPRE_BigInt *I0, HYPRE_BigInt *J0, char *X0, HYPRE_Complex *A0)
{
   ...
   HYPRE_Int N1 = std::get<0>(new_end.get_iterator_tuple()) - I0;
...
hypre_IJMatrixAssembleParCSRDevice(hypre_IJMatrix *matrix)
{
   ...
   if (nelms)
   {
      ...
      HYPRE_Int     *diag_i_new = nullptr;
      HYPRE_Int     *diag_j_new = nullptr;
      HYPRE_Complex *diag_a_new = nullptr;
      char          *diag_sora_new = nullptr;
      HYPRE_Int      offd_nnz_new;
      HYPRE_Int     *offd_i_new = nullptr;
      HYPRE_Int     *offd_j_new = nullptr;
      HYPRE_Complex *offd_a_new = nullptr;
      char          *offd_sora_new = nullptr;
      ...
      hypre_CSRMatrixSplitDevice_core( 1,
                                       ...
                                       new_j,
                                       new_data,
                                       diag_nnz_existed || offd_nnz_existed ? new_sora : nullptr,
                                       col_start,
                                       col_end - 1,
                                       hypre_CSRMatrixNumCols(hypre_ParCSRMatrixOffd(par_matrix)),
                                       ...
                                       diag_nnz_existed ? diag_sora_new + diag_nnz_existed : nullptr,
                                       &offd_nnz_new,
                                       offd_i_new + offd_nnz_existed,
                                       offd_j_new + offd_nnz_existed,
                                       offd_a_new + offd_nnz_existed,
                                       offd_nnz_existed ? offd_sora_new + offd_nnz_existed : nullptr );
      ...
      if (diag_nnz_new > 0)
      {
         if (diag_nnz_existed)
         {
            ...
            HYPRE_ONEDPL_CALL(std::fill_n, diag_sora_new, diag_nnz_existed, 0);
      ...
      if (offd_nnz_new > 0)
      {
         if (offd_nnz_existed)
         {
            ...
            HYPRE_ONEDPL_CALL(std::fill_n, offd_sora_new, offd_nnz_existed, 0);
   ...
   hypre_IJMatrixTranslator(matrix) = nullptr;
                                             ...
                                             HYPRE_Complex   value )
{
   ...
   HYPRE_ONEDPL_CALL( std::fill_n, diag_data, nnz_diag, value );
   HYPRE_ONEDPL_CALL( std::fill_n, offd_data, nnz_offd, value );
