...
HYPRE_Int
hypre_IJVectorAssembleSortAndReduce2( HYPRE_Int       N0,
                                      HYPRE_BigInt   *I0,
												  HYPRE_Int      *C0,
                                      char           *X0,
                                      HYPRE_Complex  *A0,
                                      ...
                                      HYPRE_Complex **A1 )
{
...
#else
   HYPRE_THRUST_CALL( stable_sort_by_key,
							 thrust::make_zip_iterator(thrust::make_tuple(C0,    I0)),  /* first */
							 thrust::make_zip_iterator(thrust::make_tuple(C0+N0, I0+N0)),  /* last*/
                      thrust::make_zip_iterator(thrust::make_tuple(X0, A0)) );
#endif

   HYPRE_Int     *C = hypre_TAlloc(HYPRE_Int,     N0, HYPRE_MEMORY_DEVICE);
   HYPRE_BigInt  *I = hypre_TAlloc(HYPRE_BigInt,  N0, HYPRE_MEMORY_DEVICE);
   char          *X = hypre_TAlloc(char,          N0, HYPRE_MEMORY_DEVICE);
   HYPRE_Complex *A = hypre_TAlloc(HYPRE_Complex, N0, HYPRE_MEMORY_DEVICE);
   ...
   HYPRE_THRUST_CALL(
      exclusive_scan_by_key,
      make_reverse_iterator(thrust::make_zip_iterator(thrust::device_pointer_cast<HYPRE_Int>(C0) + N0,
																		thrust::device_pointer_cast<HYPRE_BigInt>(I0) + N0)), /* key begin */
      make_reverse_iterator(thrust::make_zip_iterator(thrust::device_pointer_cast<HYPRE_Int>(C0),
																		thrust::device_pointer_cast<HYPRE_BigInt>(I0))), /* key begin */
      make_reverse_iterator(thrust::device_pointer_cast<char>(X0) + N0),         /* input value begin */
      make_reverse_iterator(thrust::device_pointer_cast<char>(X) + N0),          /* output value begin */
      char(0),                                                                   /* init */
		thrust::equal_to< thrust::tuple<HYPRE_Int, HYPRE_BigInt> >(),
      thrust::maximum<char>() );
   ...
   auto new_end = HYPRE_THRUST_CALL(
                     reduce_by_key,
							thrust::make_zip_iterator(thrust::make_tuple(C0,    I0)),      /* keys_first */
							thrust::make_zip_iterator(thrust::make_tuple(C0+N0, I0+N0)),   /* keys_last */
                     thrust::make_zip_iterator(thrust::make_tuple(X0,    A0     )), /* values_first */
                     thrust::make_zip_iterator(thrust::make_tuple(C,     I)),       /* keys_output */
                     thrust::make_zip_iterator(thrust::make_tuple(X,     A      )), /* values_output */
							thrust::equal_to< thrust::tuple<HYPRE_Int, HYPRE_BigInt> >(),   /* binary_pred */
                     hypre_IJVectorAssembleFunctor<char, HYPRE_Complex>()           /* binary_op */);
#endif

   *N1 = thrust::get<0>(new_end.first.get_iterator_tuple()) - C;
	*C1 = C;
   *I1 = I;
   *X1 = X;
   *A1 = A;
...
/*--------------------------------------------------------------------
 * hypre_IJVectorAssembleSortAndReduce4
 *--------------------------------------------------------------------*/

HYPRE_Int
hypre_IJVectorAssembleSortAndReduce4( HYPRE_Int      N0,
                                      HYPRE_BigInt  *I0,
												  HYPRE_Int     *C0,
                                      char          *X0,
                                      HYPRE_Complex *A0,
                                      HYPRE_Int     *N1 )
{
...
#else
   HYPRE_THRUST_CALL( stable_sort_by_key,
							 thrust::make_zip_iterator(thrust::make_tuple(C0, I0)),
							 thrust::make_zip_iterator(thrust::make_tuple(C0+N0, I0+N0)),
                      thrust::make_zip_iterator(thrust::make_tuple(X0, A0)) );
#endif

   HYPRE_BigInt  *I = hypre_TAlloc(HYPRE_BigInt,  N0, HYPRE_MEMORY_DEVICE);
   HYPRE_BigInt  *C = hypre_TAlloc(HYPRE_Int,  N0, HYPRE_MEMORY_DEVICE);
   HYPRE_Complex *A = hypre_TAlloc(HYPRE_Complex, N0, HYPRE_MEMORY_DEVICE);
   ...
   auto new_end = HYPRE_THRUST_CALL(
                     reduce_by_key,
							thrust::make_zip_iterator(thrust::device_pointer_cast<HYPRE_Int>(C0),
															  thrust::device_pointer_cast<HYPRE_BigInt>(I0)), /* key begin */
							thrust::make_zip_iterator(thrust::device_pointer_cast<HYPRE_Int>(C0)+N0,
															  thrust::device_pointer_cast<HYPRE_BigInt>(I0)+N0), /* key end */
                     A0,      /* values_first */
							thrust::make_zip_iterator(thrust::device_pointer_cast<HYPRE_Int>(C),
															  thrust::device_pointer_cast<HYPRE_BigInt>(I)), /* key begin */
                     A        /* values_output */);
#endif
...
#else
   auto new_end2 = HYPRE_THRUST_CALL( copy_if,
                                      thrust::make_zip_iterator(thrust::make_tuple(I, C, A)),
                                      thrust::make_zip_iterator(thrust::make_tuple(I, C, A)) + Nt,
                                      A,
                                      thrust::make_zip_iterator(thrust::make_tuple(I0, C0, A0)),
                                      thrust::identity<HYPRE_Complex>() );
   ...
   *N1 = thrust::get<0>(new_end2.get_iterator_tuple()) - I0;
   ...
   hypre_TFree(I, HYPRE_MEMORY_DEVICE);
   hypre_TFree(C, HYPRE_MEMORY_DEVICE);
   hypre_TFree(A, HYPRE_MEMORY_DEVICE);
...
/*--------------------------------------------------------------------
 * hypreGPUKernel_IJVectorAssembleParMultiVec
 *
 * y[map[i]-offset] = x[i] or y[map[i]] += x[i] depending on SorA,
 * same index cannot appear more than once in map
 *--------------------------------------------------------------------*/
...
__global__ void
hypreGPUKernel_IJVectorAssembleParMultiVec( hypre_DeviceItem &item,
														  HYPRE_Int         n,
														  HYPRE_Complex    *x,
														  HYPRE_BigInt     *map,
														  HYPRE_Int        *component,
														  ...
														  HYPRE_Complex    *y )
{
   ...
   if (SorA[i])
   {
      y[map[i] - offset  + component[i]*vecstride] = x[i];
   }
   else
   {
      y[map[i] - offset  + component[i]*vecstride] += x[i];
                                    ...
                                    const char           *action)
{
   ...
   HYPRE_Int     *stack_comp           = hypre_AuxParVectorStackComp(aux_vector);
   ...
   if (stack_elmts_max < stack_elmts_required)
   {
      ...
      if (num_vectors > 1)
      {
         stack_comp = hypre_TReAlloc_v2(stack_comp, HYPRE_Int, stack_elmts_max, HYPRE_Int,
                                        stack_elmts_max_new, HYPRE_MEMORY_DEVICE);
      }

      hypre_AuxParVectorStackI(aux_vector)        = stack_i;
      hypre_AuxParVectorStackComp(aux_vector)     = stack_comp;
   ...
   if (num_vectors > 1)
   {
      hypreDevice_IntFilln(stack_comp + stack_elmts_current, num_values,
									(HYPRE_Int) component);
   }
...
HYPRE_Int
hypre_IJVectorSetAddValuesParMultiVecDevice(hypre_IJVector       *vector,
														  HYPRE_Int             num_values,
														  const HYPRE_BigInt   *indices,
														  const HYPRE_Int      *components,
														  const HYPRE_Complex  *values,
														  const char           *action)
{
   HYPRE_BigInt    *IJpartitioning = hypre_IJVectorPartitioning(vector);
   ...
   hypreDevice_CharFilln(stack_sora + stack_elmts_current, num_values, SorA);
	...
	hypre_TMemcpy(stack_comp + stack_elmts_current, components, HYPRE_Int,  num_values,
					  HYPRE_MEMORY_DEVICE, HYPRE_MEMORY_DEVICE);
	hypre_TMemcpy(stack_i    + stack_elmts_current, indices, HYPRE_BigInt,  num_values,
                 HYPRE_MEMORY_DEVICE, HYPRE_MEMORY_DEVICE);
   hypre_TMemcpy(stack_data + stack_elmts_current, values,  HYPRE_Complex, num_values,
...
hypre_IJVectorAssembleParDevice(hypre_IJVector *vector)
{
   ...
   if (nelms_off_max)
   {
      ...
      if (nelms_off)
      {
         ...
         char *is_on_proc    = hypre_TAlloc(char,          nelms,     HYPRE_MEMORY_DEVICE);
			if (num_vectors > 1)
			{
				off_proc_comp    = hypre_TAlloc(HYPRE_Int,  nelms_off, HYPRE_MEMORY_DEVICE);
			}
         ...
         HYPRE_THRUST_CALL(transform, stack_i, stack_i + nelms, is_on_proc, pred);
			...
			if (num_vectors > 1)
			{
				auto new_end1 = HYPRE_THRUST_CALL(
					copy_if,
					thrust::make_zip_iterator(thrust::make_tuple(stack_i,
																				stack_comp,
																				stack_data,
																				stack_sora        )),  /* first */
					thrust::make_zip_iterator(thrust::make_tuple(stack_i + nelms,
																				stack_comp + nelms,
																				stack_data + nelms,
																				stack_sora + nelms)),  /* last */
					is_on_proc,                                                         /* stencil */
					thrust::make_zip_iterator(thrust::make_tuple(off_proc_i,
																				off_proc_comp,
																				off_proc_data,
         ...
         hypre_TFree(is_on_proc, HYPRE_MEMORY_DEVICE);
			...
			{
				/* sort and reduce */
				hypre_IJVectorAssembleSortAndReduce4(nelms_off, off_proc_i, off_proc_comp, off_proc_sora, off_proc_data, &new_nnz);
			}
			else
			{
				/* sort and reduce */
		...
		if (num_vectors > 1)
		{
			...
			hypre_IJVectorAssembleOffProcValsParMultiVec(vector, -1, new_nnz, HYPRE_MEMORY_DEVICE,
																		off_proc_i, off_proc_comp, off_proc_data);
			hypre_TFree(off_proc_comp,    HYPRE_MEMORY_DEVICE);
   ...
   stack_comp = hypre_AuxParVectorStackComp(aux_vector);
   ...
   if (nelms)
   {
      ...
      if (num_vectors > 1)
      {
			...
			{
				HYPRE_Int rank;
				hypre_MPI_Comm_rank(comm, &rank);

				char fname[50];
				sprintf(fname,"presort_component_and_index_%d.txt",rank);
				FILE * fid = fopen(fname,"wt");

				HYPRE_Int * h_comp = hypre_TAlloc(HYPRE_Int, nelms, HYPRE_MEMORY_HOST);
				hypre_TMemcpy(h_comp, stack_comp, HYPRE_Int, nelms, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);

				HYPRE_BigInt * h_i = hypre_TAlloc(HYPRE_BigInt, nelms, HYPRE_MEMORY_HOST);
				hypre_TMemcpy(h_i, stack_i, HYPRE_BigInt, nelms, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);

				for (HYPRE_Int i=0; i<nelms; ++i) fprintf(fid, "%d, %d, %lld\n",i,h_comp[i],h_i[i]);
				fclose(fid);
				hypre_TFree(h_comp, HYPRE_MEMORY_HOST);
				hypre_TFree(h_i, HYPRE_MEMORY_HOST);
			}
#endif
			/* sort and reduce */
			hypre_IJVectorAssembleSortAndReduce2(nelms, stack_i, stack_comp, stack_sora, stack_data,
															 &new_nnz, &new_i, &new_comp, &new_sora, &new_data);
#if 0
			{
				HYPRE_Int rank;
				hypre_MPI_Comm_rank(comm, &rank);
				...
				char fname[50];
				sprintf(fname,"postsort_component_and_index_%d.txt",rank);
				FILE * fid = fopen(fname,"wt");

				HYPRE_Int * h_comp = hypre_TAlloc(HYPRE_Int, new_nnz, HYPRE_MEMORY_HOST);
				hypre_TMemcpy(h_comp, new_comp, HYPRE_Int, new_nnz, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);

				HYPRE_BigInt * h_i = hypre_TAlloc(HYPRE_BigInt, new_nnz, HYPRE_MEMORY_HOST);
				hypre_TMemcpy(h_i, new_i, HYPRE_BigInt, new_nnz, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE);

				for (HYPRE_Int i=0; i<new_nnz; ++i) fprintf(fid, "%d, %d, %lld\n",i,h_comp[i],h_i[i]);
				fclose(fid);
				hypre_TFree(h_comp, HYPRE_MEMORY_HOST);
				hypre_TFree(h_i, HYPRE_MEMORY_HOST);
			}
			...
			HYPRE_GPU_LAUNCH( hypreGPUKernel_IJVectorAssembleParMultiVec, gDim, bDim,
									new_nnz, new_data, new_i, new_comp,
									vec_start, new_sora, vecstride,
									data );
			hypre_TFree(new_comp, HYPRE_MEMORY_DEVICE);
      }
		else
		{
			/* sort and reduce */
