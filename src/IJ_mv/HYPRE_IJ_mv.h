...
/**
 * Sets the number of components (vectors) of a multivector. A vector is assumed
 * to have a single component when this function is not called. This function must
 * be called prior to HYPRE_IJVectorInitialize.
 **/
HYPRE_Int HYPRE_IJVectorSetNumComponents(HYPRE_IJVector  vector,
                                         HYPRE_Int       num_components);

/**
 * Set the component identifier of a vector with multiple components (multivector).
