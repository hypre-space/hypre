...
/* additive versions */
   ...
   /* interpolation */
   HYPRE_Int      interp_type  = 2; /* default value */
   ...
   HYPRE_Int blk_size  = 4;
   HYPRE_Int num_wells = 0;
   HYPRE_Int relax_method = 0;
   HYPRE_Int splitting_strategy = 1;
   HYPRE_Int number_of_levels = 4;
   HYPRE_Int num_add_coarse_idx = 4;
   HYPRE_Int *additional_coarse_indices = hypre_CTAlloc(HYPRE_Int,num_add_coarse_idx);
   additional_coarse_indices[0] = 4;
   additional_coarse_indices[1] = 8;
   additional_coarse_indices[2] = 84;
   additional_coarse_indices[3] = 88;
  
   HYPRE_Int num_coarse_points[number_of_levels];
   num_coarse_points[0] = 3;
   num_coarse_points[1] = 2;
   num_coarse_points[2] = 1;
   num_coarse_points[3] = 1;

   //HYPRE_Int *coarse_index = hypre_CTAlloc(HYPRE_Int,2);
   HYPRE_Int first_lev_idx[3];
   first_lev_idx[0] = 0;
   first_lev_idx[1] = 1;
   first_lev_idx[2] = 2;
   HYPRE_Int second_lev_idx[2];
   second_lev_idx[0] = 0;
   second_lev_idx[1] = 1;
   HYPRE_Int third_lev_idx = 0;
   HYPRE_Int *coarse_index[number_of_levels];
   //coarse_index[0] = &second_lev_idx;
   coarse_index[0] = &first_lev_idx;
   coarse_index[1] = &second_lev_idx;
   coarse_index[2] = &third_lev_idx;
   coarse_index[3] = &third_lev_idx;
...
/* note: log is written to standard output, not to file */
	 ...
	 HYPRE_BoomerAMGSetDofFunc(amg_solver, dof_func);
	  ...
	  HYPRE_BoomerAMGSetTol(amg_solver, 1.0e-12);
	  ...
	  //HYPRE_SysTGSetBlockData( systg_solver, blk_size, 1, &coarse_index);
     HYPRE_SysTGSetMaxCoarseLevels(systg_solver, number_of_levels);
     HYPRE_SysTGSetBlockDataExp(systg_solver, blk_size, &num_coarse_points, &(coarse_index));
	  
	  HYPRE_SysTGSetNumWells(systg_solver,num_wells);
	  
	  HYPRE_SysTGSetCoarseSolver( systg_solver, HYPRE_BoomerAMGSolve, HYPRE_BoomerAMGSetup, amg_solver);
     HYPRE_SysTGSetConvTol(systg_solver, 1.0e-12);
     HYPRE_SysTGSetRelaxMethod(systg_solver, relax_method);
     HYPRE_SysTGSetSplittingStrategy(systg_solver, splitting_strategy);
	  HYPRE_SysTGSetRelaxType(systg_solver, 3);
     ...
     HYPRE_SysTGSetMaxIters(systg_solver, 20);
     HYPRE_SysTGSetAdditionalCoarseIndices(systg_solver, num_add_coarse_idx, &(additional_coarse_indices[0]));
...
///*         
		 ...
		 HYPRE_BoomerAMGSetPrintLevel(aux_solver, 3);
//*/         
	 //HYPRE_Int coarse_index = 0;
	 //HYPRE_SysTGSetBlockData( pcg_precond, blk_size, 1, &coarse_index);
    HYPRE_SysTGSetMaxCoarseLevels(pcg_precond, number_of_levels);
    HYPRE_SysTGSetBlockDataExp(pcg_precond, blk_size, &num_coarse_points, &coarse_index);
    ...
    HYPRE_SysTGSetSplittingStrategy(pcg_precond, splitting_strategy);
    HYPRE_SysTGSetRelaxMethod(pcg_precond, relax_method);
     ...
     HYPRE_SysTGSetAdditionalCoarseIndices(pcg_precond, num_add_coarse_idx, &(additional_coarse_indices[0]));
     HYPRE_SysTGSetPrintLevel(pcg_precond, 0);
         HYPRE_GMRESSetMaxIter(pcg_solver, mg_max_iter);         
	  ...
	  else if (solver_id == 62)
	  {
        ...
        HYPRE_SysTGSetRelaxMethod(pcg_precond, relax_method);
	     ...
	     HYPRE_SysTGSetAdditionalCoarseIndices(pcg_precond, num_add_coarse_idx, &(additional_coarse_indices[0])); 
		  ...
		  HYPRE_FlexGMRESSetMaxIter(pcg_solver, mg_max_iter);
        HYPRE_FlexGMRESSetTol(pcg_solver, 1.0e-12);
