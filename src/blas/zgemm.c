...
/* Subroutine */ HYPRE_Int zgemm_(char *transa, char *transb, integer *m, integer *
...
/*     Test the input parameters. */
    info = 0;
    if (! nota && ! conja && ! hypre_lsame_(transa, "T")) {
	info = 1;
    } else if (! notb && ! conjb && ! hypre_lsame_(transb, "T")) {
	info = 2;
    } else if (*m < 0) {
	info = 3;
    } else if (*n < 0) {
