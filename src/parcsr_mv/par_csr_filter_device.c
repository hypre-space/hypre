                                          ...
                                          HYPRE_Int         *B_offd_i)
{
   ...
   if (row < num_rows)
   {
      ...
      HYPRE_Int dof_idx = row % block_size;
           ...
           j += HYPRE_WARP_SIZE)
      {
         if (j < qA)
         {
            ...
            if ((HYPRE_Int) (global_col % big_block_size) == dof_idx)
            {
               offd_count++;
            }
         }
                                         ...
                                         HYPRE_Complex    *B_offd_a)
{
   ...
   const HYPRE_Int  dof_idx = (row % block_size);
   HYPRE_Int        p = 0, q = 0, pA, qA;
   HYPRE_BigInt     big_block_size = (HYPRE_BigInt) block_size;
        ...
        j += HYPRE_WARP_SIZE)
   {
      ...
      HYPRE_Int       write   = (j < pA && (col % block_size) == dof_idx);
   ...
   if (B_offd_nnz)
   {
           ...
           j += HYPRE_WARP_SIZE)
      {
         ...
         HYPRE_Int          write      = (j < qA) &&
                                         (HYPRE_Int) (global_col % big_block_size) == dof_idx;
         hypre_mask         ballot     = hypre_ballot_sync(item, HYPRE_WARP_FULL_MASK, write);
         HYPRE_Int          laneoff    = hypre_popc(ballot & ((hypre_mask_one << lane) - 1));
                                  ...
                                  hypre_ParCSRMatrix **B_ptr)
{
   ...
   HYPRE_Int            A_num_cols_offd = hypre_CSRMatrixNumCols(A_offd);
   ...
   HYPRE_Int            B_num_cols_offd;
   HYPRE_BigInt        *B_col_map_offd;
   HYPRE_BigInt        *tmp_bj, *tmp_bj_end;
   ...
   B = hypre_ParCSRMatrixCreate(comm, global_num_rows, global_num_cols,
                                row_starts, col_starts, A_num_cols_offd,
                                0, 0);
   ...
   B_offd_bj = hypre_TAlloc(HYPRE_BigInt,  B_offd_nnz, memory_location);
   ...
   HYPRE_GPU_LAUNCH( hypreGPUKernel_ParCSRMatrixBlkFilterFill, gDim, bDim,
                     ...
                     A_col_map_offd, B_offd_nnz,
                     B_diag_i, B_diag_j, B_diag_a,
                     B_offd_i, B_offd_bj, B_offd_a);

   /* Set up B's col_map_offd and B_offd_j */
   if (B_offd_nnz > 0)
   {
      /* Create temp array for sorting global column indices */
      tmp_bj = hypre_TAlloc(HYPRE_BigInt, B_offd_nnz, memory_location);
      hypre_TMemcpy(tmp_bj, B_offd_bj, HYPRE_BigInt, B_offd_nnz,
                    memory_location, memory_location);

      /* Sort and unique the temp array to get column map */
#if defined(HYPRE_USING_SYCL)
      HYPRE_ONEDPL_CALL(std::sort, tmp_bj, tmp_bj + B_offd_nnz);
      tmp_bj_end = HYPRE_ONEDPL_CALL(std::unique, tmp_bj, tmp_bj + B_offd_nnz);
#else
      HYPRE_THRUST_CALL(sort, tmp_bj, tmp_bj + B_offd_nnz);
      tmp_bj_end = HYPRE_THRUST_CALL(unique, tmp_bj, tmp_bj + B_offd_nnz);
#endif
      hypre_CSRMatrixNumCols(B_offd) = B_num_cols_offd = tmp_bj_end - tmp_bj;

      /* Create B's device column map */
      hypre_ParCSRMatrixDeviceColMapOffd(B) = hypre_TAlloc(HYPRE_BigInt,
                                                           B_num_cols_offd,
                                                           HYPRE_MEMORY_DEVICE);
      B_col_map_offd = hypre_ParCSRMatrixDeviceColMapOffd(B);

      /* Copy unique global indices to column map */
      hypre_TMemcpy(B_col_map_offd, tmp_bj, HYPRE_BigInt, B_num_cols_offd,
                    HYPRE_MEMORY_DEVICE, HYPRE_MEMORY_DEVICE);

      /* Convert original global indices to local compressed column indices */
#if defined(HYPRE_USING_SYCL)
      HYPRE_ONEDPL_CALL( oneapi::dpl::lower_bound,
                         B_col_map_offd,
                         B_col_map_offd + B_num_cols_offd,
                         B_offd_bj,
                         B_offd_bj + B_offd_nnz,
                         B_offd_j);
#else
      ...
      /* Free temp array */
      hypre_TFree(tmp_bj, memory_location);
   }
   ...
   *B_ptr = B;
   ...
   hypre_TFree(B_offd_bj, memory_location);
