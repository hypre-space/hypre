                          ...
                          hypre_ParCSRCommPkg *comm_pkg)
{
   ...
   j = num_procs + 1;
#endif
   HYPRE_Int *part = hypre_TAlloc(HYPRE_Int, j);
   ...
   /* send and recv b */
   HYPRE_Complex *send_b = hypre_TAlloc(HYPRE_Complex, num_rows_send);
   HYPRE_Complex *recv_b = hypre_TAlloc(HYPRE_Complex, num_rows_recv);
                          ...
                          hypre_ParCSRCommPkg **commpkg_out )
{
   ...
   A_diag_i_new = hypre_CTAlloc(HYPRE_Int,     nrow_local + 1);
   A_diag_j_new = hypre_CTAlloc(HYPRE_Int,     nnz_diag_alloc);
   A_diag_a_new = hypre_CTAlloc(HYPRE_Complex, nnz_diag_alloc);
   A_offd_i_new = hypre_CTAlloc(HYPRE_Int,     nrow_local + 1);
   A_offd_j_new = hypre_CTAlloc(HYPRE_Int,     nnz_offd_alloc);
   A_offd_a_new = hypre_CTAlloc(HYPRE_Complex, nnz_offd_alloc);
   ...
   HYPRE_Int *ext_offd;
   ext_indices = hypre_CTAlloc(HYPRE_Int, num_ext_rows);
   j = 0;
   ...
   col_map_offd_A_new = hypre_CTAlloc(HYPRE_Int, num_cols_A_offd + k);
   if (k)
   {
      /* map offd to offd_new */
      offd2new = hypre_CTAlloc(HYPRE_Int, num_cols_A_offd);
   }
   hypre_union2(num_cols_A_offd, col_map_offd_A, k, ext_offd, 
                &num_cols_A_offd_new, col_map_offd_A_new, offd2new, NULL);
   hypre_TFree(ext_offd);
   /* 
   ...
   /* marker for diag */
   marker_diag = hypre_TAlloc(HYPRE_Int, ncol_local);
   for (i = 0; i < ncol_local; i++)
   {
      marker_diag[i] = -1;
   }
   /* marker for newoffd */
   marker_newoffd = hypre_TAlloc(HYPRE_Int, num_cols_A_offd_new);
   ...
   for (block_start = first_row_block; block_start < end_row_block; block_start += blockSize)
   {
      ...
      if (lapack_info == 0)
      {
         ...
         if (lwork_opt > dgetri_lwork)
         {
            ...
            dgetri_work = hypre_TReAlloc(dgetri_work, HYPRE_Complex, dgetri_lwork);
         }
      ...
      for (i = block_start; i < block_end; i++)
      {
         ...
         if (square2)
         {
            ...
            if (nnz_diag_new == nnz_diag_alloc)
            {
               ...
               A_diag_j_new = hypre_TReAlloc(A_diag_j_new, HYPRE_Int,     nnz_diag_alloc);
               A_diag_a_new = hypre_TReAlloc(A_diag_a_new, HYPRE_Complex, nnz_diag_alloc);
            }
               ...
               for (ii = A_offd_i[rid]; ii < A_offd_i[rid+1]; ii++)
               {
                  ...
                  if (marker_newoffd[col_new] < offd_i_start)
                  {
                     ...
                     if (nnz_offd_new == nnz_offd_alloc)
                     {
                        ...
                        A_offd_j_new = hypre_TReAlloc(A_offd_j_new, HYPRE_Int,     nnz_offd_alloc);
                        A_offd_a_new = hypre_TReAlloc(A_offd_a_new, HYPRE_Complex, nnz_offd_alloc);
                     }
   ...
   A_diag_a_new = hypre_TReAlloc(A_diag_a_new, HYPRE_Complex, nnz_diag_new);
   A_offd_j_new = hypre_TReAlloc(A_offd_j_new, HYPRE_Int,     nnz_offd_new);
   A_offd_a_new = hypre_TReAlloc(A_offd_a_new, HYPRE_Complex, nnz_offd_new);
   ...
   row_starts_new = hypre_CTAlloc(HYPRE_Int, j);
   col_starts_new = hypre_CTAlloc(HYPRE_Int, j);
   ...
   *As = Anew;
   ...
   {
      hypre_TFree(dense_all);
   }
   hypre_TFree(IPIV);
   hypre_TFree(dgetri_work);
   hypre_TFree(marker_diag);
   hypre_TFree(marker_newoffd);
   hypre_TFree(offd2new);
                             ...
                             hypre_ParCSRCommPkg **commpkg_out )
{
   ...
   send_i = hypre_CTAlloc(HYPRE_Int, num_rows_send);
   recv_i = hypre_CTAlloc(HYPRE_Int, num_rows_recv + 1);
   ...
   send_j = hypre_CTAlloc(HYPRE_Int, num_nnz_send);
   send_a = hypre_CTAlloc(HYPRE_Complex, num_nnz_send);
   /* pointers to each proc in send_j */
   send_jstarts = hypre_CTAlloc(HYPRE_Int, num_sends + 1);
   ...
   recv_j = hypre_CTAlloc(HYPRE_Int, num_nnz_recv);
   recv_a = hypre_CTAlloc(HYPRE_Complex, num_nnz_recv);
   recv_jstarts = hypre_CTAlloc(HYPRE_Int, num_recvs + 1);
   ...
   comm_pkg_j = hypre_CTAlloc(hypre_ParCSRCommPkg, 1);
   ...
   *A_ext = hypre_CSRMatrixCreate(num_rows_recv, hypre_ParCSRMatrixGlobalNumCols(A), num_nnz_recv);
   ...
   hypre_TFree(send_j);
   hypre_TFree(send_a);
   hypre_TFree(send_jstarts);
   hypre_TFree(recv_jstarts);
   hypre_TFree(comm_pkg_j);
