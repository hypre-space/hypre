                          ...
                          hypre_ParCSRMatrix  *B )
{
   ...
   if (num_procs > 1)
   {
      ...
      HYPRE_Int *Cbar_ii = hypre_CsrRowPtrsToIndicesDevice(hypre_ParCSRMatrixNumRows(A),
                                                           hypre_CSRMatrixNumNonzeros(Cbar),
                                                           hypre_CSRMatrixI(Cbar));

#if defined(HYPRE_USING_SYCL)
      auto new_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(Cbar_ii, hypre_CSRMatrixJ(Cbar),
...
#endif
      hypre_CsrRowIndicesToPtrsDevice_v2(hypre_CSRMatrixNumRows(C_diag), nnz_C_diag, C_diag_ii,
...
#if defined(HYPRE_USING_SYCL)
      new_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(Cbar_ii, hypre_CSRMatrixJ(Cbar),
                                                                  hypre_CSRMatrixData(Cbar)),
                                   oneapi::dpl::make_zip_iterator(Cbar_ii, hypre_CSRMatrixJ(Cbar),
                                                                  hypre_CSRMatrixData(Cbar)) + hypre_CSRMatrixNumNonzeros(Cbar),
                                   hypre_CSRMatrixJ(Cbar),
      ...
      hypre_CsrRowIndicesToPtrsDevice_v2(hypre_CSRMatrixNumRows(C_offd), nnz_C_offd, C_offd_ii,
                                     ...
                                     HYPRE_BigInt       **col_map_offd_C_ptr )
{
   ...
   else
   {
      ...
      // Cext offd
#if defined(HYPRE_USING_SYCL)
      auto off_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(oneapi::dpl::counting_iterator(0),
                                                                       Cext_bigj),
      ...
      // Cext diag
#if defined(HYPRE_USING_SYCL)
      auto dia_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(oneapi::dpl::counting_iterator(0),
                                                                       Cext_bigj),
                                        oneapi::dpl::make_zip_iterator(oneapi::dpl::counting_iterator(0),
                                                                       Cext_bigj) + Cext_nnz,
                                        Cext_bigj,
...
#if defined(HYPRE_USING_SYCL)
      hypre_SyclSequence(ie_ii, ie_ii + num_rows, 0);
      HYPRE_ONEDPL_CALL( std::copy, send_map, send_map + num_elemt, ie_ii + num_rows);
      hypre_SyclSequence(ie_j, ie_j + num_rows + num_elemt, 0);
      ...
      HYPRE_Int *ie_i = hypre_CsrRowIndicesToPtrsDevice(num_rows, num_rows + num_elemt, ie_ii);
   ...
   HYPRE_Int     *zmp_i = hypre_CsrRowPtrsToIndicesDevice(num_rows, local_nnz_C, hypre_CSRMatrixI(Cz));
...
#if defined(HYPRE_USING_SYCL)
   auto new_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(zmp_i, zmp_j, zmp_a),
   ...
   HYPRE_Complex *C_offd_a = hypre_CSRMatrixData(C_offd);
#if defined(HYPRE_USING_SYCL)
   new_end = hypre_SyclCopy_if( oneapi::dpl::make_zip_iterator(zmp_i, zmp_j, zmp_a),
                                oneapi::dpl::make_zip_iterator(zmp_i, zmp_j, zmp_a) + local_nnz_C,
                                zmp_j,
                                oneapi::dpl::make_zip_iterator(C_offd_ii, C_offd_j, C_offd_a),
                                std::not_fn(pred) );
