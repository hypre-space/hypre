                                 ...
                                 hypre_OverlapData  **overlap_data_ptr)
{
   ...
   for (level = 0; level < overlap_order; level++)
   {
      ...
      if (level > 0 && num_external > 0)
      {
         /* We need to fetch the sparsity pattern of external rows to find their neighbors.
          * This is done by calling hypre_ParcsrGetExternalRowsInit/Wait */

         hypre_ParCSRCommPkg *ext_comm_pkg = NULL;
         hypre_CSRMatrix     *ext_rows = NULL;
         void                *request = NULL;

         /* Build comm pkg for external indices */
         ...
         /* Fetch external rows (pattern only, no data needed) */
         hypre_ParcsrGetExternalRowsInit(A, num_external, external_indices, ext_comm_pkg, 0, &request);
         ext_rows = hypre_ParcsrGetExternalRowsWait(request);

         /* Find neighbors of external rows */
         if (ext_rows)
         {
            HYPRE_Int     *ext_i = hypre_CSRMatrixI(ext_rows);
            HYPRE_BigInt  *ext_j = hypre_CSRMatrixBigJ(ext_rows);
            HYPRE_Int      ext_num_rows = hypre_CSRMatrixNumRows(ext_rows);
            ...
            hypre_CSRMatrixDestroy(ext_rows);
         }
      ...
      /* If no new external rows found at this level, we can stop early */
      if (num_potential == 0)
      {
