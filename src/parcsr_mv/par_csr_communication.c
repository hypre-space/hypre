...
hypre_ParCSRPersistentCommHandleCreate( HYPRE_Int job, hypre_ParCSRCommPkg *comm_pkg )
{
   ...
   switch (job_type)
   {
      ...
      case HYPRE_COMM_PKG_JOB_COMPLEX_TRANSPOSE:
         ...
         if (!comm_pkg->use_neighbor || !node_aware_on) {
            ...
            MPIX_Neighbor_alltoallv_init( (HYPRE_Complex *)send_buff, recv_sizes,
                                                   hypre_ParCSRCommPkgRecvVecStarts(comm_pkg),
                                                   HYPRE_MPI_COMPLEX,
      ...
      case HYPRE_COMM_PKG_JOB_INT:
         ...
         if (!comm_pkg->use_neighbor || !node_aware_on) {
            ...
            MPIX_Neighbor_alltoallv_init( (HYPRE_Int *)send_buff, send_sizes,
                                                   hypre_ParCSRCommPkgSendMapStarts(comm_pkg),
                                                   HYPRE_MPI_INT,
      ...
      case HYPRE_COMM_PKG_JOB_INT_TRANSPOSE:
         ...
         if (!comm_pkg->use_neighbor || !node_aware_on) {
            ...
            MPIX_Neighbor_alltoallv_init( (HYPRE_Int *)send_buff, recv_sizes,
                                                   hypre_ParCSRCommPkgRecvVecStarts(comm_pkg),
                                                   HYPRE_MPI_INT,
      ...
      case HYPRE_COMM_PKG_JOB_BIGINT:
         ...
         if (!comm_pkg->use_neighbor || !node_aware_on) {
            ...
            MPIX_Neighbor_alltoallv_init( (HYPRE_BigInt *)send_buff, send_sizes,
                                                   hypre_ParCSRCommPkgSendMapStarts(comm_pkg),
                                                   HYPRE_MPI_BIG_INT,
      ...
      case HYPRE_COMM_PKG_JOB_BIGINT_TRANSPOSE:
         ...
         if (!comm_pkg->use_neighbor || !node_aware_on) {
            ...
            MPIX_Neighbor_alltoallv_init( (HYPRE_BigInt *)send_buff, recv_sizes,
                                                   hypre_ParCSRCommPkgRecvVecStarts(comm_pkg),
                                                   HYPRE_MPI_BIG_INT,
   ...
   //if (rank == 0) { hypre_printf("Comm Pkg Created\n"); }
#endif
...
#ifdef HYPRE_USING_NODE_AWARE_MPI
      if (comm_handle->Xrequest && hypre_HandleUsingNodeAwareMPI(hypre_handle())) {
                                       ...
                                       void                             *send_data )
{
   ...
   //int rank;
   //hypre_MPI_Comm_rank(comm_handle->comm_pkg->comm, &rank);
   //if (rank == 0) { hypre_printf("%d\n", comm_handle->comm_pkg->use_neighbor); }
   if (!comm_handle->comm_pkg->use_neighbor || !hypre_HandleUsingNodeAwareMPI(hypre_handle())) {
      ...
      //int rank;
      //hypre_MPI_Comm_rank(comm_handle->comm_pkg->comm, &rank);
      //if (rank == 0) { hypre_printf("Node-aware starting, %p\n", comm_handle->Xrequest); }
                                      ...
                                      void                             *recv_data )
{
   ...
   //hypre_MPI_Comm_rank(comm_handle->comm_pkg->comm, &rank);
   if (!comm_handle->comm_pkg->use_neighbor || !hypre_HandleUsingNodeAwareMPI(hypre_handle())) {
      //if (rank == 0) { hypre_printf("Standard waiting\n"); }
      if (hypre_ParCSRCommHandleNumRequests(comm_handle) > 0)
      {
      ...
      //if (rank == 0) { hypre_printf("Standard waited\n"); }
   } else {
      //if (rank == 0) { hypre_printf("Node-aware waiting, %p\n", comm_handle->Xrequest); }
      if (comm_handle->Xrequest)
      {
         ...
         hypre_TMemcpy(recv_data,
                       ...
                       recv_memory_location,
                       HYPRE_MEMORY_HOST);
         //if (rank == 0) { hypre_printf("Node-aware waited\n"); }
      }
   }
#else
   if (hypre_ParCSRCommHandleNumRequests(comm_handle) > 0)
