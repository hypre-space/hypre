...
/******************************************************************************
 *
 * Unified instrumentation implementation:
 *   - Caliper region/function marks (when HYPRE_USING_CALIPER)
 *   - GPU profiling ranges NVTX / ROCTX (when HYPRE_USING_NVTX / ROCTX)
 *
 * hypre_GpuProfilingPopRange synchronizes the default compute stream and wraps
 * that in a short nested range ("StreamSync0") before popping the user range, so
 * NVTX/ROCTX timelines stay ordered relative to GPU work.
 *
 *****************************************************************************/
...
/* global variables */
HYPRE_Int hypre__caliper_maxdepth = 6;
HYPRE_Int hypre__caliper_depth = 0;

void
hypre_CaliperMarkFuncBegin(const char *func)
{
   cali_begin_string_byname("function", func);
}

void
hypre_CaliperMarkFuncEnd(void)
{
   cali_end_byname("function");
}

void
hypre_CaliperMarkBegin(const char *name)
{
   cali_begin_region(name);
}

void
hypre_CaliperMarkEnd(const char *name)
{
