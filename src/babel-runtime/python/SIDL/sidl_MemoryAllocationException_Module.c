 ...
 * Revision:      @(#) $Id: sidl_MemoryAllocationException_Module.c,v 1.1 2006/08/29 23:41:48 painter Exp $
...
sidl_MemoryAllocationException_createCast(PyObject *self, PyObject *args,     \
  ...
  int _okay = PyArg_ParseTupleAndKeywords(args, kwds, "|O&zO", _kwlist,       \
    (void *)sidl_MemoryAllocationException__convert, &optarg, &url, &implObj);
  if (_okay) {
...
sidl_MemoryAllocationException__wrap                                          \
  ...
  if (sidlobj) {
    result =                                                                  \
      _sidl_MemoryAllocationExceptionType.tp_new(                             \
...
fails "
  ...
  PyTuple_SetItem(_sidl_MemoryAllocationExceptionType.tp_bases,0,             \
    (PyObject *)sidl_SIDLException_PyType());
  PyTuple_SetItem(_sidl_MemoryAllocationExceptionType.tp_bases,1,             \
    (PyObject *)sidl_RuntimeException_PyType());
  if (PyType_Ready(&_sidl_MemoryAllocationExceptionType) < 0) {
    PyErr_Print();
    fprintf(stderr,                                                           \
      "PyType_Ready on sidl.MemoryAllocationException failed.\n");
    return;
  }
  Py_INCREF(&_sidl_MemoryAllocationExceptionType);
  PyDict_SetItemString(dict, "MemoryAllocationException",                     \
    (PyObject *)&_sidl_MemoryAllocationExceptionType);
