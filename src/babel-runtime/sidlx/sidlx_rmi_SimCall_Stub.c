...
#else
  _externals = (struct sidlx_rmi_SimCall__external*)sidl_dynamicLoadIOR(
    "sidlx.rmi.SimCall","sidlx_rmi_SimCall__externals") ;
...
#if SIDL_C_INLINE_REPEAT_DEFN
{
  enum sidlx_rmi_CallType__enum _result;
  _result = (*self->d_epv->f_getCallType)(
    self,
    _ex);
  return _result;
}
...
sidlx_rmi_SimCall__array_createCol(
  ...
  const int32_t upper[])
{
  return (struct sidlx_rmi_SimCall__array*)sidl_interface__array_createCol(
    dimen, lower, upper);
}
...
sidlx_rmi_SimCall__array_createRow(
  ...
  const int32_t upper[])
{
  return (struct sidlx_rmi_SimCall__array*)sidl_interface__array_createRow(
    dimen, lower, upper);
}
...
{
  return (struct sidlx_rmi_SimCall__array*)sidl_interface__array_create1dInit(
...
sidlx_rmi_SimCall__remoteCreate(const char *url, sidl_BaseInterface *_ex)
{
  ...
  sidl_rmi_InstanceHandle instance = sidl_rmi_ProtocolFactory_createInstance(
    url, "sidlx.rmi.SimCall", _ex ); SIDL_CHECK(*_ex);
  if ( instance == NULL) { return NULL; }
  self =
    (struct sidlx_rmi_SimCall__object*) malloc(
      sizeof(struct sidlx_rmi_SimCall__object));
