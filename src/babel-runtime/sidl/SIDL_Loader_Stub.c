 ...
 * Revision:      @(#) $Id: sidl_Loader_Stub.c,v 1.7 2007/09/27 19:35:44 painter Exp $
...
static sidl_Loader sidl_Loader__remoteCreate(const char* url, 
...
{
  return sidl_Loader__remoteCreate(url, _ex);
}
...
{
  return sidl_Loader__remoteConnect(url, TRUE, _ex);
}
...
{
  sidl_DLL _result;
  _result = (_getSEPV()->f_loadLibrary)(
    uri,
    loadGlobally,
    loadLazy,
    _ex);
...
{
  sidl_DLL _result;
  _result = (_getSEPV()->f_findLibrary)(
    sidl_name,
    target,
    lScope,
    lResolve,
...
{
  char* _result;
  _result = (_getSEPV()->f_getSearchPath)(
    _ex);
  return _result;
}
  ...
  sidl_BaseInterface* _ex)
{
  ...
  if(!connect_loaded) {
    ...
    sidl_rmi_ConnectRegistry_registerConnect("sidl.Loader", (
      void*)sidl_Loader__IHConnect,_ex);SIDL_CHECK(*_ex);
  }
  if (obj != NULL) {
...
sidl_Loader__isLocal(
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  return !sidl_Loader__isRemote(self,_ex);
}
